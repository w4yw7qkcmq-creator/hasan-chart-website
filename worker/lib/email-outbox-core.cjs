...
function resolveEmailRecipientGuardModule() {
  const candidates = [
    path.join(__dirname, "email-recipient-guard.cjs"),
    path.join(__dirname, "../worker/lib/email-recipient-guard.cjs"),
  ];
  ...
  throw new Error("email-recipient-guard.cjs not found for email-outbox-core");
}

const { blockProductionTestRecipientSend } = require(resolveEmailRecipientGuardModule());
