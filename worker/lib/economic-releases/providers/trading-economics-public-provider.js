...
const { parseTradingEconomicsCalendarHtml } = require("./parser/trading-economics-table-parser");
const {
  fingerprintTradingEconomicsCalendarHtml,
  detectTradingEconomicsSchemaChange,
} = require("./parser/trading-economics-fingerprint");

const DEFAULT_TE_CALENDAR_URL =
  process.env.TRADING_ECONOMICS_PUBLIC_CALENDAR_URL || "https://tradingeconomics.com/calendar";

function createTradingEconomicsPublicProvider(options = {}) {
  const name = "trading_economics_public";
  ...
  const calendarUrl = options.calendarUrl || DEFAULT_TE_CALENDAR_URL;
  ...
  async function fetchSchedule({ forceRefresh = false, scheduledAt = null } = {}) {
    ...
    if (httpState.blockedUntil && Date.now() < new Date(httpState.blockedUntil).getTime()) {
      ...
      metrics.lastErrorSafe = httpState.lastErrorSafe || "provider_blocked";
      return cache.events;
    }
    ...
    const parsed = parseTradingEconomicsCalendarHtml(response.body);
    if (!parsed.events.length || !parsed.strategy) {
    ...
    const nextFingerprint = fingerprintTradingEconomicsCalendarHtml(response.body);
    if (
      cache.fingerprint &&
      detectTradingEconomicsSchemaChange(cache.fingerprint, nextFingerprint) &&
      parsed.strategy !== cache.parserStrategy
    ) {
      metrics.schemaChanges += 1;
      metrics.lastErrorSafe = "parser_schema_changed";
      metrics.providerStatus = "parser_schema_changed";
  ...
  async function fetchRelease(canonical, options = {}) {
    ...
    const matched = events.filter((event) => {
      if (canonical?.eventKey?.startsWith("US_") && event.country !== "US") {
        return false;
      }

      const titleCandidates = [event.title, event.dataEvent, event.category].filter(Boolean);
      const titleMatch = titleCandidates.some((title) => calendarTitleMatchesCanonical(title, canonical));
      if (!titleMatch) {
        return false;
      }
...
module.exports = {
  createTradingEconomicsPublicProvider,
  DEFAULT_TE_CALENDAR_URL,
};
