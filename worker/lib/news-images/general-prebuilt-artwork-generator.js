...
const { listManifestEntries, getPoolBaseDir, CATEGORY_CONTEXT_LABELS } = require("./general-prebuilt-artwork-manifest");
const {
  ARTWORK_PROMPT_DEFINITIONS,
  getPromptDefinitionMap,
  validatePromptDefinitionsAgainstManifest,
} = require("./general-prebuilt-artwork-prompts");
const {
  CANVAS_SIZE,
  composeGeneralPrebuiltArtwork,
  inspectSquareBackgroundForTypography,
} = require("./general-prebuilt-square-composer");
const { inspectBackgroundExposure } = require("./general-prebuilt-background-exposure-guard");
const {
  createSceneOpenAIImageProvider,
} = require("./economic-fast-lane-artwork-generator");
const { isTransientImageError } = require("./image-error-classifier");
const { validateGeneralPrebuiltArtwork } = require("./validate-general-prebuilt-artwork");

const SAFETY_ENV = "ALLOW_GENERAL_PREBUILT_ARTWORK_GENERATION";
...
const FORBIDDEN_MODULE_PATTERNS = [
  ...
  "quality-gate",
  "economic-image-pool",
  "economic-fast-lane-artwork-manifest",
];
...
function assertSafetyGate(options = {}) {
  ...
  if (process.env[SAFETY_ENV] !== "1") {
    const message = [
      "General prebuilt artwork generation blocked.",
...
async function processBackgroundToJpeg(backgroundBuffer, promptDefinition, options = {}) {
  const exposureInspection = await inspectBackgroundExposure(backgroundBuffer, options);
  if (!exposureInspection.acceptedForComposition) {
    const error = new Error("BACKGROUND_UNDEREXPOSED");
    error.code = "BACKGROUND_UNDEREXPOSED";
    error.exposureInspection = exposureInspection;
    throw error;
  }
  ...
  const composed = await composeGeneralPrebuiltArtwork(
    backgroundBuffer,
    {
      displayTitle: promptDefinition.displayTitle,
      subtitle: promptDefinition.subtitle,
      categoryLabel: CATEGORY_CONTEXT_LABELS[promptDefinition.category] || "Breaking News",
    },
    options
  );
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS_PER_ASSET; attempt += 1) {
    try {
      ...
      const retryable =
        ...
        error.code === "AI_TYPOGRAPHY_REJECTED" ||
        error.code === "BACKGROUND_UNDEREXPOSED" ||
        error.reasonCode === "AI_OUTPUT_REJECTED";
...
async function runGenerator(options = {}) {
  ...
  summary.validation = validateGeneralPrebuiltArtwork({
