...
function buildTelegramPublicationRequest(candidate, validation, ctx = {}) {
  ...
  return {
    ...
    rawSourceText: candidate.post?.sourceRawText || candidate.post?.rawText || null,
    destination: DESTINATIONS.BOTH,
    sourceLink,
    importance,
    canonicalDisplayName: candidate.facts?.canonicalDisplayName || null,
    facts: {
      ...
      sourceReading: candidate.facts?.sourceReading || null,
      sourceReadingRaw: candidate.facts?.sourceReadingRaw || candidate.facts?.sourceReading?.raw || null,
      publishedReading: candidate.facts?.publishedReading || null,
      canonicalDisplayName: candidate.facts?.canonicalDisplayName || null,
      canonicalEventId: candidate.facts?.canonicalEventId || candidate.facts?.canonicalEventKey || null,
      countryCode: candidate.facts?.countryCode || null,
      institution: candidate.facts?.institution || null,
      sector: candidate.facts?.sector || null,
      sourceEventName: candidate.facts?.sourceEventName || candidate.facts?.title || null,
    },
    metadata: {
      candidate,
