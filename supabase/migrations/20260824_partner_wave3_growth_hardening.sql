-- Wave 3 growth hardening: atomic campaign budgets + gross-spend accounting
-- Additive only. Extends create_partner_growth_reward_atomic.

BEGIN;

ALTER TABLE public.partner_campaign_programs
  ADD COLUMN IF NOT EXISTS global_budget_amount numeric(12, 2)
    CHECK (global_budget_amount IS NULL OR global_budget_amount >= 0),
  ...
  ADD COLUMN IF NOT EXISTS amount_reversed numeric(12, 2) NOT NULL DEFAULT 0
    CHECK (amount_reversed >= 0),
  ADD COLUMN IF NOT EXISTS per_partner_reward_cap numeric(12, 2)
    CHECK (per_partner_reward_cap IS NULL OR per_partner_reward_cap >= 0);

COMMENT ON COLUMN public.partner_campaign_programs.amount_spent IS
  'Gross campaign rewards credited atomically; not reduced on reversal (see amount_reversed).';
COMMENT ON COLUMN public.partner_campaign_programs.amount_reversed IS
  'Tracked reversed campaign-linked rewards; budget slots are not released back to prevent gaming.';
...
BEGIN
  ...
  IF v_campaign_id IS NOT NULL THEN
    ...
    IF v_campaign.per_partner_reward_cap IS NOT NULL THEN
      ...
      IF round(v_partner_spent + v_ent.amount, 2) > v_campaign.per_partner_reward_cap THEN
