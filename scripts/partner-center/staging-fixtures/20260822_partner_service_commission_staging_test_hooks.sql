-- STAGING_TEST_ONLY — NOT part of production migration rollout.
-- Apply manually on Staging via: scripts/partner-center/r8-staging-harness-lib.mjs (applyTestHooksMigration)

CREATE TABLE IF NOT EXISTS public.partner_center_staging_test_flags (
  flag_key text PRIMARY KEY,
  flag_value text NOT NULL DEFAULT '',
  updated_at timestamptz NOT NULL DEFAULT now()
);

REVOKE ALL ON TABLE public.partner_center_staging_test_flags FROM PUBLIC, anon, authenticated;
GRANT ALL ON TABLE public.partner_center_staging_test_flags TO service_role;
...
AS $$
BEGIN
  INSERT INTO public.partner_center_staging_test_flags (flag_key, flag_value)
  VALUES ('create_fail_after', coalesce(p_fail_after, 'commission'))
  ON CONFLICT (flag_key) DO UPDATE
    SET flag_value = EXCLUDED.flag_value, updated_at = now();
END;
$$;

REVOKE ALL ON FUNCTION public.create_partner_commission_atomic_test_fail(text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.create_partner_commission_atomic_test_fail(text) TO service_role;

CREATE OR REPLACE FUNCTION public.reverse_partner_service_commission_atomic_test_fail(
  p_fail_after text DEFAULT 'ledger'
)
RETURNS void
LANGUAGE plpgsql
SET search_path = public, pg_temp
...
BEGIN
  INSERT INTO public.partner_center_staging_test_flags (flag_key, flag_value)
  VALUES ('reverse_fail_after', coalesce(p_fail_after, 'ledger'))
  ON CONFLICT (flag_key) DO UPDATE
    SET flag_value = EXCLUDED.flag_value, updated_at = now();
END;
$$;

REVOKE ALL ON FUNCTION public.reverse_partner_service_commission_atomic_test_fail(text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.reverse_partner_service_commission_atomic_test_fail(text) TO service_role;

-- Recreate create RPC with commission test-fail injection (after commission insert, before ledger).
...
BEGIN
  ...
  SELECT f.flag_value INTO v_fail_flag
  FROM public.partner_center_staging_test_flags f
  WHERE f.flag_key = 'create_fail_after'
  LIMIT 1;

  IF coalesce(v_fail_flag, '') <> '' THEN
    DELETE FROM public.partner_center_staging_test_flags WHERE flag_key = 'create_fail_after';
    RETURN jsonb_build_object(
      'created', false,
      'duplicate', false,
      'error', 'commission_test_fail_injected'
    );
  END IF;
...
-- Recreate reverse RPC with reversal test-fail injection (after amount calc, before balance mutation).
...
BEGIN
  ...
  SELECT f.flag_value INTO v_fail_flag
  FROM public.partner_center_staging_test_flags f
  WHERE f.flag_key = 'reverse_fail_after'
  LIMIT 1;

  IF coalesce(v_fail_flag, '') <> '' THEN
    DELETE FROM public.partner_center_staging_test_flags WHERE flag_key = 'reverse_fail_after';
    RETURN jsonb_build_object(
      'reversed', false,
      'duplicate', false,
      'error', 'reverse_test_fail_injected',
...
-- Ensure PostgREST always resolves the patched 13-arg create RPC (drop legacy 12-arg overload).
...
-- Staging harness cleanup: ledger append-only blocks commission DELETE via ON DELETE SET NULL.
CREATE OR REPLACE FUNCTION public.partner_center_staging_purge_run_commissions(
  p_partner_ids uuid[],
...
BEGIN
  ...
  FROM public.partner_commissions c
  WHERE c.partner_id = ANY(p_partner_ids)
  ...
  DELETE FROM public.partner_financial_ledger_entries WHERE legacy_commission_id = ANY(v_ids);
  DELETE FROM public.partner_service_commission_reversals WHERE commission_id = ANY(v_ids);
  DELETE FROM public.partner_commissions WHERE id = ANY(v_ids);
...
REVOKE ALL ON FUNCTION public.partner_center_staging_purge_run_commissions(uuid[], timestamptz) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.partner_center_staging_purge_run_commissions(uuid[], timestamptz) TO service_role;
