...
export const SUBSCRIPTION_TIMELINE_EVENT_META = {
  ...
  ended: { icon: "🔴", color: "red", title: "إزالة الاشتراك" },
};
...
function buildEndedDescription(details, adminEmail) {
  const lines = [];
  const notes = String(details?.removalNotes || details?.adminNotes || "").trim();

  if (notes) {
    lines.push(`ملاحظات الإدارة:\n${notes}`);
  } else {
    lines.push("تم إنهاء الاشتراك وإزالة الخدمة عن المستخدم.");
  }

  if (Object.prototype.hasOwnProperty.call(details || {}, "notificationCreated")) {
    lines.push(`الإشعار الداخلي: ${details.notificationCreated ? "تم" : "تعذر"}`);
  }

  if (Object.prototype.hasOwnProperty.call(details || {}, "emailQueued")) {
    lines.push(
      `إيميل الإنهاء: ${details.emailQueued ? "تم وضعه في قائمة الإرسال" : "تعذر"}`
    );
  }
...
function mapAuditLogToTimelineEvents(logRow, { hasDedicatedRejectLog = false } = {}) {
  ...
  if (action === "remove-subscription-request") {
    return [
      buildTimelineEvent("ended", {
        occurredAt,
        description: buildEndedDescription(details, adminEmail),
        adminEmail,
        dedupeKey: `remove:${logId}`,
      }),
    ].filter(Boolean);
  }
...
function buildSyntheticTimelineEvents(requestRow = {}) {
  ...
  if (
    String(requestRow.payment_proof_path || "").trim() ||
    String(requestRow.payment_proof || "").trim() ||
    requestRow.has_payment_proof
  ) {
    events.push(
...
export async function fetchSubscriptionRequestAuditLogs(supabase, requestIds = []) {
  ...
  if (error) {
    console.error("ADMIN_SUBSCRIPTIONS_TIMELINE_FAILED", {
      step: "fetch_admin_logs",
  ...
  console.info("ADMIN_SUBSCRIPTIONS_TIMELINE_STEP", {
    step: "fetch_admin_logs_ok",
    durationMs: Date.now() - startedAt,
    requestCount: normalizedIds.length,
...
function enrichSingleSubscriptionRequestWithTimeline(row, logsByTarget) {
  const requestId = String(row?.id || "").trim();
  const requestLogs = logsByTarget.get(requestId) || [];
  ...
  try {
    ...
    console.error("ADMIN_SUBSCRIPTIONS_TIMELINE_FAILED", {
      step: "build_timeline_for_request",
      requestId,
      message: error?.message || String(error),
      stack: error?.stack || null,
    });
    ...
    return {
      ...
      timeline_error: "تعذر بناء سجل هذا الطلب",
    };
  }
}
...
export async function enrichSubscriptionRequestsWithTimeline(supabase, rows = []) {
  ...
  try {
    auditLogs = await fetchSubscriptionRequestAuditLogs(supabase, requestIds);
  } catch (error) {
    console.error("ADMIN_SUBSCRIPTIONS_TIMELINE_FAILED", {
      step: "fetch_admin_logs_unhandled",
  ...
  const enriched = rows.map((row) =>
    enrichSingleSubscriptionRequestWithTimeline(row, logsByTarget)
  );

  const failedCount = enriched.filter((row) => row.timeline_error).length;
  console.info("ADMIN_SUBSCRIPTIONS_TIMELINE_STEP", {
    step: "enrich_subscription_requests_ok",
    durationMs: Date.now() - enrichStartedAt,
    requestCount: rows.length,
    failedTimelineCount: failedCount,
  });
