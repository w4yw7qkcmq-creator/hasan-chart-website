...
function hasActiveFilters(filters = {}) {
  return Boolean(
    String(filters.email || "").trim() ||
      (String(filters.status || "").trim() && filters.status !== "all") ||
      (String(filters.messageType || "").trim() && filters.messageType !== "all") ||
      String(filters.dateFrom || "").trim() ||
      String(filters.dateTo || "").trim()
  );
}

async function countMessagesWhere(supabase, filters, mutate) {
  let query = supabase.from("email_messages").select("id", { count: "exact", head: true });
  query = applyMessageFilters(query, filters);
  query = mutate(query);
  const { count, error } = await query;
  if (error) throw new Error(error.message || "Failed to count email messages");
  return count || 0;
}

async function fetchAggregatedEmailAnalytics(supabase, filters = {}) {
  const now = Date.now();
  ...
  const [
    ...
    countMessagesWhere(supabase, filters, (q) => q),
    countMessagesWhere(supabase, filters, (q) =>
      q.or("status.eq.delivered,delivered_at.not.is.null")
    ),
    countMessagesWhere(supabase, filters, (q) => q.eq("status", "failed")),
    countMessagesWhere(supabase, filters, (q) => q.eq("status", "bounced")),
    countMessagesWhere(supabase, filters, (q) => q.eq("status", "complained")),
    countMessagesWhere(supabase, filters, (q) => q.or("open_count.gt.0,opened.eq.true")),
    countMessagesWhere(supabase, filters, (q) => q.or("click_count.gt.0,clicked.eq.true")),
    countMessagesWhere(supabase, filters, (q) => q.gte("sent_at", last24Iso)),
    countMessagesWhere(supabase, filters, (q) => q.gte("sent_at", lastHourIso)),
  ]);
  ...
  const { data: chartRows, error: chartError } = await supabase
    .from("email_messages")
    .select(EMAIL_MESSAGE_CHART_COLUMNS)
    .order("sent_at", { ascending: false, nullsFirst: false })
    .order("created_at", { ascending: false })
    .limit(1500);

  if (chartError) {
    throw new Error(chartError.message || "Failed to load chart series source");
  }

  const chartList = chartRows || [];
  const analyticsFromChart = computeEmailAnalytics(chartList);
  const topMessageType = analyticsFromChart.todayActivity.topMessageType;
  const topMessageTypeCount = analyticsFromChart.todayActivity.topMessageTypeCount;
  ...
  return {
    ...
    todayActivity: {
      last24Hours,
      lastHour,
      averageSendTime: analyticsFromChart.todayActivity.averageSendTime,
      averageSendTimeMs: analyticsFromChart.todayActivity.averageSendTimeMs,
      topMessageType,
      topMessageTypeCount,
    },
    chartSeries: buildChartSeries(chartList),
  };
}

export async function fetchEmailAnalyticsData(
  supabase,
  { limit = 100, syncResend = false, filters = {} } = {}
) {
  if (syncResend) {
    void syncRecentEmailsFromResend(supabase).catch(() => null);
  }
  ...
  let analytics;
  if (!filtered) {
    analytics = await fetchAggregatedEmailAnalytics(supabase, filters);
  } else {
    let query = supabase
      .from("email_messages")
      .select(EMAIL_MESSAGE_COLUMNS)
    ...
    if (error) {
      if (/relation .* does not exist/i.test(error.message || "")) {
        analytics = computeEmailAnalytics([]);
      } else {
        throw new Error(error.message || "Failed to load email analytics");
      }
    } else {
      analytics = computeEmailAnalytics(messages || []);
    }
  }
  ...
  listQuery = applyMessageFilters(listQuery, filters);

  const { data: listMessages, error: listError } = await listQuery;

  if (listError) {
    if (/relation .* does not exist/i.test(listError.message || "")) {
      const empty = analytics || computeEmailAnalytics([]);
      return {
        ok: true,
    ...
    throw new Error(listError.message || "Failed to load email analytics rows");
  }
  ...
  return {
    ...
    todayActivity: analytics.todayActivity,
