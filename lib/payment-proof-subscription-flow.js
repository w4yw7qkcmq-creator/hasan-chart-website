...
import { validatePaymentNetwork } from "./payment-networks.js";
import { assertUploadSessionPlanIntegrity } from "./subscription-plan-registry.js";

const SESSION_SELECT =
  "id,user_id,user_email,username,plan_name,category,price,telegram_username,payment_network,object_path,declared_mime_type,declared_size_bytes,nonce,status,subscription_request_id,failure_reason,expires_at,completed_at,failed_at";

const REQUEST_SELECT =
  "id,user_email,status,plan_name,category,price,telegram_username,payment_network,payment_proof_path,payment_proof_mime_type,payment_proof_size_bytes";
...
export async function initUploadSession(
  ...
  if (!networkValidation.ok) {
    throw Object.assign(new Error(networkValidation.error), {
      status: 400,
...
export async function finalizePaymentProofUpload(
  ...
  const planIntegrity = assertUploadSessionPlanIntegrity(session);
  if (!planIntegrity.ok) {
    await markUploadSessionFailed(supabase, session.id, {
      userId,
      userEmail,
      reason: planIntegrity.code,
      objectPath: normalizedObjectPath,
    }).catch(() => {});
    throw Object.assign(new Error(planIntegrity.error), {
      status: 400,
      code: planIntegrity.code,
    });
  }
    ...
    .insert([
      {
        ...
        payment_network: session.payment_network,
