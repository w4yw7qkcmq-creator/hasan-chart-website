...
/**
 * App data (cache, rate limits, market pulse) uses in-process memory only.
 * Single-replica deployment — no cross-instance Redis sync needed.
 */
export function shouldUseRedis() {
  return false;
}

/** Health probe only — at most one ping per REDIS_HEALTH_CACHE_MS. */
export function shouldUseRedisHealthProbe() {
