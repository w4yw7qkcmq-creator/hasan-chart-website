...
export const PENDING_ACCOUNT_MANAGEMENT_STATUSES = new Set([
  "pending",
  "new",
  "reviewing",
  "قيد المراجعة",
...
export const INACTIVE_ACCOUNT_MANAGEMENT_STATUSES = new Set([
  "موقوف",
  "ملغى",
  "مغلق",
  "منتهي",
...
export const VIP_FOREX_CATEGORY_LABELS = new Set([
  "باقات الفوركس",
  "forex",
  "Forex",
]);
...
export const VIP_FOREX_PLAN_LABELS = new Set([
  "VIP Forex",
  "VIP Forex Monthly",
]);
...
export function normalizeAdminUserServiceType(row, { sourceTable } = {}) {
  ...
  if (
    VIP_FOREX_PLAN_LABELS.has(planName) ||
    VIP_FOREX_CATEGORY_LABELS.has(category) ||
    /forex/.test(combined) ||
    combined.includes("\u0641\u0648\u0631\u0643\u0633")
  ) {
    return ADMIN_SERVICE_TYPES.VIP_FOREX;
  }
...
export function isVipServiceType(serviceType) {
  return (
    ...
    serviceType === ADMIN_SERVICE_TYPES.VIP_FOREX ||
...
export function isPendingAccountManagementRequest(row) {
  if (!row) return false;
  const status = String(row.status || "").trim();
  const normalized = status.toLowerCase();
  return (
    PENDING_ACCOUNT_MANAGEMENT_STATUSES.has(status) ||
    PENDING_ACCOUNT_MANAGEMENT_STATUSES.has(normalized)
  );
}

export function isInactiveAccountManagementRequest(row) {
  if (!row) return false;
  if (isActiveAccountManagementRequest(row) || isPendingAccountManagementRequest(row)) return false;
  ...
  if (
    INACTIVE_ACCOUNT_MANAGEMENT_STATUSES.has(status) ||
    INACTIVE_ACCOUNT_MANAGEMENT_STATUSES.has(normalized)
  ) {
    return true;
  }

  return /expired|منته|موقوف|ملغ|cancel|closed|disabled|inactive|reject|suspend|مغلق|مؤرشف|ended/.test(
    normalized
  );
}
...
export function isInactivePriceAlertRow(row) {
  if (!row) return false;
  if (isActivePriceAlertRow(row)) return false;

  const status = String(row.status || "").trim().toLowerCase();
  if (!status) return false;

  return /expired|disabled|inactive|cancel|deleted|off|موقوف|منته|ملغ|معطل|ended|closed/.test(status);
}
