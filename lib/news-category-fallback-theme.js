import { detectNewsCategory, getNewsCategoryVisual } from "./news-images";
...
function getCategoryDisplayLabel(category, item = {}) {
  const text = getNewsText(item);

  if (category === "commodities" && /gold|xau|silver|metals|commodit|ذهب|فضة|معادن/.test(text)) {
    return "المعادن والذهب";
  }

  return getNewsCategoryVisual(category).label;
}
...
export function getNewsCategoryFallbackTheme(item = {}, options = {}) {
  ...
  const themeKey = isUrgent
    ? NEWS_FALLBACK_THEME_KEYS.breaking
    : resolveCategoryThemeKey(item, category);
  const theme = NEWS_CATEGORY_FALLBACK_THEMES[themeKey] || NEWS_CATEGORY_FALLBACK_THEMES.default;
  const categoryVisual = getNewsCategoryVisual(category);

  return {
    themeKey,
    categoryLabel: getCategoryDisplayLabel(category, item),
