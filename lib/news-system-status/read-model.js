...
const PUBLIC_CHART_QUOTA_AUTHORITY_BUCKET = "authority";
...
async function loadPublicChartQuotaAuthoritySnapshot(supabase) {
  const { data, error } = await supabase
    .from("news_system_metric_snapshots")
    .select("metrics,bucket_start,updated_at,created_at")
...
function buildChartVisualPolicyReadModel(authoritySnapshot, heartbeatChartPolicy = null) {
  ...
  const authorityHealthy = metrics.authorityHealthy !== false && Boolean(authoritySnapshot);
  ...
  return {
    ...
    authorityHealthy,
    authorityMode: metrics.authorityMode || (authoritySnapshot ? "distributed" : "unknown"),
    authorityUpdatedAt: authoritySnapshot?.updated_at || authoritySnapshot?.created_at || null,
...
export async function getNewsSystemStatusFromDb(supabase) {
  ...
  const chartVisualPolicy = buildChartVisualPolicyReadModel(chartQuotaAuthoritySnapshot, heartbeatChartPolicy);
...
export { isProductionDecision, isCanaryDecision };
