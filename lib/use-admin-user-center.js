...
export function useAdminUserCenter({
  ...
  const runAction = async (
    ...
    let actionMessage = "اكتمل الإجراء بنجاح";

    try {
      const result = await postAdminUserAction(adminFetch, userId, {
        action,
      ...
      actionMessage = result?.message || actionMessage;
      ...
      actionMessage = error?.message || "تعذر تنفيذ العملية";
    } finally {
    ...
    showAdminToast(
      actionSucceeded ? "تم التنفيذ" : toastOutcome.title,
      actionSucceeded ? actionMessage : toastOutcome.body,
      toastOutcome.type
    );
    ...
    if (!refreshResult.ok) {
      console.warn("Admin action refresh failed:", refreshResult.message);
    }
