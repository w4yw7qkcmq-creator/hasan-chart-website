...
import {
  PARTNER_ACHIEVEMENT_DEFINITION_COLUMNS,
  PARTNER_USER_ACHIEVEMENT_COLUMNS,
...
export async function listPartnerAchievementDefinitions(supabase) {
  const { data, error } = await supabase
    ...
    .select(PARTNER_ACHIEVEMENT_DEFINITION_COLUMNS)
    .eq("is_active", true)
...
async function unlockAchievement(supabase, partnerId, achievementKey, metadata = {}) {
  const { data, error } = await supabase
    ...
    .select(PARTNER_USER_ACHIEVEMENT_COLUMNS)
    .single();
