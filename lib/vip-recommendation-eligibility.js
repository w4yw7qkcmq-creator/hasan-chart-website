...
import {
  matchesForexPlanText,
  matchesFuturesPlanText,
  matchesSpotPlanText,
...
export function matchesSignalSubscription(planText, signalType) {
  const text = String(planText || "").toLowerCase();
  const normalizedType = normalizeVipSignalType(signalType);
  ...
  if (normalizedType === "futures") {
    return matchesFuturesPlanText(text);
  }

  if (normalizedType === "forex") {
    return matchesForexPlanText(text);
  }

  return matchesSpotPlanText(text);
}
