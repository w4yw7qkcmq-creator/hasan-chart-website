...
export function resolveStoredOrComputedClassification(profile = {}, authUser = null) {
  ...
  if (stored && Object.values(USER_CLASSIFICATION).includes(stored)) {
    let source = "stored";
    if (storedSource === "admin_manual") source = "admin_manual";
    else if (storedSource === "backfill_high_confidence") source = "backfill_high_confidence";

    return {
      classification: stored,
      confidence: source === "admin_manual" ? "admin" : "stored",
