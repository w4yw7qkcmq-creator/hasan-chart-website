...
export async function linkPartnerRegistration(
  ...
  try {
    ...
    if (!isFinancialRpcAvailable(error)) {
      logPartnerCenterFailure("signup_bonus.rpc_unavailable_fail_closed", {
        partnerId: referrer.id,
        referralId: referralRow.id,
  ...
  if (!usedLegacyBonus && !bonusResult.failClosed) {
    ...
    const { computeSignupVelocityContext } = await import("./security/account-risk-signals.js");
    const velocity =
      velocityContext ||
      (await computeSignupVelocityContext(supabase, {
        clientIp,
        deviceToken,
        partnerId: referrer.id,
      }).catch(() => null));
    ...
    const { data: referredProfile } = await supabase
      .from("profiles")
      .select("effective_user_classification, user_classification")
      .eq("id", normalizedUserId)
      .maybeSingle();
    const referredClassification = String(
      referredProfile?.effective_user_classification ||
        referredProfile?.user_classification ||
        "unknown"
    ).toLowerCase();
    ...
    await runPartnerCenterBridge("signup_linked", () =>
      onPartnerSignupLinked(supabase, {
        ...
        recentSignupCount: velocity?.recentSignupCount || 0,
        recentNetworkSignupCount: velocity?.recentNetworkSignupCount || 0,
        deviceAccountCount24h: velocity?.deviceAccountCount24h || 0,
        partnerNetworkSignup24h: velocity?.partnerNetworkSignup24h || 0,
        partnerUserId: referrer.user_id,
        deviceToken,
        clientIp,
        email,
        classificationBlocked: ["test", "e2e", "internal"].includes(referredClassification),
        classification: referredClassification,
      })
    );
  }
...
export async function getPartnerDashboard(supabase, userId, username) {
  ...
  const [referralsResult, commissionsResult, withdrawalsResult, upgradeResult] =
    await Promise.all([
    supabase
      .from("partner_referrals")
      .select(
    ...
    supabase
      ...
      .order("created_at", { ascending: false })
      .limit(20),
    runAutoTierUpgrade(supabase, partner.id, { userId: partner.user_id }),
  ]);
  ...
  const tierProgress =
    upgradeResult?.tierProgress ||
    (await getPartnerTierProgress(supabase, partner.id));
