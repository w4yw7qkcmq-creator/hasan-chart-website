...
function randomReferralChars(length) {
  ...
  for (let index = 0; index < length; index += 1) {
    output += REFERRAL_CHARSET[bytes[index] % REFERRAL_CHARSET.length];
  }
...
export function generateReferralCode(username) {
  const seed = buildReferralCodeSeed(username).slice(0, 5);
  const variant = new Uint8Array(1);
  crypto.getRandomValues(variant);
  const pick = variant[0] % 3;

  if (pick === 0) {
    return `HC${randomReferralChars(5)}`;
  }

  if (pick === 1) {
    return `HSC${randomReferralChars(4)}`;
  }

  return `${seed}${randomReferralChars(4)}`;
}
...
export async function linkPartnerRegistration(
  ...
  return { linked: true, partnerId: referrer.id, referralId: referralRow.id };
}
