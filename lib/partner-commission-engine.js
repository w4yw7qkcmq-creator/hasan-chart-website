...
} from "./partner-commission-rules.js";
import {
  resolvePartnerCommissionPercent,
} from "./partner-tiers.js";
import { recordCommissionReleaseLedger } from "./partner-wallet.js";
import { runPartnerAutomationAfterCommission } from "./partner-automation.js";
import { partnerLogger } from "./partner-logger.js";
import { writePartnerAuditLog } from "./partner-monitoring.js";
import { PARTNER_COMMISSION_COLUMNS } from "./supabase-query-columns.js";
import {
  createPartnerCommissionAtomic,
  releasePartnerCommissionAtomic,
  isFinancialRpcAvailable,
} from "./partner-center/financial-gateway.js";
import { onPartnerServiceActivatedBridge } from "./partner-center/integration.js";
import { logPartnerCenterFailure, logPartnerCenterEvent } from "./partner-center/observability.js";
import {
  assessServiceCommissionEligibility,
  resolveEffectiveCommissionPercent,
  buildRuleSnapshot,
} from "./partner-center/service-commission-policy.js";
import { upsertPendingQualificationEntitlement } from "./partner-center/service-commission-bridge.js";
...
async function activateReferralIfNeeded(supabase, referralId) {
  ...
  if (referralRow?.id) {
    try {
      await onPartnerServiceActivatedBridge(supabase, {
        partnerId: referralRow.partner_id,
        referralId: referralRow.id,
        referredUserId: referralRow.referred_user_id,
      });
    } catch (error) {
      if (error?.code !== "42P01") {
        logPartnerCenterFailure("integration.service_activation_qualification_failed", {
          referralId,
...
export async function createPartnerCommissionForService(
  ...
  if (!rule?.is_active || rule.is_enabled === false) {
    logPartnerCenterEvent("SERVICE_COMMISSION_SKIPPED", {
      serviceType: normalizedServiceType,
      reason: rule ? "disabled_rule" : "missing_commission_rule",
    });
    return { created: false, reason: rule ? "inactive_rule" : "missing_commission_rule" };
  }
  ...
  const tierPercent = await resolvePartnerCommissionPercent(supabase, partner.id);
  const commissionPercent = resolveEffectiveCommissionPercent(rule, tierPercent);
  ...
  if (!skipQualificationGate) {
    const eligibility = await assessServiceCommissionEligibility(supabase, {
    ...
    if (!eligibility.eligible) {
      if (eligibility.pendingQualification) {
        await upsertPendingQualificationEntitlement(supabase, {
          partnerId: partner.id,
          referralId: referral.id,
          referredUserId: normalizedUserId,
          serviceType: normalizedServiceType,
  ...
  try {
    atomicResult = await createPartnerCommissionAtomic(supabase, {
      partnerId: partner.id,
      referralId: referral.id,
      referredUserId: normalizedUserId,
      serviceType: normalizedServiceType,
    ...
    if (!isFinancialRpcAvailable(error)) {
      ...
      await addPendingBalance(supabase, partner.id, expectedAmount);
      atomicResult = {
        created: true,
        duplicate: false,
  ...
  if (atomicResult.duplicate) {
    return {
      created: false,
      reason: "duplicate",
      commissionId: atomicResult.commissionId,
    };
  }

  if (!atomicResult.created) {
    return { created: false, reason: "atomic_create_failed" };
  }

  const commissionId = atomicResult.commissionId;
  const amount = atomicResult.amount;

  await supabase
    .from("partner_commissions")
    .update({ rule_snapshot: snapshot, updated_at: new Date().toISOString() })
    .eq("id", commissionId)
    .then(({ error }) => {
  ...
  try {
    ...
    partnerLogger.commission("created", {
      ...
      amount,
      serviceType: normalizedServiceType,
      payoutHold: atomicResult.payoutHold,
    });
    ...
    await writePartnerAuditLog("commission.created", {
      ...
      serviceType: normalizedServiceType,
      payoutHold: atomicResult.payoutHold,
      fraudRisk: atomicResult.fraudRisk,
    });
  } catch (postProcessError) {
    logPartnerCenterFailure("commission.post_process_failed", {
      partnerId: partner.id,
      commissionId,
      serviceType: normalizedServiceType,
      reason: postProcessError?.message || String(postProcessError),
    });
  }
...
export async function releaseCommissionToWithdrawable(supabase, commissionId) {
  ...
  const { data: commission, error: commissionError } = await supabase
    .from("partner_commissions")
    .select("id, partner_id, amount, status, is_withdrawable, payout_hold, referral_id, user_id")
  ...
  if (commission.payout_hold) {
    throw new Error("PAYOUT_HOLD_ACTIVE");
  }
  ...
  try {
    releaseResult = await releasePartnerCommissionAtomic(supabase, commission.id);
  } catch (error) {
    if (!isFinancialRpcAvailable(error)) {
      const { data: partner, error: partnerError } = await supabase
        .from("partners")
      ...
      const { data: legacyUpdated, error: updateCommissionError } = await supabase
        .from("partner_commissions")
        .update({
          status: "withdrawable",
      ...
      updatedCommission = legacyUpdated;
      releaseResult = { released: true, legacyFinancialPath: true };
    } else if (
      error.message?.includes("payout_hold_active") ||
      error.message?.includes("fraud_blocks_payable")
    ) {
      throw new Error("FRAUD_GATE_BLOCKED");
    } else {
      throw error;
    }
  }
  ...
  if (!updatedCommission) {
    if (releaseResult.blocked || !releaseResult.released) {
      throw new Error(releaseResult.reason || "RELEASE_BLOCKED");
    }

    const { data: fetchedCommission, error: fetchError } = await supabase
      .from("partner_commissions")
      .select(PARTNER_COMMISSION_COLUMNS)
      .eq("id", commission.id)
      .single();
    ...
    updatedCommission = fetchedCommission;
  }
  ...
  const nextWithdrawable = Number(partner.balance_withdrawable || 0);

  await recordCommissionReleaseLedger(supabase, {
    partnerId: commission.partner_id,
    commissionId: updatedCommission.id,
    amount,
    balanceBefore: Math.max(0, nextWithdrawable - amount),
    balanceAfter: nextWithdrawable,
    note: `Commission ${updatedCommission.id} released (legacy wallet audit)`,
  });
