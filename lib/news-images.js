...
const SITE_BRANDING_IMAGE_PATTERN =
  /(?:^|\/)favicon(?:-\d+)?\.(?:png|ico|webp)?(?:$|\?)|(?:^|\/)brand\/|hasan-chart|site-logo|logo\.(?:png|svg|webp)/i;
...
export function getNewsImageSources(url) {
  const src = normalizeNewsImageUrl(url);
  if (!src) {
    return { src: null, webpSrc: null };
  }

  if (/\.webp(?:$|\?)/i.test(src)) {
    return { src, webpSrc: null };
  }

  const webpCandidate = src.replace(/\.(jpe?g|png)(?=($|\?))/i, ".webp");
  const webpSrc = webpCandidate !== src ? webpCandidate : null;
...
export function isSiteBrandingNewsImageUrl(url) {
  const value = String(url || "").trim();
  if (!value) return false;

  if (value.startsWith("/") && SITE_BRANDING_IMAGE_PATTERN.test(value)) {
    return true;
  }

  try {
    const parsed = new URL(value.startsWith("//") ? `https:${value}` : value);
    return SITE_BRANDING_IMAGE_PATTERN.test(parsed.pathname);
  } catch {
    return SITE_BRANDING_IMAGE_PATTERN.test(value);
  }
}

export function resolveNewsImageUrl(item) {
  const url = normalizeNewsImageUrl(item?.image_url);
  if (isSiteBrandingNewsImageUrl(url)) {
    return null;
  }
  return url;
}
