  ...
  try {
    ...
    if (userEmail) {
      ...
      if (profileError) {
        ...
        console.error("SUBSCRIPTION_ACTIVATE_PROFILE_UPDATE_FAILED", {
          requestId: normalizedRequestId,
          userEmail,
          error: profileError.message || String(profileError),
        });
        await recordActivateCriticalAudit(supabase, adminUser, normalizedRequestId, "activate-subscription-profile-update-failed", {
          userId,
          userEmail,
          planName,
          error: profileError.message || String(profileError),
