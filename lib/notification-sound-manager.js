...
import { getClientNotificationDelivery } from "./notification-delivery-gate-client.js";
import {
...
function resolvePlayOptions(notificationKeyOrOptions, maybeOptions = {}) {
  if (typeof notificationKeyOrOptions === "string") {
    return {
      ...
      skipSettingsGate: Boolean(maybeOptions.skipSettingsGate),
    };
  }
  ...
  if (opts.notificationKey || opts.notification_key) {
    return {
      ...
      skipSettingsGate: Boolean(opts.skipSettingsGate),
    };
  }
...
export async function playNotificationSound(notificationKeyOrOptions, maybeOptions = {}) {
  ...
  const { notificationKey, id, source, allowBackgroundPlayback, skipPendingStore, skipSettingsGate } =
  ...
  if (!skipSettingsGate) {
    const delivery = getClientNotificationDelivery(resolvedKey);
    if (!delivery.sound) {
      return blocked(delivery.blockedReason || "delivery-blocked");
    }
  } else if (!isNotificationKeySoundEnabled(resolvedKey)) {
