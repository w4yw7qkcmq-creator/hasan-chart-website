...
export async function playNotificationSound(notificationKeyOrOptions, maybeOptions = {}) {
  ...
  const { notificationKey, id, source, allowBackgroundPlayback } = resolvePlayOptions(
  ...
  const blocked = (reason) => {
    ...
    return { dispatched: false, blockedReason: reason };
  };
...
function handleBrowserSoundMessage(data, channelSource) {
  ...
  console.log("PRICE_ALERT_SOUND_REQUEST", {
    id: data.alertId || data.id || null,
    key: resolvedKey,
    source: `service-worker:${channelSource}`,
    channelSource,
  });

  void playNotificationSound(resolvedKey, {
    id: data.alertId || data.id || null,
    source: `service-worker:${channelSource}`,
