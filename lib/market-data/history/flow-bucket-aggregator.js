...
import { EXCHANGE_IDS } from "../symbols.js";
 ...
 * @returns {string}
 */
export function buildFlowBucketKey(symbol, exchangeScope, bucketStart) {
  return `${symbol}:${exchangeScope}:${bucketStart}`;
}

function bucketKey(symbol, exchangeScope, bucketStart) {
  return buildFlowBucketKey(symbol, exchangeScope, bucketStart);
}
...
function isValidSymbol(symbol) {
  ...
  if (CORE_SYMBOLS.includes(symbol)) return true;
  return getDynamicSymbolManager().isHistoryEligible(symbol);
}
...
export class FlowBucketAggregator {
  ...
  /**
   * @param {number} [now]
   * @returns {{ snapshots: FlowBucketSnapshot[], keys: string[] }}
   */
  getReadyBuckets(now = this.nowFn()) {
    const currentBucket = floorToMinute(now);
    /** @type {FlowBucketSnapshot[]} */
    const snapshots = [];
    /** @type {string[]} */
    const keys = [];
    ...
    snapshots.sort((a, b) => a.bucketStart - b.bucketStart);
   ...
   * @returns {number}
   */
  acknowledgeBuckets(keys) {
    let removed = 0;
    for (const key of keys) {
      if (this.buckets.delete(key)) removed += 1;
    }
    return removed;
  }
   ...
   */
  getPendingBucketStats(now = this.nowFn()) {
    const currentBucket = floorToMinute(now);
    let count = 0;
    ...
    for (const key of this.buckets.keys()) {
      ...
      if (now < bucketStart + BUCKET_MS + LATE_TRADE_GRACE_MS) continue;
      count += 1;
      if (oldestStart == null || bucketStart < oldestStart) {
        oldestStart = bucketStart;
      }
    }

    return {
      count,
      oldestAgeMs: oldestStart == null ? null : now - oldestStart,
    };
  }

  drainReadyBuckets(now = this.nowFn()) {
    const { snapshots, keys } = this.getReadyBuckets(now);
    this.acknowledgeBuckets(keys);
    return snapshots;
  }
