...
const LIQUIDITY_WALL_CONFLICT_COLUMN = "wall_key";
...
export function createSupabaseHistoryClient(options = {}) {
   ...
   */
  function mapLiquidityWallRow(row) {
    return {
      ...
      size: row.size,
      notional: row.notional,
      distance_from_mid: row.distanceFromMid,
      snapshot_time: new Date(row.snapshotTime).toISOString(),
      first_seen: new Date(row.firstSeen).toISOString(),
      last_seen: new Date(row.lastSeen).toISOString(),
      lifetime_seconds: row.lifetimeSeconds,
      appearance_count: row.appearanceCount,
      persistence_score: row.persistenceScore,
      max_size: row.maxSize,
      average_size: row.averageSize,
      reappear_count: row.reappearCount,
      strongest_notional: row.strongestNotional,
      survived_snapshots: row.survivedSnapshots,
      is_active: row.isActive,
    };
  }

  async function upsertLiquidityWalls(rows) {
    const started = Date.now();
    if (!rows.length) {
      return { ok: true, status: 200, written: 0, skipped: 0, latencyMs: 0 };
    }

    try {
      const response = await fetchFn(
        `${url}/rest/v1/market_liquidity_walls?on_conflict=${LIQUIDITY_WALL_CONFLICT_COLUMN}`,
        {
          method: "POST",
          headers: {
            apikey: serviceKey,
          ...
          body: JSON.stringify(rows.map(mapLiquidityWallRow)),
        },
        timeoutMs,
      );
      ...
      const latencyMs = Date.now() - started;
      if (response.ok) {
        return { ok: true, status: response.status, written: rows.length, skipped: 0, latencyMs };
      }
...
export {
  ...
  LIQUIDITY_WALL_CONFLICT_COLUMN,
  mapFlowBucketRow,
  mapLargeTradeRow,
  safeErrorMessage,
};
