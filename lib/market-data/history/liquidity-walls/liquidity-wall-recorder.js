...
import { LiquidityWallWriteState } from "./liquidity-wall-write-state.js";
import {
  LiquidityWallWriter,
...
export class HistoricalLiquidityWallRecorder {
  ...
  constructor(options = {}) {
    ...
    this.writeState = options.writeState ?? new LiquidityWallWriteState();
      ...
      this.writer = new LiquidityWallWriter({
        ...
        acknowledgeRows: (keys) => {
          for (const key of keys) {
            const row = this.pendingRows.get(key);
            if (row) {
              this.writeState.markWritten([row]);
            }
  ...
  recordExchangeSnapshot(snapshot) {
    try {
      ...
      for (const row of updates) {
        if (!this.writeState.shouldQueue(row)) continue;
...
export function stopHistoricalLiquidityWallRecorder() {
  const recorder = getHistoricalLiquidityWallRecorder();
