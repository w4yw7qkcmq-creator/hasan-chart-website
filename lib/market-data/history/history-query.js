...
export async function queryHistoricalFlow(params) {
  ...
  return withCache(cacheKey, async () => {
    ...
    const uniqueBucketStarts = new Set(
      (rows || []).map((row) => String(row.bucket_start)),
    );
    const bucketCount = uniqueBucketStarts.size;
    ...
    return {
      ...
      ...dominance,
      bucketCount,
      expectedBucketCount: coverage.expectedBucketCount,
      missingBucketCount: coverage.missingBucketCount,
      coverageRatio: coverage.coverageRatio,
      coveragePercent: coverage.coveragePercent,
      coverageStart,
      coverageEnd,
      completeness: coverage.coverageRatio,
...
export async function queryHistoricalLargeTrades(params) {
  ...
  return withCache(cacheKey, async () => {
    ...
    return {
      ...
      maxTrade,
      window: params.window,
      partialData: coverage.partialData || collectingSince == null,
      collectingSince,
      completeness: coverage.coverageRatio,
      coverageRatio: coverage.coverageRatio,
      coveragePercent: coverage.coveragePercent,
