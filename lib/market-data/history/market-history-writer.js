...
import { buildFlowBucketKey } from "./flow-bucket-aggregator.js";
...
export class MarketHistoryWriter {
  ...
  constructor(options) {
    ...
    this.lastWrittenFlowFingerprints = new Map();
    this.emptyFlushes = 0;
    this.unchangedFlowSkips = 0;
    this.duplicateFlowSkips = 0;
    this.duplicateLargeSkips = 0;
  }
   ...
   */
  prepareFlowBatch(rows) {
    const deduped = dedupeBatchByKey(rows, (row) =>
      buildFlowBucketKey(row.symbol, row.exchangeScope, row.bucketStart),
    );
    this.duplicateFlowSkips += Math.max(0, rows.length - deduped.length);
    const { changed, skipped } = filterUnchangedRows(
      deduped,
      (row) => buildFlowBucketKey(row.symbol, row.exchangeScope, row.bucketStart),
      buildFlowBucketFingerprint,
      this.lastWrittenFlowFingerprints,
    );
    this.unchangedFlowSkips += skipped;
    return changed;
  }
   ...
   */
  prepareLargeTradeBatch(rows) {
    const deduped = dedupeBatchByKey(rows, (row) => row.tradeKey);
    this.duplicateLargeSkips += Math.max(0, rows.length - deduped.length);
    return deduped;
  }
   ...
   * @param {FlowBucketWriteRow[]} rows
   */
  markFlowWritten(rows) {
    for (const row of rows) {
      const key = buildFlowBucketKey(row.symbol, row.exchangeScope, row.bucketStart);
      this.lastWrittenFlowFingerprints.set(key, buildFlowBucketFingerprint(row));
    }
  }
  /**
  ...
  /**
   * @param {(rows: FlowBucketWriteRow[]|LargeTradeWriteRow[]) => Promise<HistoryWriteResult>} writeFn
   * @param {FlowBucketWriteRow[]|LargeTradeWriteRow[]} rows
   * @param {{ prepare?: (rows: any[]) => any[], markWritten?: (rows: any[]) => void }} [options]
   */
  async writeWithRetry(writeFn, rows, options = {}) {
    const prepare = options.prepare ?? ((batch) => batch);
    const markWritten = options.markWritten ?? (() => {});
    let batch = prepare(rows);
    if (!batch.length) {
      return { ok: true, status: 200, written: 0, skipped: rows.length, latencyMs: 0 };
    }
  ...
  async runFlush() {
    ...
    const flowBatchRaw = snapshots.slice(0, this.config.batchSize);
    const flowKeysRaw = keys.slice(0, this.config.batchSize);
    const flowBatch = this.prepareFlowBatch(flowBatchRaw);
    const flowKeySet = new Set(
      flowBatch.map((row) => buildFlowBucketKey(row.symbol, row.exchangeScope, row.bucketStart)),
    );
    const flowKeys = flowKeysRaw.filter((key) => flowKeySet.has(key));

    const largeBatch = this.prepareLargeTradeBatch(
      this.largeTradeQueue.splice(0, this.config.batchSize),
    );
    this.metrics.largeTradesPending = this.largeTradeQueue.length;

    const hadWork = flowBatch.length > 0 || largeBatch.length > 0;
    if (!hadWork) {
      this.emptyFlushes += 1;
      this.metrics.inFlight = false;
      if (flowKeysRaw.length > 0 && flowBatch.length === 0) {
        this.acknowledgeFlowBuckets(flowKeysRaw);
      }
      return {
        flowResult: { ok: true, written: 0, skipped: 0, latencyMs: 0, status: 200 },
        largeResult: { ok: true, written: 0, skipped: 0, latencyMs: 0, status: 200 },
        flushOk: true,
      };
    }
    ...
    if (flowBatch.length > 0) {
      flowResult = await this.writeWithRetry(
        ...
        flowBatch,
        {
          prepare: (rows) => this.prepareFlowBatch(rows),
          markWritten: (rows) => this.markFlowWritten(rows),
        },
      );
    }
    ...
    if (largeBatch.length > 0) {
      largeResult = await this.writeWithRetry(
        ...
        largeBatch,
        {
          prepare: (rows) => this.prepareLargeTradeBatch(rows),
        },
      );
    }
    ...
    } else if (flowResult.ok && flowKeysRaw.length > 0 && flowBatch.length === 0) {
      this.acknowledgeFlowBuckets(flowKeysRaw);
    }
