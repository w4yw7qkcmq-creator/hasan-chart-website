...
export function detectLiquidityWalls({ bids, asks, midPrice, minNotional = WALL_MIN_NOTIONAL_USD }) {
  const bidNotionals = (bids || []).map((l) => l.notional ?? l.price * l.quantity);
  const askNotionals = (asks || []).map((l) => l.notional ?? l.price * l.quantity);
  const bidThreshold = Math.max(minNotional, median(bidNotionals) * WALL_MEDIAN_MULTIPLIER);
  const askThreshold = Math.max(minNotional, median(askNotionals) * WALL_MEDIAN_MULTIPLIER);
