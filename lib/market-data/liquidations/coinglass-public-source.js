...
export function sortRealtimeOrdersDescending(rows) {
  if (!Array.isArray(rows) || rows.length === 0) return [];

  return [...rows].sort((a, b) => {
    const ta = toNumber(a?.time ?? a?.liquidatedAt);
    const tb = toNumber(b?.time ?? b?.liquidatedAt);
...
export function parseRealtimeOrders(data) {
  const sourceRows = Array.isArray(data?.list)
    ? data.list
    : Array.isArray(data)
      ? data
  ...
  const parsed = sourceRows
    .map((row, index) => {
      ...
      const time = pickNumber(row.liquidatedAt, row.createTime, row.turnoverTime, row.time, row.ts);
  ...
  return sortRealtimeOrdersDescending(parsed).slice(0, 50);
}
