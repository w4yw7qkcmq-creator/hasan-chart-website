...
import { refreshSymbolRegistry } from "./symbol-registry.js";
...
export function validateMarketDepthQuery(searchParams = {}) {
  ...
  const symbol = normalizeMarketSymbol(rawSymbol) || "BTCUSDT";
...
export async function ensureMarketSymbolsRegistry() {
  try {
    await refreshSymbolRegistry();
  } catch {
    // registry refresh failures handled by stale fallback
  }
}
