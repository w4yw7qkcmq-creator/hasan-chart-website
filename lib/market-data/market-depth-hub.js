...
class MarketDepthHub {
  constructor() {
    ...
    /** @type {Map<string, { epoch: number, chunk: string }>} */
    this.sseSerializedCache = new Map();
    this.started = false;
    /** @type {Map<string, object>} */
  ...
  scheduleBroadcast() {
    ...
    this.broadcastTimer = setTimeout(() => {
      this.broadcastTimer = null;
      if (!this.pendingBroadcast || this.subscribers.size === 0) {
        this.pendingBroadcast = false;
        return;
      }
      this.pendingBroadcast = false;
  ...
  paramsCacheKey(params) {
    ...
    return [
      ...
      params.rangePercent,
      params.flowWindow,
      params.largeTradeWindow,
      params.largeTradeThreshold,
      params.liquidityWallWindow,
      params.liquidityDepthWindow,
      params.exchange,
    ].join("|");
  }

  getSerializedSseChunk(params) {
    const key = this.paramsCacheKey(params);
    const epoch = this.broadcastEpoch;
    const cached = this.sseSerializedCache.get(key);
    if (cached?.epoch === epoch) {
      return cached.chunk;
    }

    const payload = this.buildPayload(params);
    const chunk = `data: ${JSON.stringify({ success: true, ...payload })}\n\n`;
    this.sseSerializedCache.set(key, { epoch, chunk });
    return chunk;
  }
  ...
  subscribe(callback) {
    ...
    return () => {
      ...
      if (this.subscribers.size === 0) {
        this.clearBroadcastSchedule();
      }
    };
  }

  stop(reason = "manual") {
    if (!this.started && this.connections.size === 0) {
      return this;
    }

    logMarketDepth("stopping", { reason, connections: this.connections.size });

    for (const symbol of Array.from(this.symbolState.keys())) {
    ...
    for (const symbol of CORE_SYMBOLS) {
      const exchanges =
        this.symbolState.get(symbol)?.candidateExchanges ||
        this.symbolState.get(symbol)?.supportedExchanges ||
        this.getSymbolExchanges(symbol);
      for (const exchange of exchanges) {
        this.shutdownConnection(exchange, symbol);
      }
      this.dynamicManager.release(symbol, "__core__");
      this.symbolState.delete(symbol);
      this.flowTrackers.delete(symbol);
    }
    ...
    this.started = false;
    logMarketDepth("stopped", { reason, connections: this.connections.size });
    return this;
  }
  ...
  buildPayload(params) {
    ...
    if (!this.started) {
      return {
        ...
        partialExchangeCoverage: false,
        historyCollecting: true,
        probeStatus: "warming",
        probing: true,
        probeSummary: null,
        symbolLoadError: null,
        exchanges: [],
        exchangeStatuses: [],
        timestamp: Date.now(),
        updatedAt: 0,
        ...
        bids: [],
        asks: [],
        liquidity: { bidDominance: null, askDominance: null, imbalance: null },
        walls: { bidWalls: [], askWalls: [] },
        executedFlow: { buyVolume: 0, sellVolume: 0, netVolume: 0, tradeCount: 0 },
        dominanceFlow: { buyVolume: 0, sellVolume: 0, netVolume: 0, tradeCount: 0 },
        largeTrades: [],
        largeTradeStats: { tradesInWindow: 0, tradesAboveThreshold: 0 },
        recentTrades: [],
        depthMap: [],
        dataSources: [],
...
export function getSharedMarketDepthSnapshot(params) {
  return getMarketDepthHub().getSnapshot(params);
}
...
export function getMarketDepthHealthSnapshot() {
  ...
  try {
    if (globalThis.__marketDepthLifecycle) {
      lifecycle = globalThis.__marketDepthLifecycle.getSnapshot();
    } else {
      lifecycle = {
        state: "STOPPED",
        consumerCount: 0,
