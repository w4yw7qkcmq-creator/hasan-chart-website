...
import { getRegistryEntry, getSupportedExchangesForSymbol, isRegistrySymbolSupported } from "./symbol-registry.js";
...
class DynamicSymbolManager {
  ...
  acquire(symbol, clientId) {
    ...
    if (!this.isCoreSymbol(normalized) && !isRegistrySymbolSupported(normalized)) {
    ...
    const supportedExchanges = this.isCoreSymbol(normalized)
    ...
    if (wasInactive) {
      this.onActivate?.(normalized, supportedExchanges);
    }
    ...
    return {
      ...
      symbol: normalized,
      supportedExchanges,
      expectedExchangeCount: supportedExchanges.length,
      isCore: state.isCore,
