...
export async function backfillLegacyAdmins(supabase, params = {}) {
  ...
  for (const profile of profiles || []) {
    const roleId = String(profile.admin_role || "admin").trim().toLowerCase();
    const normalizedRole = ["super_admin", "admin", "analyst", "support", "accountant", "news_editor"].includes(roleId)
      ? roleId
      : "admin";

    const grant = await grantRole(supabase, {
      actorId,
      actorEmail: "system@backfill",
      actorIam: { isSuperAdmin: true },
      targetUserId: profile.id,
      roleId: normalizedRole,
      reason: "legacy_backfill_profiles_role",
      organizationId,
    });

    if (grant.ok) results.granted += 1;
    else if (grant.status === 409) results.skipped += 1;
    else results.errors.push({ userId: profile.id, error: grant.error });
  }

  return results;
}

function mapLegacyRole(profile, ownerEmail) {
  const normalizedOwner = normalizeEmail(ownerEmail);
  const profileEmail = normalizeEmail(profile.email);

  if (normalizedOwner && profileEmail === normalizedOwner) {
    return IAM_ROLES.SUPER_ADMIN;
  }

  const roleId = String(profile.admin_role || "admin").trim().toLowerCase();
  return ["super_admin", "admin", "analyst", "support", "accountant", "news_editor"].includes(roleId)
    ? roleId
    : IAM_ROLES.ADMIN;
}

export async function dryRunBackfillLegacyAdmins(supabase, params = {}) {
  const ownerEmail = params.ownerEmail || process.env.IAM_OWNER_EMAIL || null;
  const report = {
    usersFound: 0,
    proposed: [],
    missingProfiles: [],
  ...
  report.usersFound = (profiles || []).length;

  for (const profile of profiles || []) {
    const proposedRole = mapLegacyRole(profile, ownerEmail);
    if (proposedRole === IAM_ROLES.SUPER_ADMIN) {
      report.proposedSuperAdminCount += 1;
    }
    ...
    report.proposed.push({
      userId: profile.id,
      email: profile.email,
      legacyAdminRole: profile.admin_role || null,
      proposedRole,
    });
  }
