...
/**
 * Global IAM revocation after a successful password / security credential change.
 * Invalidates all tokens issued before the cutoff (including other devices).
 * Current session cookies should be cleared by the caller so user re-authenticates.
 */
export async function revokeAllUserAccessAfterPasswordSecurityChange(supabase, params) {
  const userId = String(params.userId || "").trim();
  const reason = params.reason || REVOCATION_REASONS.PASSWORD_SECURITY_RESET;
  const previousAccessToken = params.previousAccessToken || null;
  const currentAccessToken = params.currentAccessToken || null;
  ...
  const global = await revokeAllUserAccess(supabase, {
    userId,
    actorId: userId,
    reason,
    request: params.request,
  ...
  return {
    ok: Boolean(global.ok),
    forceLogoutAfter: global.forceLogoutAfter,
    reason,
    requireReLogin: true,
  };
}
