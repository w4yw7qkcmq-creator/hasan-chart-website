...
export async function requirePermission(permission, options = {}) {
  ...
  if (!isIamApiEnabled()) {
    if (options.request && shouldApplyAdminRateLimit(options.request)) {
      const rateResult = await guardAdminApiRateLimit(options.request, session);
      if (!rateResult.success) {
        console.info(rateResult.kind === "read" ? "ADMIN_RATE_LIMIT_READ" : "ADMIN_RATE_LIMIT_WRITE", {
...
export async function requireAllPermissions(permissions, options = {}) {
  ...
  for (const perm of list) {
    const result = await requirePermission(perm, { ...options, __skipDenyLog: true });
    if (!result.ok) {
      session = await requireAdminSession(options);
      if (session.ok) {
        await logDenied(session, list.join("+"), options);
      }
      return forbiddenResult("ليس لديك صلاحية لتنفيذ هذا الإجراء", session?.iam);
  ...
  return { ...session, permissions: list };
}
