...
/** Phase 2B.2 — shadow getClaims parity on read-safe routes (default on when IAM_API). */
export function isIamLocalAuthShadowEnabled() {
  const raw = String(process.env.IAM_LOCAL_AUTH_SHADOW_ENABLED || "").trim().toLowerCase();
  if (raw === "0" || raw === "false" || raw === "no" || raw === "off") return false;
  if (raw === "1" || raw === "true" || raw === "yes" || raw === "on") return true;
  return isIamApiEnabled();
}

/** Phase 2B.2 — authoritative getClaims on approved read-only admin routes. */
export function isIamLocalAuthReadsEnabled() {
  return readFlag("IAM_LOCAL_AUTH_READS_ENABLED");
}

/** Phase 2A unified IAM resolver RPC (default on when IAM_API enabled). */
export function isIamUnifiedResolverEnabled() {
  const raw = String(process.env.IAM_UNIFIED_RESOLVER || "").trim().toLowerCase();
  if (raw === "0" || raw === "false" || raw === "no" || raw === "off") return false;
  if (raw === "1" || raw === "true" || raw === "yes" || raw === "on") return true;
  return isIamApiEnabled();
}
