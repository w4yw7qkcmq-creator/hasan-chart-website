...
import { revokeAllUserAccess } from "./session-revocation-service.js";
import { REVOCATION_REASONS } from "./revocation-reasons.js";
...
export async function forceLogoutSession(supabase, params) {
  ...
  const result = await revokeAllUserAccess(supabase, {
    userId,
    actorId,
    token,
    reason: reason || REVOCATION_REASONS.ADMIN_FORCE_LOGOUT,
  ...
  if (sessionLogId) {
    try {
      await supabase
        ...
        .update({
          ended_at: new Date().toISOString(),
          end_reason: reason || REVOCATION_REASONS.ADMIN_FORCE_LOGOUT,
  ...
  await recordSecurityEvent(supabase, {
    ...
    severity: "warning",
    userId,
    details: { sessionLogId, actorId, reason: reason || REVOCATION_REASONS.ADMIN_FORCE_LOGOUT },
    request: params.request,
  });
