...
import { isIamDbEnabled, isIamDualReadEnabled } from "./feature-flags.js";
...
async function loadIamAssignments(supabase, userId, organizationId) {
  let query = supabase
    ...
    .select("id, role_id, organization_id, granted_at")
    .eq("user_id", userId)
    .is("revoked_at", null);
...
export async function resolveIamContext(supabase, user, options = {}) {
  ...
  // Dual-read: merge legacy permissions when IAM has no assignments
  const legacy = await resolveLegacyAdminContext(supabase, user);
  if (legacy.isAdmin && roleIds.length === 0) {
  ...
  const isSuperAdmin = roleIds.includes("super_admin") || permissions.has("iam.manage");
  const isAdmin = roleIds.length > 0 || legacy.isAdmin || permissions.size > 0;
  ...
  const ctx = {
    ...
    roleIds,
    roleLabels,
    primaryRoleId: roleIds[0] || legacy.roleId || null,
    primaryRoleLabel: roleLabels[0] || legacy.roleLabel || null,
    permissions,
