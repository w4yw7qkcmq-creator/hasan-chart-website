...
export function buildContentPostArticleJsonLd({
  path,
  title,
  description,
  content = "",
...
export function buildPrivateMetadata({ title, description } = {}) {
  const metadata = {
    ...
    // Prevent inheriting the root homepage canonical on private/noindex routes.
    alternates: {},
  };
...
export function buildNotFoundMetadata() {
  return {
    metadataBase: SITE_METADATA_BASE,
    title: "الصفحة غير موجودة | HasaN CharT World",
    description: "الصفحة المطلوبة غير متاحة على HasaN CharT World.",
    robots: buildRobots({ index: false, follow: false, nocache: true }),
    alternates: {},
  };
}
