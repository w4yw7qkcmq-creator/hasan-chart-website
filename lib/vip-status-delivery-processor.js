...
async function markRetryableFailure(supabase, row, { errorCode, errorMessage, retryable = true }) {
  ...
  if (!retryable || attempts >= maxAttempts) {
    await finalizeDelivery(supabase, row.id, {
      status: "failed",
...
export async function processVipStatusEmailDelivery(
  ...
  if (
    ...
    (row.error_code === "outbox-queued" || row.status === "pending")
  ) {
    await finalizeDelivery(supabase, row.id, {
      status: "pending",
      processing_started_at: null,
  ...
  try {
    ...
    if (mapped.delivered || mapped.duplicate) {
      await finalizeDelivery(supabase, row.id, {
        status: "delivered",
        delivered_at: new Date().toISOString(),
        processing_started_at: null,
        processing_worker_id: null,
        provider_message_id: mapped.outboxId || mapped.providerMessageId || null,
      });
      return { outcome: "delivered" };
    }
