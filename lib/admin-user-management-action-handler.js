...
import { updateUserClassificationAdmin } from "./user-classification-admin.js";
...
function normalizeActionInput(body = {}) {
  ...
  const classification = String(body.classification || payload.classification || "").trim();
...
function validateActionInput(input) {
  ...
  if (input.action === "update_user_classification" && !input.classification) {
    throwStatus("تصنيف الحساب مطلوب", 400);
  }
}
...
export async function handleAdminUserManagementAction(
  ...
  switch (input.action) {
    ...
    case "update_user_classification":
      return updateUserClassificationAdmin(supabase, {
        adminUser,
        targetUserId: normalizedUserId,
