...
import { syncVipStatusDeliveryFromOutbox } from "./vip-status-email-outbox-sync.js";
...
export function getEmailQueueConfig(overrides = {}) {
...
export function extractRecipientDomain(email) {
...
export function logEmailQueueEvent(event, meta = {}) {
  const payload = {
...
export function calculateRetryDelay(attempts, options = {}) {
...
export async function claimPendingEmailBatch(supabase, options = {}) {
...
export async function releaseStaleProcessingEmails(supabase, options = {}) {
  const config = getEmailQueueConfig(options);
...
export async function markEmailSent(supabase, { outboxId, resendId = null } = {}) {
...
export async function markEmailRetryScheduled(
  supabase,
...
export async function sendOutboxEmailViaResend(row, deps = {}) {
  const sendFn = deps.sendWebsiteResendEmail || sendWebsiteResendEmail;
  const resendApiKey = deps.resendApiKey || process.env.RESEND_API_KEY;
  const fromEmail = deps.fromEmail || process.env.EMAIL_FROM || DEFAULT_FROM;
  const replyToEmail =
    deps.replyToEmail || process.env.EMAIL_REPLY_TO || DEFAULT_REPLY_TO;
  ...
  return sendFn({
    path: "lib/email-outbox-processor.js::sendOutboxEmailViaResend",
    resendApiKey,
    payload,
    to: [recipientEmail],
  });
...
export async function processSingleOutboxEmail(
  supabase,
  row,
  { sendOutboxEmail = sendOutboxEmailViaResend, sendDeps = {} } = {}
) {
  const startedAt = Date.now();
  const sendResult = await sendOutboxEmail(row, sendDeps);
...
export async function processEmailOutboxBatch(supabase, options = {}) {
  const config = getEmailQueueConfig(options);
  const startedAt = Date.now();
...
export async function runEmailQueueCron(supabase, options = {}) {
