...
export async function fetchAdminUserList(
  ...
  if (!response.ok || !result?.success) {
    throw new Error(result?.error || "فشل تحميل قائمة المستخدمين");
  }
...
export async function fetchAdminUserSection(
  ...
  if (!response.ok || !result?.success) {
    throw new Error(result?.error || "فشل تحميل بيانات المستخدم");
  }
...
export async function createAdminUserNote(adminFetch, userId, note) {
  ...
  const result = await response.json().catch(() => ({}));
  if (!response.ok || !result?.success) {
    throw new Error(result?.error || "تعذر إضافة الملاحظة");
  }
  return result;
}

export async function updateAdminUserNote(adminFetch, userId, { noteId, note }) {
  const response = await adminFetch(
    `/api/admin/user-management/${encodeURIComponent(userId)}/notes`,
    {
      method: "PATCH",
  ...
  const result = await response.json().catch(() => ({}));
  if (!response.ok || !result?.success) {
    throw new Error(result?.error || "تعذر تحديث الملاحظة");
  }
  return result;
}
...
export async function deleteAdminUserNote(adminFetch, userId, noteId) {
  ...
  const result = await response.json().catch(() => ({}));
  if (!response.ok || !result?.success) {
    throw new Error(result?.error || "تعذر حذف الملاحظة");
  }
  return result;
}
