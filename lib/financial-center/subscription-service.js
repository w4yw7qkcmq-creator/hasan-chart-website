...
import { countPendingPaymentReviews, filterPendingPaymentReviewRows } from "./pending-payment-review.js";
import { SUBSCRIPTION_STATUSES } from "./financial-types.js";
import {
  FINANCIAL_CLIENT_FILTER_SCAN_MAX,
...
export function buildSubscriptionQuery(
  supabase,
  { search = "", sort = "created_at", order = "desc", includeTotal = false } = {}
) {
...
export async function listFinancialSubscriptions(
  ...
  if (!needsClientFilter) {
    ...
    const query = buildSubscriptionQuery(supabase, { search, sort, order, includeTotal }).range(from, to);
    ...
    const pagination = {
      ...
      hasMore: includeTotal
        ? pageNumber * resolvedPageSize < (count || 0)
        : (data || []).length === resolvedPageSize,
    };
    if (includeTotal && typeof count === "number") {
      pagination.total = count;
      pagination.totalPages = Math.max(Math.ceil(count / resolvedPageSize), 1);
    }

    return { items, pagination };
  }

  const scanLimit = exportMode ? FINANCIAL_MAX_EXPORT_ROWS : FINANCIAL_CLIENT_FILTER_SCAN_MAX;
  ...
  return {
    ...
    pagination: {
      ...
      totalPages: includeTotal ? Math.max(Math.ceil(filtered.length / resolvedPageSize), 1) : undefined,
      hasMore: from + resolvedPageSize < filtered.length,
    },
...
export async function getRecentFinancialSubscriptions(supabase, { kind = "active", limit = 5 } = {}) {
  ...
  const fetchLimit = kind === "pending" ? Math.max(limit * 4, 20) : limit;
  const { data, error } = await query.limit(fetchLimit);
  if (error) throw error;

  const rows =
    kind === "pending"
      ? filterPendingPaymentReviewRows(data || []).slice(0, limit)
      : (data || []).slice(0, limit);
