...
export async function removeSubscriptionRequest(
  supabase,
  {
    ...
    dispatchEndedEmail = dispatchSubscriptionEndedEmail,
  ...
  try {
    ...
    if (userEmail) {
      try {
        ...
        console.error("SUBSCRIPTION_REMOVE_NOTIFICATION_FAILED", {
          requestId: normalizedRequestId,
          userEmail,
      ...
      try {
        const emailResult = await dispatchEndedEmail({
        ...
        console.error("SUBSCRIPTION_REMOVE_EMAIL_QUEUE_FAILED", {
          requestId: normalizedRequestId,
          userEmail,
    ...
    try {
      ...
      auditLogged = auditResult?.ok === true;
      if (!auditLogged) {
        auditWarning = "تم إزالة الاشتراك، لكن تعذر تسجيل العملية في سجل الإدارة.";
      }
    } catch (auditError) {
      console.error("SUBSCRIPTION_REMOVE_AUDIT_FAILED", {
        requestId: normalizedRequestId,
