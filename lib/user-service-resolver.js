...
function matchesServiceKey(row, serviceKey) {
  ...
  if (serviceKey === "academy") {
    return serviceType === ADMIN_SERVICE_TYPES.ACADEMY;
  }
...
function pickBestSubscriptionRow(rows, serviceKey) {
  ...
  const active = candidates.find(isActiveSubscriptionRequest);
...
function mapSubscriptionService(row) {
  ...
  return {
    active: isActiveSubscriptionRequest(row),
...
export {
  isActiveSubscriptionRequest as isSubscriptionActive,
  matchesServiceKey,
  pickBestSubscriptionRow,
  ACTIVE_SUBSCRIPTION_STATUSES,
  INACTIVE_SUBSCRIPTION_STATUSES,
  ACTIVE_ACCOUNT_MANAGEMENT_STATUSES,
};
