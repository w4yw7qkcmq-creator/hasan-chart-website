...
const ACTIVE_ACCOUNT_STATUSES = ["نشط", "مفعل", "active", "تمت المراجعة", "approved", "completed"];
...
function isActiveSubscription(row) {
  ...
  const isActive = ACTIVE_SUBSCRIPTION_STATUSES.some(
    (item) => item === status || item.toLowerCase() === normalized
  );
  if (!isActive) {
    return false;
  }
  ...
  return ACTIVE_SUBSCRIPTION_STATUSES.some((item) => item.toLowerCase() === status);
}
...
export async function loadAdminUserList(
  supabase,
  { page = 1, search = "", sort = "created_at", order = "desc", accountStatus = "" } = {}
) {
  const startedAt = Date.now();
  const pageNumber = Math.max(Number(page) || 1, 1);
  const pageSize = ADMIN_USER_MANAGEMENT_PAGE_SIZE;
  ...
  if (normalizedSearch) {
    ...
    const filter = `email.ilike.%${escaped}%,username.ilike.%${escaped}%`;
...
function buildServicesCatalog({ subscriptions, accountRows, alerts }) {
  ...
  const accountRow = pickLatest(accountRows, (row) =>
    ACTIVE_ACCOUNT_STATUSES.includes(String(row.status || "").trim())
  );
...
export async function loadAdminUserActivity(supabase, userId, { page = 1 } = {}) {
  const startedAt = Date.now();
  const pageNumber = Math.max(Number(page) || 1, 1);
  ...
  const from = (pageNumber - 1) * pageSize;
  const paginatedEvents = events.slice(from, from + pageSize);
