...
import {
  readEffectiveClassificationFromProfile,
...
function applySharedProfileFilters(query, {
  ...
  const normalizedClassification = normalizeUserClassificationFilter(userClassification);
  if (normalizedClassification !== "all") {
    query = query.eq("effective_user_classification", normalizedClassification);
  }
...
function formatUserBase(profile, authUser, activeSubscriptionsCount = 0) {
  ...
  const readModelResult = readEffectiveClassificationFromProfile(profile);
  const classificationResult = readModelResult || resolveEffectiveUserClassification(profile, authUser);
  ...
  return {
    ...
    userClassification: classificationResult.classification,
    userClassificationLabel: getUserClassificationLabel(classificationResult.classification, { short: true }),
    userClassificationSource: classificationResult.source,
    userClassificationConfidence: classificationResult.confidence,
  };
}
...
async function enrichFormattedAdminUserRows(
  supabase,
  profiles,
  { sortKey, ascending, includeLastLoginSort }
) {
  ...
  return attachActiveServiceFlagsToUsers(supabase, rows);
}
...
export async function loadAdminUserList(
  ...
  const normalizedUserClassification = normalizeUserClassificationFilter(userClassification);
  ...
  const sharedFilters = {
    ...
    lastLoginFromIso,
    lastLoginToIso,
    userClassification: normalizedUserClassification,
  };
  ...
  let lastSignInColumnAvailable = true;
  let userClassificationColumnAvailable = true;
  let effectiveClassificationReadModelAvailable = true;
  ...
  if (listResult.error && /column .* does not exist/i.test(listResult.error.message || "")) {
    if (/last_sign_in_at/i.test(listResult.error.message || "")) {
      lastSignInColumnAvailable = false;
    }
    if (/user_classification/i.test(listResult.error.message || "")) {
      userClassificationColumnAvailable = false;
    }
    if (/effective_user_classification/i.test(listResult.error.message || "")) {
      effectiveClassificationReadModelAvailable = false;
    }
    includeLastLoginSort = false;
    const fallbackFilters = {
      ...sharedFilters,
      lastLoginFromIso: lastSignInColumnAvailable ? sharedFilters.lastLoginFromIso : "",
      lastLoginToIso: lastSignInColumnAvailable ? sharedFilters.lastLoginToIso : "",
      userClassification:
        userClassificationColumnAvailable && effectiveClassificationReadModelAvailable
          ? sharedFilters.userClassification
          : "all",
    };
  ...
  if (lastSignInColumnAvailable) {
    if (profiles?.length) {
      lastSignInColumnAvailable = Object.prototype.hasOwnProperty.call(profiles[0], "last_sign_in_at");
    }
  }

  let rows = await enrichFormattedAdminUserRows(supabase, profiles || [], {
    sortKey,
    ascending,
    includeLastLoginSort,
  });
  ...
  const responsePayload = {
    ...
    listQueryFields: {
      ...
      lastSignInSource: lastSignInColumnAvailable ? "profiles.last_sign_in_at" : "unavailable",
    },
    capabilities: {
      lastSignInFilterAvailable: lastSignInColumnAvailable,
      userClassificationFilterAvailable:
        userClassificationColumnAvailable && effectiveClassificationReadModelAvailable,
      effectiveClassificationReadModel: effectiveClassificationReadModelAvailable,
    },
  };
