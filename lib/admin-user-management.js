...
function applySharedProfileFilters(query, {
  ...
  if (normalizedClassification !== "all") {
    query = query.eq("user_classification", normalizedClassification);
  }
...
function formatUserBase(profile, authUser, activeSubscriptionsCount = 0) {
  ...
  const classificationResult = resolveStoredOrComputedClassification(profile, authUser);
...
export async function loadAdminUserList(
  ...
  if (listResult.error && /column .* does not exist/i.test(listResult.error.message || "")) {
    ...
    const fallbackFilters = {
      ...
      userClassification: userClassificationColumnAvailable ? sharedFilters.userClassification : "all",
    };
  ...
  const responsePayload = {
    ...
    capabilities: {
      ...
      userClassificationFilterAvailable: userClassificationColumnAvailable,
    },
  };
