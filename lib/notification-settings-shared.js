...
export const EMAIL_SUPPORTED_CHANNEL_KEYS = new Set(
  PHASE1_NOTIFICATION_CHANNELS.filter((item) => item.emailSupported).map((item) => item.key)
);
  ...
  const merged = normalizeNotificationSettings({
    ...
    channel_preferences:
      patch.channel_preferences !== undefined
        ? mergeChannelPreferencesPatch(
            currentSettings.channel_preferences,
            patch.channel_preferences
          )
        : currentSettings.channel_preferences,
  });
