...
async function checkMarketStreamHealth() {
  ...
  if (status !== "ok") {
    if (snapshot.lastError) {
      ...
      result.message =
        "Market stream is still connecting to Binance WebSocket; no price data received yet.";
    } else if (snapshot.status === "retrying") {
      result.message = "Market stream is retrying the Binance WebSocket connection.";
    } else if (snapshot.status === "offline") {
      result.message = "Market stream failed to connect to Binance WebSocket.";
    }
  }
