...
import { getMarketDepthHealthSnapshot } from "./market-data/market-depth-hub.js";
import { getSymbolRegistryHealth } from "./market-data/symbol-registry.js";
import { getLiquidityWallWriterStatus } from "./market-data/history/liquidity-walls/liquidity-wall-recorder.js";
...
import { buildIamRuntimeProbe } from "./iam/runtime-probe.js";
...
function checkMarketLiquidityHistoryHealth() {
  const status = getLiquidityWallWriterStatus();
  return {
    status: "ok",
    enabled: Boolean(status.enabled),
    collectingSince: status.collectingSince ?? null,
    trackedWalls: status.trackedWalls ?? 0,
    storedWalls: status.storedWalls ?? 0,
    pending: status.pending ?? 0,
    flushAttempts: status.flushAttempts ?? 0,
    flushSuccesses: status.flushSuccesses ?? 0,
    flushFailures: status.flushFailures ?? 0,
    lastFlush: status.lastFlush ?? null,
    lastSuccessfulFlushAt: status.lastSuccessfulFlushAt ?? null,
    lastErrorSafe: status.lastErrorSafe ?? null,
...
function checkMarketHistoryHealth() {
  ...
  return {
    ...
    enabled: Boolean(status.enabled),
    collectingSince: status.collectingSince ?? null,
    tradesReceived: status.tradesReceived ?? 0,
    tradesAccepted: status.tradesAccepted ?? 0,
    duplicates: status.tradesDuplicate ?? 0,
    invalid: status.tradesInvalid ?? 0,
    lateAccepted: status.lateAccepted ?? 0,
    lateDropped: status.lateDropped ?? 0,
    pending: {
      flowBuckets: status.flowBucketsPending ?? 0,
      largeTrades: status.largeTradesPending ?? 0,
    },
    flushAttempts: status.flushAttempts ?? 0,
    flushSuccesses: status.flushSuccesses ?? 0,
    flushFailures: status.flushFailures ?? 0,
    rowsWrittenFlow: status.rowsWrittenFlow ?? 0,
    rowsWrittenLarge: status.rowsWrittenLarge ?? 0,
    droppedEvents: status.droppedEvents ?? 0,
    lastSuccessfulFlushAt: status.lastSuccessfulFlushAt ?? null,
    lastErrorSafe: status.lastErrorSafe ?? null,
    lastLatencyMs: status.lastLatencyMs ?? null,
    inFlight: Boolean(status.inFlight),
...
function checkMarketDynamicSymbolsHealth() {
  const depthHealth = getMarketDepthHealthSnapshot();
  const snapshot = depthHealth.dynamic;
  const registry = getSymbolRegistryHealth();
  const bootstrapActiveSymbols = (snapshot.symbols || []).filter(
    (row) => row.source === "bootstrap" && row.referenceCount > 0,
  ).length;

  return {
    status: "ok",
    connections: depthHealth.connections,
    lifecycle: depthHealth.lifecycle,
    ...snapshot,
    registry: {
      ...registry,
      bootstrapActiveSymbols,
      probeSuccesses: snapshot.probe?.probeSuccesses ?? 0,
      probeFailures: snapshot.probe?.probeFailures ?? 0,
      probeTimeouts: snapshot.probe?.probeTimeouts ?? 0,
    },
  };
}

/** Runtime IAM flags for canary/ops — booleans only, no secrets or env raw values. */
export { buildIamRuntimeProbe } from "./iam/runtime-probe.js";

export async function collectHealthReport(options = {}) {
  const detail = Boolean(options.detail);
  const [database, redis, marketStream] = await Promise.all([
    checkDatabaseHealth(),
    verifyUpstashRedisCached(),
    detail ? checkMarketStreamHealth() : Promise.resolve({ status: "ok", streamStatus: "compact" }),
  ]);

  const build = getBuildMeta();
  const iam = buildIamRuntimeProbe();
  ...
  if (!detail) {
    const status = resolveOverallStatus({
      ...
      database,
      redis: {
        status: !redis.envConfigured ? "not_configured" : redis.usesFallback ? "degraded" : "ok",
      },
  ...
  const marketLiquidityHistory = checkMarketLiquidityHistoryHealth();
  const marketDynamicSymbols = checkMarketDynamicSymbolsHealth();
