import { tierNameLabel, PARTNER_LEADERBOARD_METRICS, resolvePartnerDisplayName } from "./partner-shared";
...
async function enrichLatestWithdrawalsWithPartners(supabase, rows) {
  ...
  const partnerMap = new Map(
    (partners || []).map((partner) => {
      ...
      return [
        ...
        {
          id: partner.id,
          displayName: resolvePartnerDisplayName({
  ...
  return rows.map((row) => ({
    ...row,
    partner: partnerMap.get(row.partnerId) || {
      id: row.partnerId || null,
      displayName: "شريك",
    },
  }));
