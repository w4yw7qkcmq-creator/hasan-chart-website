...
function getActiveRoute(explicitRoute) {
  ...
  try {
    ...
    return headers().get("x-hc-api-route");
  } catch {
    return null;
  }
}
...
export function flushRedisUsageReport() {
  ...
  console.log(
    "Redis usage per minute:",
    JSON.stringify({
      timestamp: new Date().toISOString(),
      totalGet: minuteStats.get,
      totalSet: minuteStats.set,
      totalDel: minuteStats.del,
      totalPing: minuteStats.ping,
      totalRateLimit: minuteStats.rateLimit,
      totalVerify: minuteStats.verify,
      totalOther: minuteStats.other,
      topSources,
    })
  );
