...
export function buildInlineDataUrlPaymentProofResponse(proof, { requestId = "" } = {}) {
  const value = String(proof || "").trim();
  if (!isInlinePaymentProof(value)) {
    const error = new Error("صيغة إثبات الدفع غير مدعومة");
    error.status = 422;
    throw error;
  }
  ...
  return Response.json(
    ...
    },
    {
      headers: {
        ...
        "X-Payment-Proof-Type": "inline-data-url",
        "X-Payment-Proof-Bytes": String(value.length),
        Vary: "Accept-Encoding",
      },
    }
  );
...
export function buildSignedUrlPaymentProofResponse({
  requestId,
  url,
  mimeType = null,
  sizeBytes = null,
  ...
  return Response.json(
    ...
    },
    {
      headers: {
        "Cache-Control": CACHE_NO_STORE,
        "Content-Type": "application/json; charset=utf-8",
        "X-Payment-Proof-Type": "signed-url",
        Vary: "Accept-Encoding",
      },
    }
  );
...
export const INLINE_PAYMENT_PROOF_BINARY_MAX_BYTES = 512 * 1024;
