...
import { playNotificationSound, requestPriceAlertSound, unlockNotificationSound } from "./notification-sound-manager.js";
import {
...
const RENDERED_CHANNELS_STORAGE_KEY = "notification-center:rendered-channels:v1";
const MAX_PERSISTED_RENDERED_IDS = 500;

function readPersistedRenderedIds() {
  if (typeof window === "undefined") return new Set();

  try {
    const raw = window.localStorage.getItem(RENDERED_CHANNELS_STORAGE_KEY);
...
function writePersistedRenderedIds(set) {
  if (typeof window === "undefined") return;

  try {
    window.localStorage.setItem(
      RENDERED_CHANNELS_STORAGE_KEY,
      JSON.stringify([...set].slice(-MAX_PERSISTED_RENDERED_IDS))
    );
  } catch {
    // ignore quota errors
  }
...
let renderedNotificationIds = readPersistedRenderedIds();
...
export function markNotificationCenterRendered(id) {
  if (!id) return;
  const key = String(id);
  if (renderedNotificationIds.has(key)) return;
  renderedNotificationIds.add(key);
  writePersistedRenderedIds(renderedNotificationIds);
}
...
export function clearNotificationCenterRendered() {
  renderedNotificationIds = new Set();
  notificationSoundPlayedIds.clear();

  if (typeof window !== "undefined") {
    try {
      window.localStorage.removeItem(RENDERED_CHANNELS_STORAGE_KEY);
    } catch {
      // ignore
    }
  }
...
async function sendBrowserNotification(event) {
  ...
  const stableTag = notificationId
    ? `notification-center-${notificationId}`
    : `notification-center-${event.notificationKey}-${Date.now()}`;
...
async function attemptNotificationCenterSound(event, { source = "notification-center" } = {}) {
  ...
  try {
    const result = isPriceAlert
      ? await requestPriceAlertSound({
          alertId: soundPlayId,
          source,
          fromPush: false,
        })
      : await playNotificationSound(resolvedKey, {
    ...
    if (isPriceAlert && !result?.dispatched && !result?.pendingStored) {
      logPriceAlertStage("PRICE_ALERT_SOUND_BLOCKED", {
        id: notificationId,
        alertId: alertId || null,
        key: resolvedKey,
