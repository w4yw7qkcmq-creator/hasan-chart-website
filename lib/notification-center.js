...
async function attemptNotificationCenterSound(event, { source = "notification-center" } = {}) {
  ...
  try {
    playNotificationSound(resolvedKey, {
      id: notificationId || `nc-${Date.now()}`,
      source,
    });
    ...
    if (isPriceAlert) {
      logPriceAlertStage("PRICE_ALERT_SOUND_PLAYED", {
        id: notificationId,
        key: resolvedKey,
        source,
        note: "play-request-dispatched-via-notification-center",
      });
    }
