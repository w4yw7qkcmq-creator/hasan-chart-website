...
import {
  ...
  fetchDeliveriesForSignals,
} from "./vip-status-event-summary-sync.js";
...
async function fetchStatusEventsForSignals(supabase, signalIds) {
  ...
  const deliveriesByEvent = await fetchDeliveriesForSignals(supabase, signalIds);

  const map = {};
  for (const row of data || []) {
    if (!map[row.signal_id]) map[row.signal_id] = [];
    const deliveryKey = `${row.signal_id}:${row.event_type}`;
    const deliveryContract = buildVipStatusEventDeliveryContract(
      deliveriesByEvent[deliveryKey] || [],
      row.eligible_recipient_count
    );
    ...
    map[row.signal_id].push({
      ...
      deliverySummary: deliveryContract,
      deliveryStatus: deliveryContract.completed
        ? deliveryContract.partialFailure
          ? "partial"
          : "completed"
        : deliveryContract.processing > 0 || deliveryContract.pending > 0
          ? "processing"
          : "pending",
      partialFailure: deliveryContract.partialFailure,
    });
  }
  return map;
}
