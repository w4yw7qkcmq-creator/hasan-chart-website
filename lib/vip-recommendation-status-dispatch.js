...
const SIGNAL_SELECT =
  "id, signal_type, coin, entry, targets, stop_loss, notes, status, trade_status, created_at, publish_recipient_count, published_by_email, published_by, last_status_event, last_status_event_at, last_status_updated_by";
...
export async function listRecentVipRecommendations(supabase, { limit = 3 } = {}) {
  const { data, error } = await supabase
    .from("vip_signals")
    .select(SIGNAL_SELECT)
  ...
  if (error) {
    return { ok: false, error: error.message || "تعذر قراءة التوصيات" };
  }
