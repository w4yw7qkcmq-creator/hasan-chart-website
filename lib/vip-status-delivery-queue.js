...
} from "./vip-status-delivery-heartbeat.js";
import {
...
export async function getVipStatusDeliveryQueueMetrics(supabase) {
  ...
  return {
    pending: pendingError ? null : pendingCount || 0,
    processing: processingError ? null : processingCount || 0,
    oldestPendingAgeMs,
    staleProcessingCount: staleProcessing || 0,
  };
}

export async function logVipStatusDeliveryHealth(supabase, { workerId, summary } = {}, deps = {}) {
  ...
  logVipStatusDeliveryEvent("VIP_STATUS_DELIVERY_HEALTH", {
    ...
    heartbeatStaleThresholdSeconds: VIP_HEARTBEAT_STALE_SECONDS,
    pendingUnhealthyThresholdSeconds: VIP_PENDING_UNHEALTHY_SECONDS,
  ...
  const heartbeat = await persistVipWorkerHeartbeat(
    supabase,
    {
      metrics,
      cycleSucceeded: true,
  ...
  return { healthy: health.healthy, metrics, health, heartbeat };
}
