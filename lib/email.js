...
const PRICE_ALERT_EMAIL_BLOCKED_PATH = "worker/price-alert-email.js::sendPriceAlertEmail";

function isBlockedPriceAlertEmail({ subject, html, tags }) {
  const subjectText = String(subject || "");
  const htmlText = String(html || "");

  if (
    subjectText.includes("وصل السعر إلى هدف التنبيه") ||
    subjectText.includes("هدف التنبيه")
  ) {
    return true;
  }

  if (
    htmlText.includes("وصل السعر إلى هدف التنبيه") &&
    htmlText.includes("فتح تنبيهات الأسعار")
  ) {
    return true;
  }
  ...
  return tagList.some(
    (tag) =>
      (tag?.name === "message_type" || tag?.name === "category") &&
      String(tag?.value || "") === "price-alert"
  );
}
