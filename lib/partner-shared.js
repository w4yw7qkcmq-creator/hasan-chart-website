...
export const PARTNER_EMAIL_UNAVAILABLE_AR = "البريد غير متوفر";

export function maskPartnerEmail(email) {
...
export function resolvePartnerDisplayName(source = {}) {
  ...
  const fullName = String(source.fullName || source.full_name || "").trim();
  if (fullName && fullName !== "—") return fullName;

  const masked = maskPartnerEmail(source.email);
  if (masked) return masked;

  return "شريك";
}

export function resolvePartnerAccountEmail(source = {}) {
  const email = String(source?.email || "").trim();
  if (email && email.includes("@") && email !== "—") {
    return email;
  }
  return PARTNER_EMAIL_UNAVAILABLE_AR;
}

export function buildPartnerWithdrawalOwner(profile = null) {
  const source = profile && typeof profile === "object" ? profile : {};
  return {
    displayName: resolvePartnerDisplayName(source),
    email: resolvePartnerAccountEmail(source),
  };
}

export function attachWithdrawalPartnerIdentity(rows, partnerOwnerByPartnerId) {
  ...
  return rows.map((row) => ({
    ...row,
    partner: partnerOwnerByPartnerId.get(row.partnerId) || {
      id: row.partnerId || null,
      displayName: "شريك",
      email: PARTNER_EMAIL_UNAVAILABLE_AR,
    },
  }));
}

export function partnerDisplayInitial(name) {
  const label = String(name || "ش").trim();
  const first = label.charAt(0);
  return first && first !== "—" ? first.toUpperCase() : "ش";
}
