...
export function buildSubscriptionEndedEmailContent({
  username,
  planName,
  price,
  endedAt,
  ...
  const safeEndedAt = escapeEmailHtml(formatSubscriptionRejectedCreatedAt(endedAt));
  const safeNotes = escapeEmailHtml(removalNotes || "لا توجد ملاحظات إضافية");
...
${buildEmailParagraph(`نود إبلاغك بأنه تم <strong style="color:#ffffff;">إنهاء اشتراكك</strong> في باقة <strong style="color:#ffffff;">${safePlan}</strong>.`)}
${buildEmailToneCard({
  tone: "red",
  title: "حالة الاشتراك",
  body: "لم يعد بإمكانك الوصول إلى الخدمة المرتبطة بهذا الاشتراك.",
})}
${buildEmailToneCard({
  tone: "blue",
...
${buildEmailParagraph("تفاصيل الاشتراك:")}
${buildEmailDataPanel(`
${buildEmailDetailRows([
  { label: "الباقة", value: safePlan },
  { label: "المبلغ", value: safePrice },
  { label: "تاريخ الإنهاء", value: safeEndedAt },
  { label: "رقم الطلب", value: safeRequestId },
])}
`)}
${buildEmailParagraph("إذا رغبت بتجديد الاشتراك، يمكنك تقديم طلب جديد من صفحة الاشتراكات.", { muted: true })}
