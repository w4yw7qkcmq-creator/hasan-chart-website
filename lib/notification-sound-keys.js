...
export function normalizeNotificationKey(value) {
  ...
  for (const key of NOTIFICATION_SOUND_KEY_ORDER) {
    const legacyHyphen = key.replace(/_/g, "-");
    ...
    if (
      raw.startsWith(`${key}_`) ||
      raw.startsWith(`${legacyHyphen}-`) ||
      raw.startsWith(`${legacyHyphen}_`)
    ) {
      return key;
    }
  }
  ...
  if (NOTIFICATION_SOUND_KEY_ORDER.includes(underscored)) {
    return underscored;
  }

  const legacyUnderscored = LEGACY_SOUND_TYPE_TO_NOTIFICATION_KEY[raw.replace(/_/g, "-")];
  if (legacyUnderscored) {
    return legacyUnderscored;
  }

  return resolveNotificationKeyFromSiteType(raw.replace(/_/g, "-"));
}
