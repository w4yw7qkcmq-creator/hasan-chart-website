...
import { buildSmartLinkUrl } from "./smart-link-service.js";
...
export async function getPartnerGrowthOverview(supabase, partnerId, { tierKey = "partner" } = {}) {
  const [{ data: partner }, metrics, missions, milestones] = await Promise.all([
    supabase
      .from("partners")
      .select(
...
export async function getPartnerSmartLinksView(supabase, partnerId, referralCode) {
  const { data: links } = await supabase
    ...
    .select("id, token, label, source, medium, destination_path, status, created_at, campaign_program_id")
    .eq("partner_id", partnerId)
    .order("created_at", { ascending: false })
    .limit(50);
  ...
  return (links || []).map((link) => {
    ...
    const url = buildSmartLinkUrl(siteOrigin, {
      referralCode,
      token: link.token,
