...
import { loadPartnerCommissionRules } from "../partner-commission-rules.js";
import { SERVICE_COMMISSION_ARABIC_LABELS, TIER_POLICIES } from "./service-commission-constants.js";
import { getPartnerSiteUrl } from "../partner-shared.js";

export async function getPartnerServiceCommissionOffer(supabase, { tierKey = "partner", tierPercent = 10 } = {}) {
  const rules = await loadPartnerCommissionRules(supabase);
  const enabled = (rules || []).filter((r) => r.is_enabled !== false && r.is_active !== false);

  return {
    currentTierPercent: Number(tierPercent),
    headline: `نسبتك الحالية: ${Number(tierPercent)}%`,
    services: enabled.map((rule) => ({
      serviceType: rule.service_type,
      label: SERVICE_COMMISSION_ARABIC_LABELS[rule.service_type] || rule.display_name_ar || rule.service_type,
      tierPolicy: rule.tier_policy,
      displayPercent:
        rule.tier_policy === TIER_POLICIES.FIXED_SERVICE_RATE
          ? Number(rule.commission_percent)
          : Number(tierPercent),
...
export async function getPartnerGrowthOverview(supabase, partnerId, { tierKey = "partner" } = {}) {
  ...
  const serviceCommissionOffer = await getPartnerServiceCommissionOffer(supabase, {
    tierKey: partner?.tier_key || tierKey,
    tierPercent: Number(currentTier?.commission_percent || 10),
  });
...
export async function getPartnerCampaignMissionsView(supabase, partnerId, campaignId, { tierKey = "partner" } = {}) {
  ...
  const embedded = campaign.tracking_metadata?.missions || [];
  const { data: linkedDefs } = await supabase
    .from("partner_mission_definitions")
    .select("*")
    .eq("campaign_program_id", campaignId)
    .in("status", ["active", "paused"]);

  const missionDefs = (linkedDefs || []).length ? linkedDefs : [];
  const { data: progressRows } = await supabase
    .from("partner_mission_progress")
    .select("*")
    .eq("partner_id", partnerId);
  ...
  const rows = missionDefs.map((mission) => {
    ...
    return {
      id: mission.id,
      code: mission.code,
      title: mission.creative_metadata?.name_ar || mission.name,
      currentValue: Number(current),
      targetValue: target,
      progressPercent: safePercent(current, target),
      uiStatus,
  ...
  if (!rows.length && embedded.length) {
    return embedded.map((m, idx) => ({
      id: `embedded-${campaignId}-${idx}`,
      code: m.code,
      title: m.name_ar || m.name || m.code,
      currentValue: 0,
      targetValue: Number(m.target_value || 1),
      progressPercent: 0,
      uiStatus: { key: "available", label: missionStatusLabel("available") },
    }));
  }
  return rows;
}

function buildCampaignCountdown(campaign, within) {
  const now = new Date();
  if (!within && campaign.end_at) {
    return { kind: "ended", label: formatCountdownAr(campaign.end_at, { now }) };
  }
  if (campaign.start_at && new Date(campaign.start_at) > now) {
    return { kind: "starts_in", label: formatCountdownAr(campaign.start_at, { now }) };
  }
  if (campaign.end_at) {
    return { kind: "ends_in", label: formatCountdownAr(campaign.end_at, { now }) };
  }
  return null;
}
...
export async function getPartnerCampaignsView(supabase, partnerId, { tierKey = "partner" } = {}) {
  ...
  for (const campaign of campaigns || []) {
    ...
    const lifecycle = campaign.tracking_metadata?.lifecycle || null;
    const eligibility = await validatePartnerCampaignEligibility(supabase, {
      campaign,
      partnerId,
      tierKey,
    });
    const missions = await getPartnerCampaignMissionsView(supabase, partnerId, campaign.id, { tierKey });
    results.push({
      id: campaign.id,
      code: campaign.code,
      name: campaign.creative_metadata?.name_ar || campaign.name,
      nameAr: campaign.creative_metadata?.name_ar || campaign.name,
      description: campaign.creative_metadata?.description_ar || campaign.description || "",
      status: campaign.status,
      statusLabel: campaignStatusLabel(campaign.status, { withinWindow: within, lifecycle }),
      landingPath: campaign.landing_path,
      startAt: campaign.start_at,
      endAt: campaign.end_at,
      countdown: buildCampaignCountdown(campaign, within),
      ...
      missionsSummary: {
        ...
        completed: missions.filter((m) => ["completed", "reward_credited", "reward_pending"].includes(m.uiStatus?.key)).length,
        inProgress: missions.filter((m) => m.uiStatus?.key === "in_progress").length,
      },
    });
  }
  return results;
