...
export async function markReferralQualifiedOnActivation(
  ...
  const { ACTIVITY_EVENT_TYPES, recordTrustedQualificationActivity } = await import(
    "./qualification-activity.js"
  );
  const { markReferralCustomerOnService } = await import("./qualification-evaluator.js");

  const activity = await recordTrustedQualificationActivity(supabase, {
    referredUserId,
    partnerId,
    referralId,
    activityType: ACTIVITY_EVENT_TYPES.SERVICE_ACTIVATED,
    sourceEntityId: referralId,
    payload: { reason },
  });

  const customerResult = await markReferralCustomerOnService(supabase, {
    partnerId,
    referralId,
    referredUserId,
    reason: "service_customer",
    sourceEventId: activity.event?.eventId || null,
  });

  return {
    ...customerResult,
    activity,
    qualified: customerResult.evaluation?.currentState === QUALIFICATION_STATES.QUALIFIED,
  };
}
...
export const QUALIFICATION_STATE_MACHINE_SUMMARY = Object.freeze({
  ...
  note: "Qualified requires verified email, minimum account age, trusted server activity, and acceptable fraud risk — signup alone never qualifies",
});
