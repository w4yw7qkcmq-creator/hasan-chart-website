...
export function buildPeriodKey(periodType = PERIOD_TYPES.ONCE, at = new Date()) {
  const type = String(periodType || PERIOD_TYPES.ONCE).toLowerCase();
  if (type === PERIOD_TYPES.ONCE || !type) return "";

  const parts = getZonedParts(at, PARTNER_BUSINESS_TIMEZONE);

  if (type === PERIOD_TYPES.DAILY) {
