...
export async function buildQualificationEvaluationContext(supabase, referredUserId, now = new Date()) {
  ...
  const identityRisk = await evaluateDuplicateIdentityRisk(supabase, {
    referredUserId,
    partnerUserId: bundle.partner?.user_id || null,
    email: authUser?.email || null,
  });
  const duplicateIdentity = Boolean(identityRisk.duplicateIdentity);
  ...
  for (const nextState of path) {
    ...
    if (result.transitioned) {
      ...
      if (nextState === QUALIFICATION_STATES.QUALIFIED) {
        ...
        await creditPendingServiceCommissionsOnQualification(supabase, {
          referralId: ctx.bundle.referral.id,
          partnerId: ctx.bundle.referral.partner_id,
        });
