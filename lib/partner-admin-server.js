...
export async function listAdminPartnerWalletLedger(supabase, { partnerId, limit = 200 } = {}) {
  if (partnerId) {
    return listPartnerWalletLedger(supabase, partnerId, { limit });
  }
...
export async function markPartnerWithdrawalPaid(
  ...
  await createPartnerNotification(supabase, {
    ...
    body: `تم دفع ${paidRow.amount} ${paidRow.currency} بنجاح.`,
    payload: {
      withdrawalId: paidRow.id,
      amount: paidRow.amount,
    },
