...
function isBlockedPriceAlertQueueItem({ label, item }) {
  ...
  if (PRICE_ALERT_QUEUE_LABELS.has(normalizedLabel)) {
    return true;
  }
  ...
  if (normalizedLabel.includes("price") && normalizedLabel.includes("alert")) {
    return true;
  }
...
export async function processEmailQueue(items, options = {}) {
  ...
  for (const item of queueItems) {
    ...
    if (isBlockedPriceAlertQueueItem({ label, item })) {
      logPriceAlertEmailBlockedFromWebsite({
        path: "lib/email-queue.js::processEmailQueue",
        label,
        to,
