import { resolvePasswordRecoveryRedirectUrl } from "./auth-password-recovery.js";
...
import {
  revokeAllUserAccess,
  revokeAccessForAccountStatus,
} from "./iam/session-revocation-service.js";
import { REVOCATION_REASONS } from "./iam/revocation-reasons.js";
...
async function revokeUserSessionsForStatus(supabase, targetUserId, adminUser, nextStatus, request) {
  await revokeAccessForAccountStatus(supabase, {
    userId: targetUserId,
...
async function applyStatusTransition(
  supabase,
  { targetUserId, adminUser, reason, transitionKey, signOut = true, request = null }
) {
  ...
  if (signOut && transition.next !== "active") {
    await revokeUserSessionsForStatus(supabase, targetUserId, adminUser, transition.next, request);
  }
...
export async function forceSignOutUser(supabase, { targetUserId, adminUser, request = null }) {
...
export async function requestPasswordReset(supabase, { targetUserId, adminUser, email = "" }) {
  ...
  const { data, error } = await supabase.auth.admin.generateLink({
    ...
    options: {
      redirectTo: resolvePasswordRecoveryRedirectUrl(),
    },
  });
...
export async function forceLogoutUser(supabase, targetUserId, options = {}) {
