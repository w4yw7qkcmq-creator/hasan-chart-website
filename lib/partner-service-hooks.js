...
export async function onPartnerGenericServiceActivated(
  ...
  return safePartnerHook("GENERIC_SERVICE", async () => {
    ...
    let trustedBaseAmount = parseSubscriptionPrice(subscriptionPrice);

    const normalizedSubscriptionId = String(subscriptionId || "").trim();
    if (normalizedSubscriptionId) {
      const { data: subscription } = await supabase
        .from("subscription_requests")
        .select("id, price, status, user_email")
        .eq("id", normalizedSubscriptionId)
        .maybeSingle();

      if (subscription?.id && subscription.status === "مفعل") {
        trustedBaseAmount = parseSubscriptionPrice(subscription.price);
      }
    }
