...
export function buildCampaignSnapshotFingerprint(campaign) {
  const payload = {
    ...
    htmlContent: String(campaign?.html_content || "").trim(),
  };
...
export function campaignPatchInvalidatesSnapshot(existing, patch = {}) {
  ...
  for (const key of ["audienceType", "audienceFilter", "subject", "previewText", "htmlContent"]) {
    if (patch[key] !== undefined) {
      const draft = {
        ...
        preview_text: patch.previewText ?? existing.preview_text,
        html_content: patch.htmlContent ?? existing.html_content,
      };
