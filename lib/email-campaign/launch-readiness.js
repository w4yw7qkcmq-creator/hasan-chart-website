...
export function isPostLaunchCampaignStatus(status) {
  return POST_LAUNCH_STATUSES.has(String(status || "").trim());
}

export function getCampaignRuntimeMetrics(campaign) {
  return {
    queued: Number(campaign?.queued_count) || 0,
    delivered: Number(campaign?.delivered_count) || 0,
    failed: Number(campaign?.failed_count) || 0,
    providerAccepted: Number(campaign?.provider_accepted_count) || 0,
    eligible: Number(campaign?.eligible_count) || 0,
  };
}

export function buildPostLaunchWizardReadiness(campaign) {
  const status = String(campaign?.status || "").trim();
  const metrics = getCampaignRuntimeMetrics(campaign);
  ...
  return {
    ...
    campaignExists: true,
    showPreLaunchReadiness: false,
    blockers: [],
    confirmationBlockers: [],
    warnings: [],
    eligibleCount: metrics.eligible,
    excludedCount: Number(audienceStats.excluded) || 0,
...
export function getCampaignWizardReadiness(campaign) {
  if (campaign && isPostLaunchCampaignStatus(campaign.status)) {
    return buildPostLaunchWizardReadiness(campaign);
  }
