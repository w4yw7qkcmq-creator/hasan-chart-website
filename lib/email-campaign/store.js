...
import { resolveCampaignNamePatch } from "./draft-payload.js";
import { campaignPatchInvalidatesSnapshot } from "./launch-readiness.js";
import { sanitizeCampaignHtml } from "./renderer.js";
import { EMAIL_CAMPAIGN_LIST_COLUMNS } from "../supabase-query-columns.js";
...
export async function listCampaigns(
  ...
  let query = supabase
    ...
    .select(EMAIL_CAMPAIGN_LIST_COLUMNS, { count: "exact" })
...
export async function updateCampaignDraft(supabase, campaignId, patch = {}) {
  ...
  if (patch.name !== undefined) {
    const namePatch = resolveCampaignNamePatch(existing.name, patch.name);
    if (namePatch.action === "reject") {
      throw new Error(namePatch.error);
    }
    if (namePatch.action === "set") {
      updates.name = namePatch.value;
    }
  }
  ...
  if (campaignPatchInvalidatesSnapshot(existing, patch)) {
    updates.status = CAMPAIGN_STATUS.DRAFT;
    updates.eligible_count = 0;
    updates.audience_snapshot_count = 0;
    updates.metadata = {
      ...(existing.metadata || {}),
      audienceSnapshotStale: true,
