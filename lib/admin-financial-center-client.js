...
const INVALID_CURSOR_LITERALS = new Set(["undefined", "null"]);

export function isValidFinancialCenterCursor(value) {
  if (value == null) return false;
  const normalized = String(value).trim();
  if (!normalized) return false;
  return !INVALID_CURSOR_LITERALS.has(normalized.toLowerCase());
}

export function buildFinancialCenterSearchParams(section, query = {}) {
  ...
  for (const [key, value] of Object.entries(query)) {
    ...
    if (key === "cursor") {
      if (!isValidFinancialCenterCursor(value)) continue;
      params.set("cursor", String(value).trim());
      continue;
    }
...
export async function fetchFinancialCenterSection(adminFetch, section, { signal, query = {} } = {}) {
  const params = buildFinancialCenterSearchParams(section, query);
