...
import { countPendingSubscriptionRequests } from "./admin-pending-subscription-request.js";
...
export const SUBSCRIPTION_LIST_SELECT_FIELDS =
  "id,user_email,username,plan_name,category,price,telegram_username,payment_network,status,started_at,expires_at,created_at,admin_disabled,payment_proof_path";
  ...
  try {
    ...
    if (section === "stats") {
      const [
        ...
        countPendingSubscriptionRequests(supabase),
      ...
      console.info("ADMIN_SUBSCRIPTIONS_LOAD_STEP", {
        ...
        paymentProofPathCount: (data || []).filter((row) =>
      ...
      const rowsForEnrichment = (data || []).map((row) => ({
        ...row,
        has_payment_proof: Boolean(String(row?.payment_proof_path || "").trim()),
      }));
