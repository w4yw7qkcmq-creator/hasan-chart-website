...
export function createMemoryRateLimit({
  ...
  function check(identifier, { consume = true } = {}) {
    ...
    return {
      success: !blocked,
      remaining: Math.max(0, max - (consume ? projectedCount : entry.count)),
      resetTime: entry.resetTime,
    };
  }
      ...
      if (upstashLimiter) {
        try {
          const result = consume
            ? await upstashLimiter.limit(normalizedIdentifier)
            : await upstashLimiter.getRemaining(normalizedIdentifier);
...
export const loginFailedAuthLimiter = createRateLimit({
  ...
  max: readPositiveInt("LOGIN_FAILED_MAX", 8),
  useRedis: true,
});
