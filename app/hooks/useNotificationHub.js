...
export function useNotificationHub() {
  ...
  const filtersRef = useRef({ search: "", filterKey: "all", filterRead: "all" });
  ...
  const fetchFeed = useCallback(
    async ({ append = false, cursor = null } = {}) => {
      ...
      try {
        ...
        setItems((current) =>
          sortHubNotifications(append ? mergeHubItems(current, incoming) : incoming)
        );
    ...
    return subscribeNotificationHub((event) => {
      ...
      if (event.type === "upsert" && event.notification) {
        ...
        setItems((current) => {
          ...
          return matches ? sortHubNotifications([notification, ...without]) : without;
        });
        return;
      }
      ...
      if (event.type === "patch" && event.id) {
        setItems((current) =>
          sortHubNotifications(
            current.map((item) =>
              item.id === event.id ? enrichHubNotification({ ...item, ...event.patch }) : item
            )
          )
        );
        return;
  ...
  const togglePin = useCallback(async (notificationId, pinned) => {
    if (!notificationId) return;
