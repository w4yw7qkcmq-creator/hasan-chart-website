...
const ORDER_BOOK_FEAR_GREED_URL =
  "/api/market-sentiment/fear-greed?source=coinmarketcap";
...
function SemicircleGauge({ value, themed = false }) {
  ...
  const pointerStroke = themed
    ? "var(--ob-surface-elevated)"
    : "var(--ob-surface-elevated)";
  return (
    <div
      className={`relative mx-auto w-full max-w-[280px] ${themed ? ob.textStrong : ""}`}
    >
      {" "}
      <svg
        viewBox="0 0 200 118"
      ...
      >
        ...
        <text
          ...
          textAnchor="middle"
          fill={pointerFill}
          className={`text-[2rem] ${scoreClass}`}
        >
          {" "}
        ...
        <text
          ...
          y="108"
          textAnchor="middle"
          fill={themed ? "var(--ob-text-muted)" : "currentColor"}
  ...
  if (
    ...
    return {
      ...
      current: {
        ...
        classificationAr:
          result.classificationAr || fearGreedClassificationAr(result.value),
      },
    ...
    return (
      <div className={wrapperClass}>
        ...
        <div className="mb-2 flex items-center justify-between gap-2">
          {" "}
          <h3 className={`text-lg font-bold ${ob.textStrong}`}>
            مؤشر الخوف والطمع
          </h3>{" "}
          {isRefreshing ? (
            <span
        ...
        {initialLoading && !displayPayload?.current ? (
          <div
            className={`flex ${bodyHeightClass} flex-col items-center justify-center gap-2`}
          >
            {" "}
            <div
              className={`h-[7.5rem] w-full max-w-[280px] animate-pulse rounded-t-full ${ob.surfaceMuted}`}
            />{" "}
            <div
              className={`h-4 w-24 animate-pulse rounded ${ob.surfaceMuted}`}
            />{" "}
          </div>
        ...
        {displayPayload?.staleNotice ? (
          <p
            className={`mt-2 text-center text-[10px] ${ob.alertWarning} border-0 bg-transparent px-0 py-0`}
          >
            {" "}
            {displayPayload.staleNotice}{" "}
          </p>
        ...
        {displayPayload?.attribution ? (
          <p className={`mt-2 text-center text-[10px] ${ob.textSubtle}`}>
            {displayPayload.attribution}
          </p>
        ) : null}{" "}
      </div>
    ...
    <div className={wrapperClass}>
      ...
      <div className="mb-4 flex items-center justify-between gap-3">
        ...
        <div>
          ...
          <h3 className="site-price-card__title mb-0">
            مؤشر الخوف والطمع
          </h3>{" "}
        </div>{" "}
        {displayPayload?.staleNotice ? (
      ...
      {initialLoading && !displayPayload?.current ? (
        <div className="flex min-h-[8rem] flex-col items-center justify-center gap-3">
          {" "}
          <div className="h-24 w-full max-w-xs animate-pulse rounded-xl ob-skeleton-shimmer" />{" "}
          <div className="h-4 w-32 animate-pulse rounded ob-skeleton-shimmer" />{" "}
        </div>
      ) : !displayPayload?.current ? (
        <p className="site-price-card__meta min-h-[5rem] text-sm">
        ...
        <>
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            <p className="site-price-card__value text-base font-semibold">
              {" "}
              {displayPayload.current.classificationAr ||
                fearGreedClassificationAr(numericValue)}{" "}
            </p>{" "}
          </div>{" "}
          {displayPayload.history?.length ? (
            <div className="mt-4 flex h-16 items-end gap-1">
              {" "}
              {[...displayPayload.history]
                .reverse()
                .slice(-14)
                .map((entry) => (
                  <div
                    ...
                    style={{
                      ...
                      backgroundColor:
                        FEAR_GREED_GAUGE_SEGMENTS.find(
                          (s) => entry.value >= s.from && entry.value < s.to,
                        )?.color || "var(--ui-chart-fallback)",
                    }}
          ...
          {displayPayload.attribution ? (
            <p className="site-price-card__meta mt-4 text-xs">
              {displayPayload.attribution}
            </p>
          ) : null}{" "}
        </>
