...
export default function OrderBookPageContent() {
  ...
  const [liquidityWallWindow, setLiquidityWallWindow] = useState(
    DEFAULT_LIQUIDITY_WALL_WINDOW,
  );
  const [liquidityWallsWindow, setLiquidityWallsWindow] = useState(
    DEFAULT_LIQUIDITY_WALLS_SUMMARY_WINDOW,
  );
  const [liquidityDepthWindow, setLiquidityDepthWindow] = useState(
    DEFAULT_LIQUIDITY_DEPTH_WINDOW,
  );
  const {
    flowHistory,
  ...
  const {
    ...
    prefs,
    hydrated,
    wallWindow: isSidebarWallsLive
      ? DEFAULT_LIQUIDITY_WALL_WINDOW
      : liquidityWallsWindow,
  ...
  } = useLiquidityDepthHistory({
    prefs,
    hydrated,
    depthWindow: liquidityDepthWindow,
  });
  const {
  ...
  const largeTradeThreshold =
    prefs.largeTradeThreshold ?? DEFAULT_LARGE_TRADE_THRESHOLD;
  ...
  const historicalExecutedFlow =
    needsFlowHistory && flowHistory?.window === prefs.flowWindow
      ? flowHistory
      : null;
  const executedFlow = needsFlowHistory
    ? historicalExecutedFlow
    : liveExecutedFlow;
  const executedFlowLoading = needsFlowHistory
    ? historyLoading ||
      (Boolean(prefs.flowWindow) && !historicalExecutedFlow && !historyError)
  ...
  const connectedCount =
    data?.connectedExchangeCount ??
    (data?.exchangeStatuses || []).filter((item) => item.status === "connected")
      .length;
  const totalExchanges =
    data?.expectedExchangeCount ?? (data?.exchangeStatuses?.length || 3);
  ...
  useEffect(() => {
    ...
    if (!normalized) {
      ...
      setPrefs(
        {
          symbol: "BTCUSDT",
          precision: getDefaultPrecision("BTCUSDT", data?.lastPrice),
        },
        { source: "fallback" },
      );
    ...
    void fetch(
      `/api/market-symbols?query=${encodeURIComponent(normalized)}&limit=5&minExchanges=2`,
    )
      .then((response) => response.json())
      .then((payload) => {
        const supported = (payload?.symbols || []).some(
          (entry) => entry.symbol === normalized,
        );
        if (supported) {
          setPrefs(
            {
              symbol: normalized,
              precision: getDefaultPrecision(normalized, data?.lastPrice),
            },
            { source: "url" },
          );
          return;
        ...
        if (!payload?.available) {
          setSymbolNotice(
            "تعذّر تحميل قائمة العملات حاليًا. العملات الأساسية فقط متاحة مؤقتًا.",
          );
        } else {
          setSymbolNotice(
            "الرمز المطلوب غير متاح حاليًا، تم الرجوع إلى BTC/USDT.",
          );
        }
        setPrefs(
          {
  ...
  if (!hydrated) {
    return (
      <div className={`mx-auto max-w-7xl px-4 py-8 ${ob.page}`}>
        {" "}
        <div
          className="h-96 animate-pulse rounded-2xl bg-[var(--ob-surface-muted)] motion-reduce:animate-none"
          aria-hidden="true"
        />{" "}
    ...
    <div className={`mx-auto max-w-7xl px-4 py-6 sm:py-8 ${ob.page}`} dir="rtl">
      ...
      <header className="mt-4 mb-6">
        ...
        <p className={`mt-2 max-w-2xl ${ob.body} ob-text-muted`}>
          {" "}
          متابعة لحظية لدفتر الأوامر، السيولة، جدران الأوامر، الصفقات الكبيرة،
          وحجم التداول المنفذ.{" "}
        </p>{" "}
      </header>{" "}
      ...
      <section className="mb-5 grid items-stretch gap-3 lg:grid-cols-12 lg:gap-4">
        {" "}
        <div
          className={`flex min-h-[7.75rem] flex-col justify-between p-4 sm:p-5 lg:col-span-5 ${ob.surface}`}
        >
          {" "}
          <div className="flex flex-wrap items-start justify-between gap-2">
            ...
            <div className="min-w-0">
              ...
              <h2 className={`truncate text-xl font-bold ${ob.textStrong}`}>
                {displaySymbol}
              </h2>{" "}
          ...
          <div className="mt-3">
            ...
            <NumericValue
              className={`text-2xl font-bold sm:text-3xl ${ob.textStrong}`}
            >
              {" "}
        ...
        <div className="grid min-w-0 grid-cols-2 gap-3 lg:col-span-7 lg:grid-cols-4">
          ...
          <StatTile
            label="السبريد"
            sublabel="لحظي"
            value={formatPrice(data?.spread, 4)}
            tone="neutral"
            icon="↔"
          />{" "}
          <StatTile
      ...
      <section className={`mb-5 grid gap-4 p-4 sm:p-5 ${ob.surface}`}>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <OrderBookListbox
            ...
            value={String(
              prefs.liquidityRange ?? DEFAULT_LIQUIDITY_RANGE_PERCENT,
            )}
            onChange={(value) => setPrefs({ liquidityRange: Number(value) })}
            options={LIQUIDITY_RANGE_OPTIONS.map((entry) => ({
              value: String(entry),
              label: `${entry}%`,
            }))}
            optionValueDir="ltr"
        ...
        <div className="grid gap-3 border-t pt-4 sm:grid-cols-2 border-[var(--ob-border)]">
          ...
          <OrderBookListbox
            ...
            options={precisionOptions.map((entry) => ({
              value: String(entry),
              label: String(entry),
            }))}
            optionValueDir="ltr"
          ...
          <OrderBookListbox
            ...
            options={DEPTH_LEVEL_OPTIONS.map((entry) => ({
              value: String(entry),
              label: String(entry),
            }))}
            optionValueDir="ltr"
      ...
      <div className="space-y-4">
          ...
          <div className="flex min-h-0 min-w-0 flex-col gap-3 lg:col-span-4 lg:gap-4">
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <SegmentedControl
                  ...
                  scrollable
                  options={FLOW_WINDOW_OPTIONS.map((value) => ({
                    value,
                    label: value,
                  }))}
                />{" "}
                <HistoryState
                  loading={
                    needsDominanceHistory &&
                    (historyLoading ||
                      (!historicalDominanceFlow && !historyError))
                  }
                ...
                {dominanceFlow ? (
                  ...
                  <div
                    className="h-8 rounded-lg ob-surface-muted"
                    aria-hidden="true"
                  />
                )}{" "}
              </div>{" "}
              <div className="mt-3 grid gap-2 sm:grid-cols-2">
                ...
                <MetricLine
                  label="شراء منفذ"
                  value={formatUsd(dominanceFlow?.buyNotional, {
                    compact: true,
                  })}
                  tone="buy"
                />{" "}
                <MetricLine
                  label="بيع منفذ"
                  value={formatUsd(dominanceFlow?.sellNotional, {
                    compact: true,
                  })}
                  tone="sell"
                />{" "}
                ...
                />{" "}
                <p
                  className={`flex items-center justify-center rounded-lg px-2.5 py-2 text-center text-xs font-medium sm:col-span-2 ob-surface-muted ${ob.textStrong}`}
                >
                  {" "}
                  {dominanceFlow?.dominanceLabel ||
                    dominanceFlow?.dominanceClassification ||
                    "متوازن"}{" "}
                </p>{" "}
              </div>{" "}
            </Panel>{" "}
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <HistoryState
                  loading={
                    needsFlowHistory &&
                    (historyLoading ||
                      (!historicalExecutedFlow && !historyError))
                  }
              ...
              <div className="mt-3 grid grid-cols-2 gap-2 transition-opacity duration-200">
                ...
                <MetricLine
                  label="شراء منفذ"
                  value={formatUsd(executedFlow?.buyNotional, {
                    compact: true,
                  })}
                  tone="buy"
                />{" "}
                <MetricLine
                  label="بيع منفذ"
                  value={formatUsd(executedFlow?.sellNotional, {
                    compact: true,
                  })}
                  tone="sell"
                />{" "}
                <MetricLine
                  label="صافي التدفق"
                  value={formatUsd(
                    executedFlow?.netNotional ?? executedFlow?.netFlow,
                    { compact: true },
                  )}
                />{" "}
                <p
                  className={`flex min-h-[2.75rem] items-center justify-center rounded-lg px-2.5 py-2 text-center text-xs font-medium sm:col-span-2 ob-surface-muted ${ob.textStrong}`}
                >
                  {" "}
                  {executedFlow?.dominanceLabel ||
                    executedFlow?.dominanceClassification ||
                    "متوازن"}{" "}
                </p>{" "}
              </div>{" "}
              {executedFlow?.window ? (
          ...
          >
            ...
            {isSidebarWallsLive ? (
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                {" "}
                <LiveWallCard
                  title="أكبر جدار شراء"
                  wall={sidebarWallsBid}
                  tone="buy"
                />{" "}
                <LiveWallCard
                  title="أكبر جدار بيع"
                  wall={sidebarWallsAsk}
                  tone="sell"
                />{" "}
              </div>
            ) : sidebarWallsLoading ||
              sidebarWallsError ? null : sidebarWallsEmpty ? (
            ...
            <div className="flex min-h-0 flex-1 flex-col">
              ...
              <LiquidityDepthChart
                ...
                points={
                  isLiveDepth
                    ? data?.depthMap || []
                    : depthHistory?.aggregatedDepthPoints || []
                }
            ...
            <div className="shrink-0 space-y-4">
              ...
              <div className="flex flex-col gap-4 lg:flex-row lg:flex-wrap lg:items-start lg:gap-x-8 lg:gap-y-3">
                ...
                <SegmentedControl
                  ...
                  onChange={(value) =>
                    setPrefs({ largeTradeThreshold: Number(value) })
                  }
                  mobileScrollable
                ...
                <SegmentedControl
                  ...
                  mobileScrollable
                  options={LARGE_TRADE_WINDOW_OPTIONS.map((value) => ({
                    value,
                    label: value,
                  }))}
                />{" "}
              ...
              {displayedLargeTrades.length ? (
                <div className="max-h-[26rem] overflow-y-auto overflow-x-auto overscroll-contain rounded-xl border [scrollbar-width:thin] border-[var(--ob-border)]">
                  ...
                  <table className="w-full min-w-[42rem] table-auto text-sm tabular-nums">
                    {" "}
                    <thead
                      className={`${ob.tableHeader} text-xs leading-normal`}
                    >
                      {" "}
                      <tr>
                        {" "}
                        <th className="whitespace-nowrap px-2 py-2.5 text-right sm:px-3">
                          الوقت
                        </th>{" "}
                        <th className="whitespace-nowrap px-2 py-2.5 text-right sm:px-3">
                          المنصة
                      ...
                      {displayedLargeTrades.map((trade) => (
                        ...
                        >
                          ...
                          <td className="whitespace-nowrap px-2 py-2.5 align-middle sm:px-3">
                            {" "}
                            <NumericValue className="text-xs leading-normal">
                              {formatTime(trade.ts)}
                            </NumericValue>{" "}
                          </td>{" "}
                          ...
                          <td className="whitespace-nowrap px-2 py-2.5 align-middle sm:px-3">
                            {" "}
                            <NumericValue
                              className={`font-semibold leading-normal ${ob.textStrong}`}
                            >
                              {" "}
        ...
        <section>
          ...
          <details open className={`group ${ob.surface}`}>
            {" "}
            <summary
              className={`cursor-pointer list-none p-4 sm:p-5 [&::-webkit-details-marker]:hidden ${ob.focusRing}`}
            >
              {" "}
              <div className="flex items-center justify-between gap-3">
                ...
                <span
                  className={`text-xs ${ob.textSubtle} group-open:rotate-180 transition-transform motion-reduce:transition-none`}
                  aria-hidden="true"
                >
                  ▾
                </span>{" "}
            ...
            <div className={`${ob.divider} px-4 pb-4 pt-3 sm:px-5 sm:pb-5`}>
              ...
              <div className="grid gap-2 sm:grid-cols-3">
                ...
                {(data?.exchangeStatuses || []).map((item) => {
                  ...
                  const label =
                    item.probeLabel || (connected ? "متصل" : "غير متصل");
                  return (
                    <div
                      key={item.exchange}
              ...
              {data?.disclaimer ? (
                <p className={`mt-2 text-xs leading-5 ${ob.textMuted}`}>
                  {data.disclaimer}
                </p>
              ) : null}{" "}
            </div>{" "}
