...
const ANALYTICS_ITEMS = [
  {
    key: "strongestBid",
    label: "أقوى جدار شراء",
    hint: "أقوى مستوى شراء خلال الفترة.",
  },
  {
    key: "strongestAsk",
    label: "أقوى جدار بيع",
    hint: "أقوى مستوى بيع خلال الفترة.",
  },
  {
    key: "strongestWall",
    label: "أقوى جدار",
    hint: "الجدار صاحب أعلى مؤشر قوة وثبات خلال الفترة.",
  },
  {
    key: "longestLivingWall",
    label: "الأطول بقاءً",
    hint: "الجدار الذي بقي ظاهرًا لأطول مدة.",
  },
  {
    key: "mostReappearedWall",
    label: "الأكثر تكرارًا",
    hint: "الجدار الذي اختفى وعاد للظهور أكثر من مرة.",
  },
  {
    key: "largestNotionalWall",
    label: "الأكبر من حيث القيمة",
    hint: "الجدار صاحب أعلى قيمة نقدية مسجّلة.",
  },
];
...
function AnalyticsCard({ label, hint, row }) {
  ...
  return (
    <div
      className={`flex h-full min-h-[8.5rem] flex-col rounded-xl border p-3 sm:p-4 ${isBuy ? "border-[var(--ob-positive-border)] bg-[var(--ob-positive-soft)]" : "border-[var(--ob-negative-border)] bg-[var(--ob-negative-soft)]"}`}
      title={hint}
    >
      {" "}
      ...
      <div className="mt-2 flex flex-1 flex-col justify-end gap-1.5 text-sm">
        ...
        {Number.isFinite(row.notional) ? (
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <NumericValue
              className={`font-semibold ${isBuy ? ob.positive : ob.negative}`}
            >
              {" "}
...
function historyHasRows(history) {
  return (
    ...
    Boolean(
      history?.analytics?.strongestBid || history?.analytics?.strongestAsk,
    )
  );
}
    ...
    <div>
      ...
      {usingFallback ? (
        <p className={`mb-3 text-xs ${ob.textMuted}`}>
          {" "}
          لا توجد نتائج في هذا التبويب حاليًا — يُعرض أفضل ما هو متاح من بقية
          السجل.{" "}
        </p>
      ) : null}{" "}
      <div className="hidden overflow-x-auto md:block">
        {" "}
        <div
          className={`max-h-80 overflow-y-auto rounded-xl border ${ob.surfaceMuted}`}
        >
          {" "}
          <table className="w-full min-w-[640px] text-sm">
            ...
            <thead className={`text-xs ${ob.tableHeader} ${ob.textMuted}`}>
              ...
              <tr>
                ...
                {showLastSeen ? (
                  <th className="px-3 py-2.5 text-right">آخر ظهور</th>
                ) : null}{" "}
              </tr>{" "}
            </thead>{" "}
            <tbody>
              ...
              {visibleRows.map((row) => (
                ...
                >
                  ...
                  <td className="px-3 py-2.5">
                    {formatDurationAr(row.lifetimeSeconds)}
                  </td>{" "}
                  <td className="px-3 py-2.5">
                    {" "}
      ...
      <div className="space-y-3 md:hidden">
        ...
        {visibleRows.map((row) => {
          ...
          return (
            <div
              key={row.wallKey}
              className={`rounded-xl border p-3 ${isBuy ? "border-[var(--ob-positive-border)] bg-[var(--ob-positive-soft)]" : "border-[var(--ob-negative-border)] bg-[var(--ob-negative-soft)]"}`}
            >
              {" "}
      ...
      {rows.length > DEFAULT_VISIBLE ? (
        ...
        >
          ...
          {expanded
            ? "عرض أقل"
            : `عرض المزيد (${Math.min(rows.length, EXPANDED_VISIBLE) - DEFAULT_VISIBLE}+)`}{" "}
        </button>
      ) : null}{" "}
    </div>
  );
...
export default function HistoricalLiquidityWallsPanel({
  ...
  const currentTab =
    WALL_TABS.find((tab) => tab.id === activeTab) || WALL_TABS[0];
  ...
  const usingFallback = Boolean(
    displayHistory && tabRows.length === 0 && rows.length > 0,
  );
  ...
  return (
    ...
    >
      {" "}
      <details
        className={`mb-4 rounded-xl border px-3 py-2 text-xs ${ob.surfaceMuted} ${ob.textNormal}`}
      >
        {" "}
        <summary className={`cursor-pointer font-medium ${ob.textStrong}`}>
          ما المقصود بالجدار؟
        </summary>{" "}
        <ul
          className={`mt-2 list-disc space-y-1 pr-4 leading-6 ${ob.textMuted}`}
        >
          {" "}
      ...
      {!showInitialLoading && error && !displayHistory ? (
        <p className={`mb-4 min-h-[2.5rem] ${ob.alertError}`}>
          {" "}
          تعذر تحميل البيانات التاريخية. حاول تحديث الإطار أو أعد المحاولة
          لاحقًا.{" "}
        </p>
      ) : null}{" "}
        ...
        <div className="mb-4 space-y-2">
          ...
          {isShowingPreviousWindow ? (
            <>
              ...
              <span
                className={`inline-flex items-center rounded-full border px-2.5 py-1 text-[11px] ${ob.alertInfo}`}
              >
                {" "}
          ...
          {!isShowingPreviousWindow ? (
            <p className={`text-[11px] leading-5 ${ob.textMuted}`}>
              {" "}
              قد تبقى أسعار الجدران متطابقة بين الإطارات إذا كان نفس الجدار هو
              الأقوى خلالها.{" "}
            </p>
          ) : null}{" "}
        </div>
      ...
      />{" "}
      <p className={`mt-3 text-xs leading-6 ${ob.textMuted}`}>
        {currentTab.hint}
      </p>{" "}
      <div className="relative mt-4">
        ...
        {showRefreshOverlay ? (
          <div
            aria-hidden="true"
            className={`${ob.overlayScrim} z-10 rounded-xl`}
          />
        ) : null}{" "}
        {!showInitialLoading &&
        !(error && !displayHistory) &&
        !rows.length &&
        !analytics.length ? (
          <EmptyState message="لا توجد جدران تاريخية ضمن الإطار المختار بعد." />
          ...
          <WallsTable
            rows={rows}
            showLastSeen={activeTab === "disappeared"}
