...
export function ConnectionStatusBadge({
  ...
  return (
    <span className={className} role="status" aria-live="polite">
      ...
      <span
        aria-hidden="true"
        className={`h-1.5 w-1.5 rounded-full ${allConnected ? ob.statusDotConnected : partial ? ob.statusDotWarning : ob.statusDotDisconnected}`}
      />{" "}
      {label}{" "}
    </span>
  );
...
export function Panel({
  ...
  return (
    <section
      className={`min-w-0 overflow-x-hidden ${ob.surface} ${compact ? "p-3 sm:p-4" : "p-4 sm:p-5"} ${className}`}
    >
      {" "}
      <div className={`min-w-0 shrink-0 ${compact ? "mb-3" : "mb-4"}`}>
        ...
        {description ? (
          <p className={`mt-1 ${ob.body} ob-text-muted`}>{description}</p>
        ) : null}{" "}
        {action ? (
          <div className="mt-3 w-full min-w-0 max-w-full">{action}</div>
        ) : null}{" "}
      </div>{" "}
      {children}{" "}
    </section>
        ...
        {options.map((option) => {
          ...
          return (
            <button
              ...
              className={`${ob.segmentedBtn} ${ob.focusRing} ${compact ? "py-1.5" : "py-2"} ${active ? ob.segmentedActive : ob.segmentedIdle} ${option.tone === "buy" && active ? ob.segmentedRingBuy : ""} ${option.tone === "sell" && active ? ob.segmentedRingSell : ""}`}
            >
              {" "}
              {option.label}{" "}
            </button>
...
function useListboxMenuPosition(open, triggerRef) {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const openUp =
        spaceBelow < LISTBOX_MENU_MAX_HEIGHT && spaceAbove > spaceBelow;
      setStyle({
        position: "fixed",
...
export function OrderBookListbox({
                ...
                <li key={option.value} role="presentation">
                  ...
                  >
                    ...
                    {isSelected ? (
                      <span
                        aria-hidden="true"
                        className={ob.listboxSelectedMark}
                      >
                        {" "}
                        ✓{" "}
                      </span>
  ...
  return (
    <div
      ref={rootRef}
      className={`relative min-w-0 ${compact ? "min-w-[7rem]" : ""}`}
    >
      {" "}
      <div className="flex min-w-0 flex-col gap-1.5 text-sm">
        ...
        >
          ...
          <span
            dir={optionValueDir}
            className={`min-w-0 flex-1 truncate text-right ${optionValueDir === "ltr" ? "tabular-nums" : ""}`}
          >
            {" "}
            {selected?.label || "—"}{" "}
          </span>{" "}
...
export function SymbolSearchCombobox({
  ...
  const selected =
    [...entries, ...remoteEntries, ...lastGoodEntries].find(
      (entry) => entry.value === value,
    ) || (value ? { value, label: formatMarketSymbol(value) } : null);
  const filtered = useMemo(() => {
    const pool = remoteEntries.length
      ? remoteEntries
      : lastGoodEntries.length
        ? lastGoodEntries
        : entries;
    ...
    return pool.filter((entry) => {
      ...
      return candidates.some(
        (candidate) =>
          ...
          compact.includes(candidate.replace("/", "")),
      );
    });
                  ...
                  <li key={entry.value} role="presentation">
                    ...
                    >
                      ...
                      <span className="min-w-0 text-right">
                        {" "}
                        <span className={ob.listboxOptionPrimary}>
                          {entry.label}
                        </span>{" "}
                        {entry.displayName ? (
                          <span className={ob.listboxOptionMuted}>
                            {entry.displayName}
                          </span>
                        ) : null}{" "}
                      </span>{" "}
...
export function RefreshSpinner({ className = "" }) {
  return <span aria-hidden="true" className={`${ob.spinner} ${className}`} />;
}
export function DepthHistoryState({
  loading,
  error,
  partial,
  coveragePercent,
  collecting,
  minHeight = "h-44 sm:h-48",
}) {
  if (loading) {
    return (
      <div
        className={`mb-3 flex ${minHeight} items-center justify-center rounded-xl text-sm ob-surface-muted ob-text-muted`}
        role="status"
        aria-live="polite"
      >
        {" "}
  ...
  if (error) {
    return (
      <div
        className={`mb-3 flex ${minHeight} items-center justify-center rounded-xl border px-4 text-sm ${ob.alertError}`}
        role="alert"
      >
        {" "}
...
export function HistoryState({
  ...
  if (loading) {
    return (
      <p
        className={`mb-3 min-h-[2.5rem] rounded-lg px-3 py-2 text-xs ${ob.alertInfo}`}
        role="status"
        aria-live="polite"
      >
        {" "}
  ...
  if (error) {
    return (
      <p
        className={`mb-3 min-h-[2.5rem] rounded-lg px-3 py-2 text-xs ${ob.alertError}`}
        role="alert"
      >
        {" "}
        {errorMessage}{" "}
  ...
  if (collecting) {
    return (
      <p
        className={`mb-3 min-h-[2.5rem] rounded-lg px-2.5 py-1.5 text-xs ${ob.alertWarning}`}
        role="status"
        aria-live="polite"
      >
        {" "}
...
export function MetricLine({ label, value, tone }) {
  ...
  return (
    <div
      className={`flex items-center justify-between gap-3 border-b py-2 text-sm last:border-b-0 border-[var(--ob-border)]`}
    >
      {" "}
      <span className={ob.textNormal}>{label}</span>{" "}
      <NumericValue className={`font-semibold ${toneClass}`}>
        {value}
      </NumericValue>{" "}
    </div>
  );
    ...
    <div className={ob.statTile}>
      ...
      <div className="flex items-start justify-between gap-2">
        ...
        <div className="min-w-0 flex items-start gap-2">
          {" "}
          <span
            className={`mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-sm ${ob.surfaceMuted} ob-text-muted`}
            aria-hidden="true"
          >
            {" "}
            {iconGlyph}{" "}
          ...
          <div className="min-w-0">
            ...
            {sublabel ? (
              <p className={`mt-0.5 text-xs ${ob.textSubtle}`}>{sublabel}</p>
            ) : null}{" "}
          </div>{" "}
        </div>{" "}
...
export function EmptyState({ message, icon = "◌" }) {
  return (
    <div
      className={`flex min-h-[8rem] flex-col items-center justify-center rounded-xl border border-dashed px-4 py-8 text-center ob-surface-muted`}
    >
      {" "}
