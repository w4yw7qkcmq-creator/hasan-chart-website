...
function SideBadgeLabel({ side }) {
  if (side === "long") {
    return (
      <span
        className={`inline-flex items-center gap-1 text-xs font-semibold ${ob.negative}`}
      >
        {" "}
        <span aria-hidden="true">▲</span> Long (تصفية مراكز شراء){" "}
      </span>
    );
  }
  if (side === "short") {
    return (
      <span
        className={`inline-flex items-center gap-1 text-xs font-semibold ${ob.positive}`}
      >
        {" "}
        <span aria-hidden="true">▼</span> Short (تصفية مراكز بيع){" "}
      </span>
    );
  }
  return "—";
...
function SummaryWindowCard({ title, bucket }) {
  return (
    <div className={`rounded-xl border p-3 ${ob.surfaceMuted}`}>
      {" "}
      <div className={`mb-2 text-xs font-semibold ${ob.textMuted}`}>
        {title}
      </div>{" "}
      <div className={`mb-2 text-lg font-bold ${ob.textStrong}`}>
        {" "}
        <NumericValue>
          {formatUsd(bucket?.total, { compact: true })}
        </NumericValue>{" "}
      </div>{" "}
...
function ExchangeTable({ rows }) {
  ...
  return (
    <div
      className={`overflow-x-auto rounded-xl border [scrollbar-width:thin] ${ob.surfaceMuted}`}
    >
      {" "}
      <table className="w-full min-w-[520px] text-sm">
        ...
        <tbody>
          ...
          {rows.map((row) => (
            ...
            >
              {" "}
              <td className={`px-3 py-2 font-medium ${ob.textStrong}`}>
                {row.exchange}
              </td>{" "}
              <td className="px-3 py-2">
                {" "}
                <NumericValue>
                  {formatUsd(row.total, { compact: true })}
                </NumericValue>{" "}
              </td>{" "}
              <td className={`px-3 py-2 ${ob.negative}`}>
                {" "}
                <NumericValue>
                  {formatUsd(row.long, { compact: true })}
                </NumericValue>{" "}
              </td>{" "}
              <td className={`px-3 py-2 ${ob.positive}`}>
                {" "}
                <NumericValue>
                  {formatUsd(row.short, { compact: true })}
                </NumericValue>{" "}
              </td>{" "}
...
function RealtimeTable({ rows }) {
  ...
  return (
    <div
      className={`max-h-72 overflow-y-auto overflow-x-auto rounded-xl border [scrollbar-width:thin] ${ob.surfaceMuted}`}
    >
      {" "}
      <table className="w-full min-w-[560px] text-sm">
        {" "}
        <thead
          className={`sticky top-0 z-10 text-[11px] ${ob.tableHeader} ${ob.textMuted}`}
        >
          {" "}
          <tr>
            {" "}
        ...
        <tbody>
          ...
          {rows.map((row, index) => (
            <tr
              key={`${row.id ?? "liq"}-${index}`}
              className={`border-t border-[var(--ob-border)] ${ob.rowHover}`}
            >
              {" "}
              <td className={`px-3 py-1.5 font-medium ${ob.textStrong}`}>
                {row.symbol || "—"}
              </td>{" "}
              <td className="px-3 py-1.5">
                {" "}
                <NumericValue className="text-xs">
              ...
              <td className="px-3 py-1.5">
                {" "}
                <NumericValue className={`text-xs ${ob.textMuted}`}>
                  {formatTime(row.time)}
                </NumericValue>{" "}
              </td>{" "}
              <td className="px-3 py-1.5">
              ...
              <td className={`px-3 py-1.5 text-left text-xs ${ob.textMuted}`}>
                {row.exchange || "—"}
              </td>{" "}
            </tr>
          ))}{" "}
...
export default function LiquidationsPanel({
        ...
        <Panel title="تصفيات السوق" description="جاري تحميل بيانات التصفيات...">
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            ...
            {Array.from({ length: 4 }).map((_, index) => (
              <div
                key={index}
                className={`h-28 animate-pulse rounded-xl ${ob.surfaceMuted}`}
              />
            ))}{" "}
          </div>{" "}
        </Panel>{" "}
  ...
  if (unavailable) {
    return (
      <section className="mb-5">
        ...
        <Panel
          title="تصفيات السوق"
          description="ملخص التصفيات حسب الإطار الزمني والمنصة."
        >
          {" "}
  ...
  return (
    <section className="mb-5 space-y-5">
      ...
      <Panel
        title="التصفيات حسب المنصة"
        description="Binance وBybit وOKX مع الإجمالي والحصة (4h)."
      >
        {" "}
        <ExchangeTable rows={exchanges} />{" "}
      </Panel>{" "}
      <Panel
        title="التصفيات في الوقت الفعلي"
        description="آخر أوامر التصفية الظاهرة على CoinGlass."
      >
        {" "}
