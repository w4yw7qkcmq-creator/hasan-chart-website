...
export default function StatusBadge({ status, variant = "admin-request", subscriptionRow = null }) {
  if (variant === "partner") {
  ...
  if (subscriptionRow) {
    ...
    if (diagnostic.reason === LEGACY_ENGLISH_PENDING_WITHOUT_PROOF_REASON) {
      return (
        <span className="admin-badge admin-badge--suspended shrink-0">طلب قديم</span>
      );
    }
