...
export function UiEmptyState({
  ...
  return (
    <div className={`${ui.empty} ${className}`.trim()} {...props}>
      ...
      {description ? (
        <p className="mt-2 text-sm ui-text-muted">{description}</p>
      ) : null}{" "}
    </div>
  );
}
export function UiLoadingState({ label = "جاري التحميل...", className = "", ...props }) {
  return (
    <div
      className={`${ui.loading} ${className}`.trim()}
      aria-live="polite"
      aria-busy="true"
      {...props}
    >
...
export function UiErrorState({
  ...
  return (
    <div className={`${ui.error} ${className}`.trim()} role="alert" {...props}>
      {" "}
