...
export function IamLoadingSkeleton({ rows = 4, variant = "rows" }) {
  if (variant === "cards") {
    return (
      <div
        className="iam-skeleton iam-skeleton--cards"
        aria-busy="true"
  ...
  if (variant === "table") {
    return (
      <div
        className="iam-skeleton iam-skeleton--table"
        aria-busy="true"
        aria-label="جاري التحميل"
      >
        {" "}
...
export function IamStatCard({ title, value, hint, icon, href, onNavigate }) {
  ...
  if (href && onNavigate) {
    return (
      <button
        type="button"
        className="iam-stat iam-stat--link"
    ...
    <div className="iam-user-cell">
      ...
      <div className="iam-user-cell__text">
        ...
        {showTechnical && record?.user_id ? (
          <code className="iam-tech-id">
            {String(record.user_id).slice(0, 8)}…
          </code>
        ) : null}{" "}
      </div>{" "}
    </div>
...
export function IamRoleBadge({ roleId }) {
  ...
  const tone =
    risk === "critical" ? "danger" : risk === "high" ? "warning" : "primary";
