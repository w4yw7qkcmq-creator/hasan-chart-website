      ...
      >
        ...
        <div className="iam-modal__steps" aria-label="خطوات الإسناد">
          ...
          {["المستخدم", "الدور", "المراجعة"].map((label, i) => (
            <span
              key={label}
              className={`iam-step ${step > i ? "is-done" : ""} ${step === i + 1 ? "is-active" : ""}`}
            >
              {" "}
              {i + 1}. {label}{" "}
            </span>
        ...
        <form onSubmit={handleSubmit} className="iam-modal__body">
          ...
          {step === 1 && (
            <>
              ...
              <p className="iam-field-hint">
                ابحث بالبريد — لا حاجة لمعرف المستخدم.
              </p>{" "}
            </>
          )}{" "}
          {step === 2 && (
            <>
              ...
              <div className="iam-role-preview">
                ...
                <p>
                  {IAM_ROLE_DESCRIPTIONS[form.roleId] ||
                    selectedRole?.description ||
                    "—"}
                </p>{" "}
                {risk === "critical" || risk === "high" ? (
                  <p className="iam-warning">
                    ⚠ دور عالي الخطورة — تأكد من الحاجة قبل الإسناد.
                  </p>
                ) : null}{" "}
              </div>{" "}
              <label className="iam-field">
          ...
          {step === 3 && (
            <div className="iam-review">
              ...
              <dl>
                ...
                <div>
                  {" "}
                  <dt>المستخدم</dt> <dd>{form.email}</dd>{" "}
                </div>{" "}
                <div>
                  {" "}
                  <dt>الدور</dt>{" "}
                ...
                <div>
                  {" "}
                  <dt>النطاق</dt> <dd>عام — المؤسسة الافتراضية</dd>{" "}
                </div>{" "}
                <div>
                  {" "}
                  <dt>المدة</dt> <dd>دائم</dd>{" "}
                </div>{" "}
                <div>
                  {" "}
                  <dt>السبب</dt> <dd>{form.reason}</dd>{" "}
                </div>{" "}
              </dl>{" "}
            </div>
          )}{" "}
...
export function IamRevokeModal({
  open,
  assignment,
  onClose,
  onConfirm,
    ...
    <div className="iam-modal-backdrop" role="presentation" onClick={onClose}>
      ...
      >
        ...
        <div className="iam-modal__body">
          ...
          <p className="iam-warning">
            {" "}
            سيفقد المستخدم صلاحيات الدور{" "}
            <strong>{labelRole(assignment.role_id)}</strong> فورًا.{" "}
          </p>{" "}
          <dl className="iam-review">
            ...
            <div>
              {" "}
              <dt>الدور</dt> <dd>{labelRole(assignment.role_id)}</dd>{" "}
            </div>{" "}
          </dl>{" "}
          <label className="iam-field">
            {" "}
...
export function IamOverrideRevokeModal({
  open,
  override,
  onClose,
  onConfirm,
...
export function IamUserDrawer({
  ...
  const userSessions = (sessions || []).filter(
    (s) => s.user_id === user.user_id && !s.ended_at,
  );
    ...
    <div className="iam-drawer-backdrop" role="presentation" onClick={onClose}>
      ...
      >
        ...
        <header className="iam-drawer__hero">
          ...
          <button
            type="button"
            className="iam-drawer__close"
            onClick={onClose}
            aria-label="إغلاق"
          >
            ×
          </button>{" "}
          <IamAvatar record={user} size="lg" /> <h2>{displayName || email}</h2>{" "}
        ...
        <div className="iam-drawer__body">
          ...
          <section className="iam-drawer__section">
            ...
            <dl className="iam-meta-grid">
              {" "}
              <div>
                <dt>تاريخ أول تعيين</dt>
                <dd>{formatDateTime(firstAssignment?.granted_at)}</dd>
              </div>{" "}
              <div>
                <dt>آخر نشاط</dt>
              ...
              <div>
                <dt>آخر تعيين</dt>
                <dd>{formatDateTime(user.last_granted_at)}</dd>
              </div>{" "}
              {firstAssignment?.grant_reason ? (
                <div>
                  <dt>سبب التعيين</dt>
                  <dd>
                    <IamReasonText reason={firstAssignment.grant_reason} />
                  </dd>
                </div>
              ) : null}{" "}
            </dl>{" "}
                ...
                <div key={g.category} className="iam-perm-group">
                  ...
                  <ul className="iam-perm-list">
                    ...
                    {g.permissions.length > 8 ? (
                      <li className="iam-muted">
                        +{g.permissions.length - 8} أخرى
                      </li>
                    ) : null}{" "}
                  </ul>{" "}
                </div>
