      ...
      <div className="iam-overview-grid iam-overview-grid--wide">
        ...
        <section className="iam-panel iam-panel--health">
          ...
          <div className="iam-health-row">
            {" "}
            <IamBadge tone={allFlagsOn ? "success" : "warning"}>
              {allFlagsOn ? "سليم" : "يتطلب مراجعة"}
            </IamBadge>{" "}
          ...
          <ul className="iam-flag-list">
            ...
            {Object.entries(IAM_FLAG_LABELS).map(([key, label]) => (
              <li key={key}>
                {" "}
                <span>{label}</span>{" "}
                <IamBadge tone={flags[key] ? "success" : "muted"}>
                  {flags[key] ? "مفعلة" : "غير مفعلة"}
                </IamBadge>{" "}
              </li>
            ))}{" "}
          </ul>{" "}
          ...
          {!activeAssignments.length ? (
            ...
            <ul className="iam-timeline">
              ...
              {activeAssignments.slice(0, 5).map((a) => (
                <li key={a.id}>
                  {" "}
                  <IamUserCell record={a} /> <IamRoleBadge roleId={a.role_id} />{" "}
...
export function IamAdminUsersTab({
  ...
  const users = useMemo(
    () => buildAdminUsersFromAssignments(assignments),
    [assignments],
  );
    ...
    <div className="iam-tab-panel">
      ...
      {!filtered.length ? (
        <IamEmptyState
          ...
          description="لم يتم العثور على تعيينات نشطة."
        />
      ) : (
        <IamTableWrap>
          {" "}
          <table className="iam-table">
            ...
            <thead>
              ...
              <tr>
                {" "}
                <th>المستخدم</th> <th>البريد</th> <th>الدور الحالي</th>{" "}
                <th>الحالة</th> <th>آخر تعيين</th> <th>الإجراءات</th>{" "}
              </tr>{" "}
            </thead>{" "}
            <tbody>
              {" "}
    ...
    <div className="iam-tab-panel iam-roles">
      ...
      {selected ? (
        <section className="iam-panel">
          ...
          />{" "}
          <p className="iam-readonly-note">
            عرض للقراءة — التعديل عبر API الإداري فقط.
          </p>{" "}
        </section>
      ) : (
        <IamEmptyState
          title="اختر دورًا"
          description="اضغط على بطاقة دور لعرض صلاحياته."
          icon="🛡"
        />
      )}{" "}
    </div>
    ...
    <div className="iam-tab-panel">
      ...
      {!filtered.length ? (
        <IamEmptyState
          ...
          description="لم يتم العثور على تعيينات مطابقة."
        />
      ) : (
        <IamTableWrap>
          {" "}
          <table className="iam-table">
            ...
            <thead>
              ...
              <tr>
                ...
                <th>تم التعيين بواسطة</th> <th>التاريخ</th> <th>السبب</th>{" "}
                <th />{" "}
              </tr>{" "}
            </thead>{" "}
            <tbody>
                ...
                <tr key={a.id}>
                  ...
                  <td>
                    ...
                    {canRevoke && !a.revoked_at ? (
                      <button
                        type="button"
                        className="iam-btn iam-btn--danger-text"
                        onClick={() => onRevoke(a)}
                      >
                        إلغاء التعيين
                      </button>
  ...
  return (
    <div className="iam-tab-panel">
      ...
      <div className="iam-callout" role="note">
        {" "}
        تُستخدم الاستثناءات لمنح أو منع صلاحية محددة لمستخدم دون تغيير دوره
        الأساسي. المنع يتغلب على السماح.{" "}
      </div>{" "}
      <form onSubmit={onResolveUser} className="iam-toolbar iam-form-inline">
        ...
        <input
          ...
          onChange={(e) =>
            setOverrideLookup({ email: e.target.value, userId: "" })
          }
      ...
      {userOverrides.length ? (
        <IamTableWrap>
          ...
          <table className="iam-table">
            ...
            <thead>
              ...
              <tr>
                {" "}
                <th>الصلاحية</th> <th>النوع</th> <th>السبب</th> <th>منذ</th>{" "}
                <th />{" "}
              </tr>{" "}
            </thead>{" "}
            <tbody>
              ...
              {userOverrides.map((row) => (
                <tr key={row.id}>
                  ...
                  <td>
                    <IamBadge
                      tone={row.effect === "deny" ? "danger" : "success"}
                    >
        ...
        <IamEmptyState
          ...
          description="ابحث عن مستخدم لعرض استثناءاته."
        />
      )}{" "}
      <PermissionGate permission={IAM_PERMISSIONS.IAM_MANAGE}>
        {" "}
          ...
          <form onSubmit={onGrantOverride} className="iam-form-grid">
            ...
            <label className="iam-field">
              ...
              <input
                ...
                onChange={(e) =>
                  setOverrideForm((c) => ({ ...c, email: e.target.value }))
                }
              />
            </label>{" "}
            <label className="iam-field">
            ...
            <label className="iam-field">
              ...
              <UiSelect
                ...
                onChange={(e) =>
                  setOverrideForm((c) => ({ ...c, effect: e.target.value }))
                }
              >
            ...
            <label className="iam-field">
              ...
              <input
                ...
                onChange={(e) =>
                  setOverrideForm((c) => ({ ...c, reason: e.target.value }))
                }
              />
            </label>{" "}
...
export function IamSessionsTab() {
  const feed = useIamListFeed("/api/iam/sessions", {
    legacyKey: "sessions",
    defaultLimit: 50,
  });
  useEffect(() => {
...
export function IamSecurityTab() {
  const feed = useIamListFeed("/api/iam/security-events", {
    legacyKey: "events",
    defaultLimit: 50,
  });
...
export function IamAuditTab() {
  const feed = useIamListFeed("/api/iam/audit", {
    legacyKey: "logs",
    defaultLimit: 50,
  });
  const handleFiltersApply = useCallback(
