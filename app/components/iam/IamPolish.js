...
const PAGE_SIZE = 15;
export function IamPermissionsViewer({
  permissions,
  roleMatrix,
  selectedRoleId,
  showTechnical = false,
  ...
  const groups = useMemo(() => {
    ...
    const rolePerms = new Set(
      (roleMatrix[selectedRoleId]?.permissions || []).map(
        (p) => p.permissionId || p.id,
      ),
    );
    return all
      .map((g) => ({
    ...
    <div className="iam-perm-viewer">
      ...
      <div className="iam-toolbar">
        ...
        <button
          ...
          onClick={() =>
            setOpenGroups(new Set(filtered.map((g) => g.category)))
          }
        >
          {" "}
          فتح الكل{" "}
      ...
      {filtered.length === 0 ? (
        <IamEmptyState
          ...
          description="جرّب تعديل البحث أو اختر دورًا آخر."
          icon="🔐"
        />
      ) : (
                ...
                {isOpen ? (
                  <ul className="iam-perm-list">
                    ...
                    {g.permissions.map((p) => (
                      <li key={p.id}>
                        {" "}
                        <span className="iam-perm-list__label">
                          ✅ {labelPermission(p.id)}
                        </span>{" "}
                        {showTechnical ? (
                          <code className="iam-tech-id">{p.id}</code>
                        ) : null}{" "}
                      </li>
                    ))}{" "}
                  </ul>
...
export function IamRoleCardPro({
  role,
  permCount,
  userCount,
  selected,
  ...
  return (
    ...
    >
      ...
      <div className="iam-role-card-pro__meta">
        {" "}
        <span>{userCount} مستخدم</span> <span>{permCount} صلاحية</span>{" "}
        <IamBadge
          tone={
            risk === "critical"
              ? "danger"
...
export function IamAuditViewer({
  ...
  const filtered = useMemo(() => {
    let rows = logs || [];
    rows = filterBySearch(rows, query, [
      "action",
      "actor_email",
      "target_type",
      "target_id",
    ...
    if (severityFilter)
      rows = rows.filter(
        (r) => String(r.severity || "").toLowerCase() === severityFilter,
      );
    return rows;
  ...
  return (
    <div className="iam-audit-viewer">
      ...
      <div className="iam-filter-bar">
        ...
        <UiSelect
          ...
          onChange={(e) => {
            ...
            applyServerFilters({ action: e.target.value || undefined });
          }}
          aria-label="فلتر الإجراء"
        >
          {" "}
        ...
        <input
          ...
          onChange={(e) => {
            ...
            applyServerFilters({ dateFrom: e.target.value || undefined });
          }}
          aria-label="من تاريخ"
        />{" "}
        <input
          ...
          onChange={(e) => {
            ...
            applyServerFilters({ dateTo: e.target.value || undefined });
          }}
        ...
        <button
          type="button"
          className="iam-btn iam-btn--ghost"
          onClick={() => exportToJson(filtered, "iam-audit.json")}
        >
          {" "}
          تصدير JSON{" "}
        </button>{" "}
        <button
          ...
          className="iam-btn iam-btn--ghost"
          onClick={() =>
            exportToCsv(filtered, "iam-audit.csv", [
              {
                key: "action",
                label: "الإجراء",
                format: (v) => labelAuditAction(v),
              },
      ...
      {!filtered.length ? (
        <IamEmptyState
          title="لا توجد سجلات"
          description="لا توجد نتائج مطابقة للفلاتر."
          icon="📜"
        />
      ) : (
        <>
          ...
          <IamTableWrap>
            ...
            <table className="iam-table">
              ...
              <thead>
                ...
                <tr>
                  {" "}
                  <th>الإجراء</th> <th>المنفّذ</th> <th>الهدف</th>{" "}
                  <th>السبب</th> <th>الوقت</th> <th />{" "}
                </tr>{" "}
              </thead>{" "}
              <tbody>
                {" "}
                {filtered.map((log) => (
                  <tr
                    key={log.id}
                    className={expandedId === log.id ? "is-expanded" : ""}
                  >
                    {" "}
          ...
          {expandedId ? (
            <details className="iam-tech-details iam-audit-detail" open>
              ...
              {detailLoading ? (
                <p className="iam-muted">جاري التحميل…</p>
              ) : null}{" "}
              <pre>
                {JSON.stringify(
                  detailRow || filtered.find((l) => l.id === expandedId) || {},
                  null,
                  2,
                )}
              </pre>{" "}
  ...
  return (
    <div className="iam-security-timeline">
      ...
      <div className="iam-filter-bar">
        ...
        <UiSelect
          ...
          onChange={(e) => {
            ...
            applyServerFilters({ severity: e.target.value || undefined });
          }}
          aria-label="فلتر الخطورة"
        >
          {" "}
        ...
        <input
          ...
          onChange={(e) => {
            ...
            applyServerFilters({ eventType: e.target.value || undefined });
          }}
      ...
      {!filtered.length ? (
        <IamEmptyState
          title="لا توجد أحداث"
          description="لم تُسجَّل أحداث أمنية مطابقة."
          icon="🔒"
        />
      ) : (
        <>
          ...
          <ol className="iam-timeline-pro">
            ...
            {filtered.map((e) => (
              <li
                key={e.id}
                className={`iam-timeline-pro__item iam-timeline-pro__item--${e.severity || "info"}`}
              >
                {" "}
  ...
  if (!active.length) {
    return (
      <IamEmptyState
        ...
        description="ستظهر الجلسات الإدارية النشطة هنا."
        icon="🖥"
      />
    );
  }
      ...
      <IamTableWrap>
        ...
        <table className="iam-table iam-table--sessions">
          ...
          <thead>
            ...
            <tr>
              {" "}
              <th>المستخدم</th> <th>الجهاز / المتصفح</th> <th>المنصة</th>{" "}
              <th>IP</th> <th>بداية الجلسة</th> <th>آخر نشاط</th> <th>المدة</th>{" "}
              <th>الحالة</th>{" "}
            </tr>{" "}
          </thead>{" "}
          <tbody>
    ...
    <section className="iam-panel">
      ...
      <ul className="iam-distribution">
        ...
        {counts.map(({ roleId, count }) => (
          <li key={roleId}>
            {" "}
            <span className="iam-distribution__label">
              {labelRole(roleId)}
            </span>{" "}
