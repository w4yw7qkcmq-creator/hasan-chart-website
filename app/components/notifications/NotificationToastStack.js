...
export function NotificationToastStack() {
  ...
  const visualType = isGrouped
    ? "general"
    : notification?.visualType || "general";
  const targetHref = isGrouped
    ? "/notifications"
    : notification?.href ||
      getNotificationHref(notification?.type) ||
      "/notifications";
  ...
  return (
    <div className="notificationToastStack">
      ...
      <div
        key={id}
        className={`notificationToast notificationToast--${visualType} ${exiting ? "notificationToast--exit" : "notificationToast--enter"} pointer-events-auto relative overflow-hidden rounded-[24px] border p-4 ui-text-strong backdrop-blur-2xl`}
        aria-live="polite"
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-3 flex items-center gap-2">
              ...
              <button
                ...
                className="notificationToast__action rounded-xl px-4 py-2 text-xs font-black ui-text-strong"
              >
                {" "}
                عرض{" "}
              </button>{" "}
