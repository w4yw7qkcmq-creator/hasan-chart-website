...
const VARIANTS = {
  dark: "text-cyan-300 underline decoration-cyan-400/50 underline-offset-4 transition hover:text-cyan-200",
  light: "font-bold text-cyan-700 underline decoration-cyan-300 underline-offset-4 transition hover:text-cyan-900",
};
...
export default function LinkifiedText({
  ...
  for (const { phrase, href } of INTERNAL_LINK_PHRASES) {
    ...
    for (const part of parts) {
      ...
      nextParts.push(
        <Link key={`${href}-${matchIndex}-${linksUsed}`} href={href} className={linkClassName}>
          {phrase}
        </Link>
      );
