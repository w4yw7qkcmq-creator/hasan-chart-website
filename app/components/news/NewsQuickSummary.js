...
export default function NewsQuickSummary({
  ...
  return (
    <section
      className="mb-6 rounded-[1.5rem] border border-slate-200 bg-slate-50/90 p-5 shadow-sm"
      dir="rtl"
      aria-label="ملخص سريع"
    >
      <h2 className="mb-4 text-lg font-black text-slate-950">ملخص سريع</h2>
      ...
      <dl className="grid gap-3 sm:grid-cols-2">
        {items.map((item) => (
          ...
          >
            <dt className="text-xs font-black text-cyan-700">{item.label}</dt>
            <dd className="mt-1 text-sm font-bold leading-7 text-slate-800">
