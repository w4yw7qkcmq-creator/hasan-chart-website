...
export function AuthProvider({ children }) {
  ...
  const isCurrentBootstrap = useCallback((requestId) => {
    return (
      mountedRef.current &&
      isBootstrapRequestCurrent(bootstrapRequestRef.current, requestId)
    );
  }, []);
  const setBootstrapStatus = useCallback(
  ...
  const applyAuthenticatedUser = useCallback(
    (authUser, { enrichProfile = true, serverSessionUser = null } = {}) => {
      ...
      if (enrichProfile) {
        ...
        const enrichSource = authUser?.email
          ? authUser
          : {
              id: minimalUser.id,
  ...
  const markAuthErrorForRequest = useCallback((requestId) => {
    if (
      !shouldMarkBootstrapError({
  ...
  const finishAuthBootstrap = useCallback(
    (requestId) => {
      if (!isCurrentBootstrap(requestId)) return;
  ...
  const runAuthBootstrap = useCallback(
    async ({ requestId, signal }) => {
      for (
        let attempt = 1;
        attempt <= AUTH_BOOTSTRAP_MAX_ATTEMPTS;
        attempt += 1
      ) {
        ...
        try {
          ...
          if (
            restored.outcome === "authenticated" &&
            restored.sessionUser?.email
          ) {
  ...
  const refreshIam = useCallback(async () => {
    ...
    try {
      const response = await fetch("/api/iam/me", {
      ...
      const next = {
        ...
        primaryRoleId:
          payload.primaryRoleId || payload.iam?.primaryRoleId || null,
        primaryRoleLabel:
          payload.primaryRoleLabel || payload.iam?.primaryRoleLabel || null,
        isSuperAdmin: Boolean(
          payload.isSuperAdmin ?? payload.iam?.isSuperAdmin,
        ),
  ...
  const value = useMemo(
    () => ({
      ...
      refreshIam,
      can: (permission) => {
        if ((iamUiEnabled || iamApiEnabled) && iam?.can)
          return iam.can(permission);
        return isAdminLegacy;
      },
      acknowledgeSignIn,
      logout,
