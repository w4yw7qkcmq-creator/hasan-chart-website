...
function cleanNewsSitemapTitle(item) {
  ...
  return content
    ? content.replace(/^عاجل\s*[:：-]?\s*/i, "").slice(0, 150)
    : "خبر اقتصادي عاجل";
}
...
export async function GET() {
  ...
  const newsUrls = (news || [])
    .map((item) => {
      ...
      const lastModified =
        toSitemapIsoDate(item.created_at) || createdAt.toISOString();
      ...
      const googleNewsBlock = isRecentForGoogleNews(item.created_at)
        ? ` <news:news> <news:publication> <news:name>${escapeXml(SITE_ORGANIZATION_NAME)}</news:name> <news:language>ar</news:language> </news:publication> <news:publication_date>${escapeXml(publicationDate)}</news:publication_date> <news:title>${escapeXml(headline)}</news:title> </news:news>`
        : "";
      return ` <url> <loc>${escapeXml(buildAbsoluteUrl(path))}</loc> <lastmod>${escapeXml(lastModified)}</lastmod> <changefreq>hourly</changefreq> <priority>0.9</priority>${googleNewsBlock} </url>`;
    })
    .filter(Boolean)
    .join("");
