...
export default function News() {
  ...
  const fetchNewsPage = useCallback(async ({ limit, offset, signal }) => {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        payload?.error || "تعذر تحميل الأخبار من قاعدة البيانات.",
      );
    }
    ...
    async ({ silent = false, force = false } = {}) => {
      ...
      const requestPromise = (async () => {
        try {
          ...
          setErrorMessage(
            error?.message || "حدث خطأ غير متوقع أثناء تحميل الأخبار.",
          );
          setNews([]);
        } finally {
  ...
  const displayNews = useMemo(() => {
    return categoryFilteredNews
      ...
      .filter((item) =>
        matchesNewsSearch(item, debouncedSearchQuery, extractArabicTitle),
      );
    ...
    <main className="news-page">
      ...
      <div className="news-page__inner">
        ...
        <header className="news-page-hero">
          {" "}
          <span className="news-page-hero__eyebrow">
            تغطية مالية مباشرة
          </span>{" "}
          <h1 className="news-page-hero__title">الأخبار الاقتصادية العاجلة</h1>{" "}
          <p className="news-page-hero__text">
            {" "}
            تغطية يومية لأهم تحركات الأسواق العالمية، العملات الرقمية، الفوركس،
            الذهب والسلع، النفط والطاقة، والبيانات الاقتصادية المؤثرة على قرارات
            التداول.{" "}
          </p>{" "}
          <div className="news-page-hero__actions">
            ...
            {lastUpdated ? (
              <span className="news-page-hero__updated">
