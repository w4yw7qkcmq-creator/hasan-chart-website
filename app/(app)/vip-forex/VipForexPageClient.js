...
import { AuthGuestLandingGate } from "../../components/public-seo/GuestPublicLandingGate";
...
function VipForexAuthenticated() {
  ...
  const loadSignals = async ({ silent = false } = {}) => {
    ...
    try {
      const response = await fetch("/api/vip-signals?type=forex&limit=50", {
        method: "GET",
        cache: "no-store",
        credentials: "include",
      });
      ...
      if (!response.ok || !result?.success) {
        console.error("VIP Forex signals error:", result?.error || "Unknown error");
        setSignals([]);
        return;
      }
      ...
      console.error("VIP Forex signals error:", error);
      setSignals([]);
    } finally {
      ...
      <main className="min-h-screen bg-[#020617] px-4 py-20 text-white">
        <div className="mx-auto max-w-2xl rounded-[36px] border border-red-200/30 bg-white p-10 text-center text-slate-950 shadow-2xl">
          ...
          <p className="mt-4 text-lg font-bold text-slate-600">
            انتهت صلاحية الباقة الخاصة بك. قم بتجديد الاشتراك للعودة إلى توصيات VIP Forex.
          </p>
          <Link
            href="/subscriptions"
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 py-20 md:px-6">
        <div className="rounded-[36px] border border-cyan-200/70 bg-white/90 p-8 text-slate-950 shadow-[0_30px_120px_rgba(14,165,233,0.18)] backdrop-blur-3xl md:p-12">
          <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
            <div>
              <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-teal-200 bg-teal-50 px-4 py-2 text-sm font-black text-teal-700">
                💱 قسم توصيات VIP Forex
              </div>

              <h1 className="text-4xl font-black md:text-6xl">توصيات Forex الاحترافية</h1>

              <p className="mt-6 max-w-3xl text-lg font-bold leading-8 text-slate-600">
                هنا تظهر توصيات الفوركس الخاصة بالمشتركين فقط، ويتم تحديثها مباشرة عند نشر توصية جديدة من لوحة الإدارة.
              </p>
            </div>
...
export default function VipForexPageClient({ landing, initialAuthenticated = false }) {
  return (
    <AuthGuestLandingGate landing={landing} initialAuthenticated={initialAuthenticated}>
      <VipForexAuthenticated />
    </AuthGuestLandingGate>
  );
}
