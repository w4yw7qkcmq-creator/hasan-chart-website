...
const SubscriptionsAuthenticated = dynamic(
  () => import("./SubscriptionsAuthenticated"),
  {
    ssr: false,
    loading: () => (
      <main className="subscriptions-page">
        ...
        <div className="subscriptions-page__inner">
          {" "}
          <div
            className="subscriptions-state subscriptions-state--loading"
            role="status"
          >
            {" "}
            ...
            <h1 className="subscriptions-state__title">
              جاري تحميل الاشتراكات…
            </h1>{" "}
            <p className="subscriptions-state__text">
              نحضّر باقات الاشتراك لحسابك.
            </p>{" "}
          </div>{" "}
        </div>{" "}
      </main>
...
export default function SubscriptionsPage() {
  const { user, sessionPending, isAuthenticated, shouldShowLogin } =
    useRequireAuth();
  if (sessionPending) {
    return (
      <main className="subscriptions-page">
