      ...
      <div className="subscriptions-hub__grid">
        ...
        {SUBSCRIPTIONS_HUB_LINKS.map((link) => (
          <Link
            key={link.href}
            href={link.href}
            className="subscriptions-hub__link"
          >
            {" "}
            {link.label}{" "}
          </Link>
    ...
    const loadCurrentSubscription = async () => {
      try {
        ...
        setCurrentSubscription(
          result.current_subscription || result.plans?.[0] || null,
        );
      } catch {
        setCurrentSubscription(null);
      } finally {
  ...
  const submitSubscriptionRequest = async () => {
    ...
    try {
      ...
      const authorizeResponse = await fetch(
        "/api/subscription-request/upload-authorize",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
      ...
      const finalizeResponse = await fetch(
        "/api/subscription-request/finalize",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
      ...
      if (!finalizeResponse.ok || !finalizeResult?.success) {
        ...
        if (
          finalizeResult?.errorCode === "MIME_MISMATCH" ||
          finalizeResult?.errorCode === "INVALID_UPLOAD_MIME"
        ) {
          throw new Error(
            "صيغة إثبات الدفع غير مدعومة أو لا تطابق محتوى الملف.",
          );
        }
      ...
      setNotification({
        ...
        message:
          "تم استلام طلب الاشتراك وإثبات الدفع، وسيقوم الدعم بمراجعته وتفعيل الباقة.",
      });
    } catch (error) {
    ...
    <main className="subscriptions-page">
        ...
        <div className="subscriptions-modal-overlay">
          ...
          <div className="subscriptions-modal">
            ...
            <div className="subscriptions-modal__head">
              ...
              <div>
                {" "}
                <p className="subscriptions-modal__eyebrow">
                  طلب اشتراك جديد
                </p>{" "}
                <h3 className="subscriptions-modal__title">
                  إتمام طلب الاشتراك
                </h3>{" "}
                <p className="subscriptions-modal__text">
                  {" "}
                  أرسل بيانات الدفع ليتمكن الدعم من مراجعة الطلب وتفعيل
                  الباقة.{" "}
                </p>{" "}
              </div>{" "}
              <button
            ...
            <div className="subscriptions-modal__body">
              ...
              <div className="subscriptions-modal__plan">
                {" "}
                <p className="subscriptions-modal__plan-label">
                  الباقة المختارة
                </p>{" "}
              ...
              <div className="subscriptions-field">
                ...
                <div
                  className="subscriptions-network-options"
                  role="radiogroup"
                  aria-label="نوع الشبكة"
                >
                  {" "}
                  {PAYMENT_NETWORK_OPTIONS.map((option) => {
                    ...
                    return (
                      <label
                        key={option.value}
                        className={`subscriptions-network-option ${selected ? "subscriptions-network-option--selected" : ""}`}
                      >
                        {" "}
                        <input
                          type="radio"
                ...
                <div className="subscriptions-network-address">
                  ...
                  <div className="subscriptions-network-address__head">
                    ...
                    <p className="subscriptions-network-address__label">
                      {" "}
                      عنوان التحويل —{" "}
                      {getPaymentNetworkLabel(paymentNetwork)}{" "}
                    </p>{" "}
                    <button
                      type="button"
                  ...
                  {addressCopied ? (
                    <p
                      className="subscriptions-network-address__copied"
                      role="status"
                    >
                      {" "}
                      تم نسخ العنوان{" "}
                  ...
                  <p className="subscriptions-network-address__warning">
                    {" "}
                    تأكد من إرسال المبلغ عبر الشبكة المختارة فقط، لأن التحويل
                    عبر شبكة مختلفة قد يؤدي إلى فقدان الأموال.{" "}
                  </p>{" "}
                </div>
              ) : (
              ...
              <label className="subscriptions-field">
                ...
                />{" "}
                {paymentProofFile ? (
                  <div className="subscriptions-proof-ok">
      ...
      <div className="subscriptions-page__inner">
        ...
        <header className="subscriptions-hero">
          {" "}
          <span className="subscriptions-hero__eyebrow">
            PREMIUM MEMBERSHIPS
          </span>{" "}
          <h1 className="subscriptions-hero__title">
            اشتراكات التوصيات Spot & Futures
          </h1>{" "}
          <p className="subscriptions-hero__text">
            {" "}
            اختر باقة السبوت أو الفيوتشر المناسبة لأسلوب تداولك، مع توصيات واضحة
            ومتابعة احترافية من فريق HasaN CharT World.{" "}
          </p>{" "}
        </header>{" "}
        <section
          className="subscriptions-status-card"
          aria-label="اشتراكك الحالي"
        >
          {" "}
          <div className="subscriptions-status-card__head">
            ...
            <div>
              {" "}
              <span className="subscriptions-status-card__eyebrow">
                CURRENT MEMBERSHIP
              </span>{" "}
              <h2 className="subscriptions-status-card__title">
                اشتراكك الحالي
              </h2>{" "}
              <p className="subscriptions-status-card__text">
                {" "}
                تابع حالة باقتك وتاريخ البداية والانتهاء وعدد الأيام
                المتبقية.{" "}
              </p>{" "}
            </div>{" "}
            {currentSubscription ? (
          ...
          {subscriptionLoading ? (
            ...
            <div className="subscriptions-status-grid">
              ...
              <div className="subscriptions-status-item">
                ...
                <p>تاريخ الانتهاء</p>{" "}
                <strong>
                  {formatSubscriptionDate(currentSubscription.expires_at)}
                </strong>{" "}
              </div>{" "}
            ...
            <div className="subscriptions-status-empty">
              {" "}
              <p className="subscriptions-status-empty__title">
                لا يوجد اشتراك مفعل حالياً
              </p>{" "}
            ...
            <div key={category} className="subscriptions-plans-group">
              ...
              <div className="subscriptions-plans-grid">
                ...
                {SUBSCRIPTION_PLANS.filter(
                  ...
                  <article
                    key={plan.name}
                    className={`subscriptions-plan-card ${plan.featured ? "subscriptions-plan-card--featured" : ""}`}
                  >
                    {" "}
                    <div className="subscriptions-plan-card__head">
                      ...
                      <div>
                        {" "}
                        <span className="subscriptions-plan-card__badge">
                          {plan.badge}
                        </span>{" "}
                        <h3>{plan.name}</h3>{" "}
                      </div>{" "}
                      <span
                        className="subscriptions-plan-card__icon"
                        aria-hidden="true"
                      >
                        {" "}
                        {plan.icon}{" "}
        ...
        <section className="subscriptions-support">
          ...
          <div>
            {" "}
            <span className="subscriptions-support__eyebrow">
              SUPPORT & CONSULTING
            </span>{" "}
            <h2>تحتاج خطة خاصة أو استشارة؟</h2>{" "}
            <p>
              {" "}
              تواصل مع فريق HasaN CharT للحصول على اشتراك مخصص، إدارة حسابات، أو
              خدمات تداول خاصة.{" "}
            </p>{" "}
          </div>{" "}
