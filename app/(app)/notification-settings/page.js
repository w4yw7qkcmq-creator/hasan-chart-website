  ...
  return (
    <section className="notificationsPage__panel space-y-4 rounded-[28px] border p-5 backdrop-blur-2xl sm:p-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div>
          {" "}
          <h2 className="ui-public-seo-title text-lg">{title}</h2>{" "}
          {description ? (
            <p className="mt-1 text-xs font-bold admin-text-subtle">
              {description}
            </p>
          ) : null}{" "}
        </div>{" "}
        {badge ? (
          <span className="rounded-full border admin-panel-border admin-panel px-3 py-1 text-[11px] font-black ui-public-seo-link-chip">
            {" "}
            {badge}{" "}
          </span>
        ) : null}{" "}
...
function ToggleRow({
  ...
  return (
    <label
      className={`flex items-start justify-between gap-4 rounded-2xl border admin-panel-border ui-glass-04 ${compact ? "p-3" : "p-4"} ${disabled ? "opacity-60" : ""}`}
    >
      {" "}
      <span>
        {" "}
        <span className="block text-sm font-black admin-text">
          {label}
        </span>{" "}
        {description ? (
          <span className="mt-1 block text-xs font-bold admin-text-subtle">
            {description}
          </span>
        ) : null}{" "}
      </span>{" "}
...
function ChannelSettingsCard({
  ...
  return (
    <article className="rounded-[24px] border admin-panel-border admin-panel p-4 sm:p-5">
      {" "}
      <div className="mb-4 flex items-start gap-3">
        {" "}
        <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border admin-panel-border admin-panel text-xl">
          {" "}
          {channel.icon}{" "}
        </span>{" "}
        <div className="min-w-0 flex-1">
          {" "}
          <h3 className="text-base font-black admin-text">
            {channel.label}
          </h3>{" "}
          <p className="mt-1 text-xs font-bold leading-relaxed admin-text-subtle">
            {" "}
            {channel.description}{" "}
          </p>{" "}
          <p className="mt-1 text-[10px] font-bold uppercase tracking-wide admin-text-subtle">
            {" "}
            {channel.key}{" "}
          </p>{" "}
        </div>{" "}
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        ...
        <ToggleRow
          ...
          onChange={(checked) =>
            onUpdateChannel(channel.key, { enabled: checked })
          }
        />{" "}
        <ToggleRow
          compact
          ...
          disabled={channelDisabled}
          onChange={(checked) =>
            onUpdateChannel(channel.key, { push_enabled: checked })
          }
        />{" "}
        <ToggleRow
          compact
          ...
          onChange={(checked) =>
            onUpdateSound(channel.key, { enabled: checked })
          }
        />{" "}
        <ToggleRow
          compact
          ...
          onChange={(checked) =>
            onUpdateChannel(channel.key, { email_enabled: checked })
          }
        />{" "}
      </div>{" "}
...
export default function NotificationSettingsPage() {
  ...
  const updateChannelPreference = useCallback((key, patch) => {
    setSettings((current) => ({
      ...
      channel_preferences: {
        ...
        [key]: { ...(current.channel_preferences?.[key] || {}), ...patch },
      },
    }));
  }, []);
  const updateSoundPreference = useCallback((key, patch) => {
    setSettings((current) => ({
      ...
      sound_preferences: {
        ...
        [key]: { ...(current.sound_preferences?.[key] || {}), ...patch },
      },
    }));
  }, []);
  const applyToneToAllKeys = useCallback((toneId) => {
    setSettings((current) => {
      ...
      return { ...current, sound_preferences: nextPreferences };
    });
  }, []);
  ...
  if (!authResolved) {
    return (
      <main className="notificationsPage relative min-h-[calc(100vh-120px)] overflow-hidden rounded-[34px] border p-6">
        {" "}
        <div className="relative z-10 mx-auto max-w-4xl text-center text-sm font-bold admin-text-muted">
          {" "}
    ...
    <main className="notificationsPage relative min-h-[calc(100vh-120px)] overflow-hidden rounded-[34px] border p-4 shadow-[0_25px_90px_rgba(0,102,255,0.16)] sm:p-6">
      ...
      <div className="relative z-10 mx-auto max-w-4xl space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div>
            {" "}
            <p className="text-xs font-black uppercase tracking-[0.2em] admin-text-muted/70">
              {" "}
              HasaN CharT World{" "}
            </p>{" "}
            ...
            <p className="notificationsPage__subtitle mt-2 max-w-2xl text-sm font-bold">
              {" "}
              تحكم كامل في الإشعارات داخل الموقع، Web Push، الصوت، والبريد — لكل
              نوع على حدة.{" "}
            </p>{" "}
          </div>{" "}
          <Link
        ...
        {loading ? (
          <div className="notificationsPage__panel space-y-6 rounded-[28px] border p-6 backdrop-blur-2xl sm:p-8">
            ...
            <div className="flex items-center justify-center gap-3 py-4">
              {" "}
              <div className="h-5 w-5 animate-spin rounded-full border-2 admin-panel-border border-t-cyan-300" />{" "}
            ...
            <div className="space-y-4" aria-hidden="true">
              ...
              {[1, 2, 3].map((section) => (
                <div
                  key={section}
                  className="rounded-2xl border admin-panel-border ui-glass-03 p-5"
                >
                  {" "}
                  <div className="mb-4 h-5 w-40 animate-pulse rounded ui-glass-10" />{" "}
                  <div className="space-y-3">
                    {" "}
                    <div className="h-14 animate-pulse rounded-xl ui-glass-06" />{" "}
                    <div className="h-14 animate-pulse rounded-xl ui-glass-06" />{" "}
                  </div>{" "}
                </div>
              ))}{" "}
          ...
          <div className="notificationsPage__panel rounded-[28px] border p-10 text-center backdrop-blur-2xl">
            {" "}
            <p className="ui-public-seo-title text-lg">يجب تسجيل الدخول</p>{" "}
            <p className="mt-2 text-sm font-bold admin-text-muted">
              {" "}
          ...
          <>
            ...
            >
              ...
              <div className="space-y-3">
                ...
                <ToggleRow
                  ...
                  onChange={(checked) =>
                    updateSettings({ email_copy_enabled: checked })
                  }
                />{" "}
              </div>{" "}
            </SectionCard>{" "}
            ...
            >
              ...
              <div className="space-y-3">
                ...
                />{" "}
                <div className="rounded-2xl border admin-panel-border ui-glass-04 p-4">
                  {" "}
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    {" "}
                    <p className="text-sm font-black admin-text">
                      مستوى الصوت
                    </p>{" "}
                    <span className="rounded-lg admin-panel px-2 py-1 text-xs font-black ui-public-seo-link-chip">
                      {" "}
                      {volumePercent}%{" "}
                    </span>{" "}
                  </div>{" "}
                  <input
                    ...
                    onChange={(event) =>
                      updateSettings({
                        sound_volume: Number(event.target.value) / 100,
                      })
                    }
                ...
                <div className="rounded-2xl border admin-panel-border ui-glass-04 p-4">
                  {" "}
                  <label
                    className="block text-sm font-black admin-text"
                    htmlFor="notification-tone"
                  >
                    {" "}
                    نغمة التنبيه{" "}
                  </label>{" "}
                  <UiSelect
                    ...
                    className="mt-3 w-full rounded-xl border admin-panel-border bg-slate-950/70 px-3 py-2.5 text-sm font-bold admin-text"
                  >
                    {" "}
            ...
            >
              ...
              <div className="space-y-3">
                ...
                <ToggleRow
                  ...
                  disabled={saving || resetting || !masterEnabled}
                  onChange={(checked) =>
                    updateSettings({ dnd_enabled: checked })
                  }
                />{" "}
                <div className="grid gap-3 sm:grid-cols-2">
                  {" "}
                  <label className="rounded-2xl border admin-panel-border ui-glass-04 p-4">
                    {" "}
                    <span className="block text-sm font-black admin-text">
                      وقت البداية
                    </span>{" "}
                    <input
                      ...
                      onChange={(event) =>
                        updateSettings({ dnd_start_time: event.target.value })
                      }
                      className="mt-2 w-full rounded-xl border admin-panel-border bg-slate-950/70 px-3 py-2 text-sm font-bold admin-text"
                    />{" "}
                  </label>{" "}
                  <label className="rounded-2xl border admin-panel-border ui-glass-04 p-4">
                    {" "}
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        updateSettings({ dnd_end_time: event.target.value })
                      }
                      className="mt-2 w-full rounded-xl border admin-panel-border bg-slate-950/70 px-3 py-2 text-sm font-bold admin-text"
                    />{" "}
                  </label>{" "}
