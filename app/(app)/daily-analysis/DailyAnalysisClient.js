...
const DailyAnalysisAdminForm = dynamic(
  () => import("./DailyAnalysisAdminForm"),
  { ssr: false },
);
...
const DIRECTION_LABELS = Object.fromEntries(
  DIRECTION_OPTIONS.map((item) => [item.value, item.label]),
);
  ...
  return (
    <div className="daily-analysis-grid" aria-hidden="true">
      ...
      {Array.from({ length: 6 }).map((_, index) => (
        <div
          key={index}
          className="daily-analysis-card daily-analysis-card--skeleton"
        >
          {" "}
...
function TrendingMarketsSection() {
  return (
    <section
      className="daily-analysis-trending"
      aria-label="الأسواق الأكثر متابعة"
    >
      {" "}
      <div className="daily-analysis-trending__head">
        {" "}
        <h2 className="daily-analysis-trending__title">
          الأسواق الأكثر متابعة
        </h2>{" "}
      ...
      <div className="daily-analysis-trending__grid">
        ...
        {TRENDING_MARKET_LINKS.map((market) => (
          <Link
            key={market.href}
            href={market.href}
            className="daily-analysis-trending__card"
          >
            {" "}
            <span className="daily-analysis-trending__symbol">
              {market.symbol}
            </span>{" "}
            <span className="daily-analysis-trending__label">
              {market.label}
            </span>{" "}
          </Link>
        ))}{" "}
...
function RequestAnalysisSection() {
  return (
    <section className="daily-analysis-request" aria-label="اطلب تحليلك الخاص">
      ...
      <div>
        {" "}
        <h2 className="daily-analysis-request__title">
          اطلب تحليلك الخاص
        </h2>{" "}
        <p className="daily-analysis-request__text">
          {" "}
          احصل على رؤية مخصصة لسوقك من فريق HasaN CharT World عندما تحتاج قراراً
          أوضح.{" "}
        </p>{" "}
      </div>{" "}
    ...
    <nav className="daily-analysis-hub-links" aria-label="روابط داخلية">
      ...
      {DAILY_ANALYSIS_HUB_LINKS.map((link) => (
        <Link
          key={link.href}
          href={link.href}
          className="daily-analysis-hub-link"
        >
          {" "}
          {link.label}{" "}
        </Link>
  ...
  return (
    <article id={anchorId} className="daily-analysis-card">
      ...
      <div className="daily-analysis-card__head">
        ...
        <time
          className="daily-analysis-card__date"
...
function AnalysisEmptyState({ selectedFilter, onResetFilter }) {
  return (
    <div className="daily-analysis-state">
      ...
      <h2 className="daily-analysis-state__title">
        لا توجد تحليلات مطابقة حالياً
      </h2>{" "}
      ...
      <div className="daily-analysis-state__actions">
        ...
        {selectedFilter !== "all" ? (
          <button
            ...
            className="daily-analysis-state__action"
          >
            {" "}
            عرض كل التحليلات{" "}
          </button>
        ...
        <Link
          ...
          className="daily-analysis-state__action daily-analysis-state__action--link"
        >
          {" "}
          طلب تحليل{" "}
        </Link>{" "}
...
export default function DailyAnalysisClient() {
  ...
  const filteredAnalyses = useMemo(() => {
    return analyses.filter((item) =>
      matchesDailyAnalysisFilter(item, selectedFilter),
    );
    ...
    <main className="daily-analysis-page">
      ...
      <div className="daily-analysis-page__inner">
        ...
        <header className="daily-analysis-hero">
          {" "}
          <span className="daily-analysis-hero__eyebrow">
            تحليلات HasaN CharT
          </span>{" "}
          <h1 className="daily-analysis-hero__title">التحليلات اليومية</h1>{" "}
          <p className="daily-analysis-hero__text">
            {" "}
            آخر تحليلات الأسواق من فريق HasaN CharT World — رؤية يومية واضحة
            للعملات الرقمية، الفوركس، الذهب والسلع، والأسهم والمؤشرات.{" "}
          </p>{" "}
          <div className="daily-analysis-hero__actions">
            ...
            <Link
              href="/subscriptions"
              className="daily-analysis-hero__cta daily-analysis-hero__cta--secondary"
            >
              {" "}
              الاشتراكات{" "}
            </Link>{" "}
        ...
        {loading ? (
          ...
          <div
            className="daily-analysis-state daily-analysis-state--error"
            role="alert"
          >
            {" "}
            <span className="daily-analysis-state__icon" aria-hidden="true">
