...
const normalizeAdminStatusValue = (status) =>
  String(status || "")
    .trim()
    .toLowerCase();
...
export const getAdminStatusLabel = (status) => {
  ...
  return getAdminStatusKey(status) === "reviewed"
    ? "تمت المراجعة"
    : "بانتظار المراجعة";
};
...
export const countAdminStatusFilter = (
  list,
  filterKey,
  getStatus = (item) => item.status,
) =>
  list.filter((item) => matchesAdminStatusFilter(getStatus(item), filterKey))
    .length;
export const getSimpleStatusSelectValue = (status) => getAdminStatusKey(status);
const normalizeAdminSearch = (value) =>
  String(value || "")
    .trim()
    .toLowerCase();
  ...
  rejectionDetails: item.rejection_details
    ? {
        ...
        notificationCreated: Boolean(
          item.rejection_details.notificationCreated,
        ),
