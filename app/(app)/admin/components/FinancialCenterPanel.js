  ...
  return (
    <div className="admin-premium-skeleton">
      ...
      {Array.from({ length: rows }).map((_, index) => (
        <div
          key={index}
          className="admin-premium-skeleton__row animate-pulse"
        />
      ))}{" "}
    </div>
  );
      ...
      >
        ...
        <defs>
          ...
          <linearGradient id="adminRevenueLine" x1="0" y1="0" x2="1" y2="0">
            {" "}
            <stop offset="0%" stopColor="var(--ui-chart-series-2)" />{" "}
            <stop offset="100%" stopColor="var(--ui-chart-series-1)" />{" "}
          </linearGradient>{" "}
        </defs>{" "}
        ...
        {coordinates.map(({ x, y, point }) => (
          <circle
            ...
            cy={y}
            r="4"
            className="admin-financial-line-chart__dot"
          />
        ))}{" "}
      </svg>{" "}
...
function ServiceRevenueBars({ revenueByService = {} }) {
  ...
  const maxTotal = Math.max(
    ...entries.map(([, totals]) => sumCurrencyTotals(totals)),
    1,
  );
  return (
    <div className="admin-financial-service-bars admin-financial-service-bars--premium">
      ...
      {entries.map(([service, totals], index) => {
        ...
        return (
          <div
            key={service}
            className="admin-financial-service-bars__row admin-animate-in"
            style={{ animationDelay: `${index * 40}ms` }}
          >
            {" "}
            <div className="admin-financial-service-bars__meta">
              {" "}
              <span className="admin-financial-service-bars__label">
                {service}
              </span>{" "}
            ...
            <div className="admin-financial-service-bars__track">
              {" "}
              <span
                className="admin-financial-service-bars__fill"
                style={{ width }}
              />{" "}
            </div>{" "}
          </div>
  ...
  return (
    <article className="admin-financial-op-card">
      ...
      <div className="admin-financial-op-card__body">
        ...
        {subtitle ? (
          <p className="admin-financial-op-card__subtitle">{subtitle}</p>
        ) : null}{" "}
        {meta ? (
          <p className="admin-financial-op-card__meta">{meta}</p>
        ) : null}{" "}
      </div>{" "}
      <div className="admin-financial-op-card__aside">
        {" "}
        {badge ? (
          <span className="admin-financial-op-card__badge">{badge}</span>
        ) : null}{" "}
...
function RevenueDonutChart({ revenueByService = {} }) {
  const entries = useMemo(() => {
    return Object.entries(revenueByService || {})
      .map(([service, totals]) => ({
        service,
        total: sumCurrencyTotals(totals),
      }))
  ...
  if (entries.length === 0) {
    return (
      <ChartEmptyState
        icon="🍩"
        title="لا يوجد توزيع إيرادات"
        desc="سيظهر الرسم عند توفر بيانات الخدمات."
      />
    );
  }
  const total = entries.reduce((sum, entry) => sum + entry.total, 0);
  const colors = [
    "var(--ui-chart-series-2)",
    "var(--ui-chart-series-1)",
    "var(--ui-chart-neutral-mid)",
    "var(--ui-warning)",
    "var(--ui-chart-buy)",
    "var(--ui-negative)",
  ];
  ...
  return (
    <div className="admin-financial-donut">
      ...
      <svg
        ...
        className="admin-financial-donut__svg"
        role="img"
        aria-label="توزيع الإيرادات"
      >
        {" "}
        {slices.map((slice) => (
          <path
            ...
            d={slice.path}
            fill={slice.color}
            className="admin-financial-donut__slice"
          />
        ))}{" "}
        <circle
          cx={cx}
          cy={cy}
          r={30}
          className="admin-financial-donut__hole"
        />{" "}
      </svg>{" "}
      <ul className="admin-financial-donut__legend">
        ...
        {slices.map((slice) => (
          <li key={slice.service}>
            {" "}
            <span
              className="admin-financial-donut__dot"
              style={{ "--ui-slice-color": slice.color }}
            />{" "}
            <span className="admin-financial-donut__label">
              {slice.service}
            </span>{" "}
  ...
  return (
    ...
    >
      ...
      <div className="admin-financial-kpi__head">
        ...
        <span
          className={`admin-financial-kpi__status ${status === "attention" ? "is-attention" : "is-stable"}`}
        >
          {" "}
          {statusLabel}{" "}
        </span>{" "}
      ...
      {description ? (
        <p className="admin-financial-kpi__description">{description}</p>
      ) : null}{" "}
      {hint ? <p className="admin-financial-kpi__hint">{hint}</p> : null}{" "}
      {onClick ? (
        <span className="admin-financial-kpi__direction" aria-hidden="true">
          → عرض
        </span>
      ) : null}{" "}
    </Tag>
...
function SubscriptionActivateConfirmModal({
  request,
  loading = false,
  apiError = "",
  onCancel,
  ...
  return (
    <div className="admin-crm-action-modal" role="presentation">
      ...
      />{" "}
      <div
        className="admin-crm-action-modal__dialog"
        role="dialog"
        aria-modal="true"
      >
        {" "}
        <h2 className="admin-heading text-xl">
          تأكيد قبول وتفعيل الاشتراك
        </h2>{" "}
        <p className="admin-crm-action-modal__description mt-3">
          {" "}
          سيتم استخدام مسار التفعيل الرسمي نفسه المستخدم في صفحة
          الاشتراكات.{" "}
        </p>{" "}
        <dl className="admin-financial-activate-summary mt-4 space-y-2 text-sm">
          {" "}
          <div>
            <dt className="admin-muted inline">المستخدم: </dt>
            <dd className="inline font-bold">
              {request.username || request.userEmail || "—"}
            </dd>
          </div>{" "}
          <div>
            <dt className="admin-muted inline">الباقة: </dt>
            <dd className="inline font-bold">{request.planName || "—"}</dd>
          </div>{" "}
          <div>
            <dt className="admin-muted inline">السعر: </dt>
            <dd className="inline font-bold">{request.price || "—"}</dd>
          </div>{" "}
          <div>
            <dt className="admin-muted inline">إثبات الدفع: </dt>
            <dd className="inline font-bold">
              {request.hasPaymentProof ? "متاح" : "غير متاح"}
            </dd>
          </div>{" "}
        </dl>{" "}
        {apiError ? (
          <p className="mt-4 font-bold admin-text-danger">{apiError}</p>
        ) : null}{" "}
        <div className="admin-crm-action-modal__actions mt-6 flex flex-wrap gap-3">
          {" "}
          <button
            type="button"
            className="admin-financial-action-button admin-financial-action-button--secondary px-4 py-2"
            disabled={loading}
            onClick={onCancel}
          >
            {" "}
          ...
          <button
            type="button"
            className="admin-financial-action-button admin-financial-action-button--primary px-4 py-2"
            disabled={loading}
            onClick={onConfirm}
          >
            {" "}
...
function PaymentReviewActions({
  ...
  const showDecisionActions =
    canActivatePaymentReviewItem(item) || canRejectPaymentReviewItem(item);
  return (
    <div className="flex flex-wrap gap-2 min-w-[220px]">
      ...
      {showDecisionActions ? (
        <>
          ...
          <button
            type="button"
            className="rounded-2xl admin-panel px-4 py-2 text-sm font-black admin-text"
            disabled={rowBusy}
...
export default function FinancialCenterPanel({ standalone = false }) {
  ...
  const selectedPeriod =
    PERIOD_OPTIONS.find((item) => item.id === overviewPeriod) ||
    PERIOD_OPTIONS[2];
  const loadOverviewBundle = useCallback(
    async (signal) => {
  ...
  const refreshPendingOverviewCount = useCallback(async (signal) => {
    const overviewResult = await fetchFinancialCenterSection(
      adminFetch,
      "overview",
      { signal },
    );
    ...
    setOverview((current) =>
      current
        ? {
            ...current,
            pendingReviews:
              overviewResult.overview.pendingReviews ?? current.pendingReviews,
          }
    ...
    async (tab, { force = false, background = false } = {}) => {
      ...
      try {
        ...
        if (tab === "subscriptions") {
          const result = await fetchFinancialCenterSection(
            adminFetch,
            "subscriptions",
            {
          ...
          setSubscriptionPagination(
            result.pagination || subscriptionPagination,
          );
        }
        ...
        if (tab === "revenue") {
          const result = await fetchFinancialCenterSection(
            adminFetch,
            "revenue",
            { signal: controller.signal, query: { period: revenuePeriod } },
          );
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const result = await fetchFinancialCenterSection(adminFetch, section, {
        query,
      });
      if (result.csvBlob)
  ...
  const applyPaymentReviewStatusUpdate = useCallback(
    (requestId, nextReviewStatus, nextRawStatus = "") => {
      ...
      setRecentPending((current) =>
        current.filter(
          (item) => String(item.requestId || item.id) !== normalizedRequestId,
        ),
      );
        ...
        current
          ? {
              ...
              pendingReviews: Math.max(
                0,
                Number(current.pendingReviews || 0) - 1,
              ),
            }
          : current,
      );
  ...
  const confirmActivatePaymentReview = async () => {
    ...
    const flowResult = await runAdminUserActionFlow({
      ...
      execute: async () =>
        postSubscriptionActivateViaDashboard(adminFetch, request),
      refresh: refreshFinancialSections,
      successMessage: "تم تفعيل الاشتراك",
      errorMessage: "تعذر تفعيل طلب الاشتراك",
      onSuccess: () => {
        applyPaymentReviewStatusUpdate(
          request.id,
          PAYMENT_REVIEW_STATUSES.CONFIRMED,
          "مفعل",
        );
        dispatchAdminSubscriptionUpdatedEvent({
  ...
  const confirmRejectPaymentReview = async ({ reasonLabel, notes }) => {
    ...
    const flowResult = await runAdminUserActionFlow({
      ...
      onSuccess: () => {
        applyPaymentReviewStatusUpdate(
          request.id,
          PAYMENT_REVIEW_STATUSES.REJECTED,
          "مرفوض",
        );
        dispatchAdminSubscriptionUpdatedEvent({
          requestId: request.id,
    ...
    return [
      {
        key: "today",
        label: "إيرادات اليوم",
        value: formatCurrencyTotals(overview.recognizedRevenueToday),
      },
      {
        key: "month",
        label: "إيرادات هذا الشهر",
        value: formatCurrencyTotals(overview.recognizedRevenueMonth),
      },
      {
        key: "year",
        label: "إيرادات هذه السنة",
        value: formatCurrencyTotals(overview.recognizedRevenueYear),
      },
      {
        key: "total",
        label: "إجمالي الإيرادات",
        value: formatCurrencyTotals(overview.recognizedRevenueTotal),
      },
      {
        key: "active",
        label: "الاشتراكات النشطة",
        value: (overview.activeSubscriptions || 0).toLocaleString("ar"),
      },
      {
        key: "pending",
        label: "بانتظار المراجعة",
        value: (overview.pendingReviews || 0).toLocaleString("ar"),
      },
    ];
  }, [overview]);
  return (
    <section
      className={`admin-financial-dashboard ${standalone ? "admin-financial-dashboard--standalone" : ""}`}
    >
      {" "}
      <header className="admin-financial-dashboard__header admin-section">
        ...
        <div className="admin-financial-dashboard__header-main">
          ...
          <div>
            {" "}
            <p className="admin-financial-dashboard__eyebrow">
              HasaN CharT · Finance
            </p>{" "}
            <h1 className="admin-heading text-3xl">المركز المالي</h1>{" "}
            <p className="admin-financial-dashboard__subtitle">
              {" "}
              أرقام تقديرية مبنية على الاشتراكات المفعلة يدوياً — ليست سجل
              معاملات مصرفية.{" "}
            </p>{" "}
          </div>{" "}
          <div className="admin-financial-dashboard__header-actions">
            ...
            <button
              type="button"
              className="admin-financial-action-button admin-financial-action-button--secondary px-4 py-2"
              onClick={() => void loadSection(activeTab, { force: true })}
            >
              {" "}
              تحديث{" "}
            </button>{" "}
      ...
      {error ? (
        <div className="admin-section admin-financial-dashboard__error">
          {" "}
          <p className="font-black ui-signal-risk-label">{error}</p>{" "}
          <button
            type="button"
            className="admin-financial-action-button admin-financial-action-button--primary mt-4 px-5 py-3"
            onClick={() => void loadSection(activeTab, { force: true })}
          >
            {" "}
            إعادة المحاولة{" "}
      ...
      {refreshWarning ? (
        <div className="admin-section admin-financial-dashboard__warning">
          {" "}
          <p className="font-bold admin-text-warning">{refreshWarning}</p>{" "}
          <button
            type="button"
      ...
      {actionNotice ? (
        <div className="admin-section admin-financial-dashboard__notice">
          {" "}
          <p className="font-bold ui-text-positive">{actionNotice}</p>{" "}
        </div>
      ) : null}{" "}
      {activeTab === "overview" ? (
          ...
          {loading && !overview ? (
            ...
            <>
              ...
              <div className="admin-financial-kpi-grid admin-financial-kpi-grid--premium admin-animate-in">
                ...
                {overviewKpis.map((card) => (
                  <KpiCard
                    ...
                    status={
                      card.key === "pending" &&
                      Number(overview.pendingReviews || 0) > 0
                        ? "attention"
                        : "stable"
                    }
                    onClick={
              ...
              {!overview.revenueScanComplete ? (
                <p className="admin-financial-note">
                  تم حساب الإيرادات من {overview.revenueScannedRows || 0} سجل
                  نشط.
                </p>
              ) : null}{" "}
              <div className="admin-financial-dashboard__grid admin-financial-dashboard__grid--charts">
                ...
                <article className="admin-section admin-financial-panel">
                  ...
                  <p className="admin-financial-panel__subtitle">
                    Area Chart — تقديري
                  </p>{" "}
                  <RevenueLineChart
                    daily={chartReport?.daily || []}
                ...
                <article className="admin-section admin-financial-panel">
                  ...
                  <p className="admin-financial-panel__subtitle">
                    Donut Chart — حسب الخدمة
                  </p>{" "}
                  <RevenueDonutChart
                    revenueByService={overview.revenueByService}
                  />{" "}
                </article>{" "}
                ...
                <article className="admin-section admin-financial-panel">
                  ...
                  <div className="admin-financial-op-grid mt-4">
                    ...
                    {recentActive.length === 0 ? (
                      <ChartEmptyState
                        icon="💳"
                        title="لا توجد اشتراكات حديثة"
                        desc="ستظهر آخر الاشتراكات المفعلة هنا."
                      />
                    ) : (
                ...
                <article className="admin-section admin-financial-panel">
                  ...
                  <div className="admin-financial-panel__head">
                    ...
                    <button
                      ...
                      onClick={() => setActiveTab("payment-reviews")}
                    >
                      {" "}
                      عرض الكل{" "}
                    </button>{" "}
                  </div>{" "}
                  <p className="admin-financial-warning mt-2">
                  ...
                  <div className="admin-financial-op-grid mt-4">
                    ...
                    {recentPending.length === 0 ? (
                      <ChartEmptyState
                        ...
                        desc="ستظهر إثباتات الدفع قيد المراجعة هنا."
                      />
                    ) : (
        ...
        <div className="admin-section admin-financial-panel space-y-4">
          ...
          <div className="admin-user-filters-grid">
            ...
            <UiSelect
              ...
              onChange={(e) =>
                setFilters((c) => ({ ...c, service: e.target.value }))
              }
            >
              {" "}
          ...
          {loading ? (
            ...
            <p className="text-center font-bold admin-text-subtle">
            ...
            <div className="admin-table-wrap overflow-x-auto">
              ...
              <table className="admin-user-table">
                ...
                <thead>
                  ...
                  <tr>
                    {" "}
                    <th>المستخدم</th> <th>الخدمة</th> <th>الحالة</th>{" "}
                    <th>السعر</th> <th>البداية</th> <th>الانتهاء</th>{" "}
                    <th />{" "}
                  </tr>{" "}
                </thead>{" "}
                <tbody>
                  ...
                  {subscriptions.map((item) => (
                    <tr key={item.id}>
                      ...
                      <td>{item.service}</td> <td>{item.status}</td>{" "}
                      <td>{item.priceRaw}</td>{" "}
                      <td>
                        {item.startedAt
                      ...
                      <td>
                        {item.expiresAt
                          ? new Date(item.expiresAt).toLocaleDateString("ar")
                          : "—"}
                      </td>{" "}
                      <td>
                        {" "}
          ...
          {subscriptionPagination?.hasMore ? (
            <div className="flex justify-center pt-2">
              ...
              <button
                ...
                disabled={loading}
                onClick={() =>
                  setSubscriptionCursor(subscriptionPagination.nextCursor)
                }
              >
                {" "}
      ...
      {activeTab === "payment-reviews" ? (
        <div className="admin-section admin-financial-panel space-y-4">
          {" "}
          <p className="admin-financial-warning">
            وجود إثبات دفع لا يعني أن العملية مؤكدة.
          </p>{" "}
          ...
          {loading && paymentReviews.length === 0 ? (
            <SectionSkeleton />
            ...
            <div className="admin-table-wrap overflow-x-auto">
              ...
              <table className="admin-user-table">
                ...
                <thead>
                  ...
                  <tr>
                    {" "}
                    <th>المستخدم</th> <th>الخطة</th> <th>السعر</th>{" "}
                    <th>حالة المراجعة</th> <th>الإرسال</th>{" "}
                    <th>الإجراءات</th>{" "}
                  </tr>{" "}
                </thead>{" "}
                <tbody>
                  ...
                  {paymentReviews.map((item) => (
                    <tr key={item.id}>
                      {" "}
                      <td>{item.username || item.userEmail}</td>{" "}
                      <td>{item.plan}</td> <td>{item.priceRaw}</td>{" "}
                      <td>{formatPaymentReviewStatusLabel(item.status)}</td>{" "}
                      <td>
                        {item.submittedAt
                          ? new Date(item.submittedAt).toLocaleDateString("ar")
                          : "—"}
                      </td>{" "}
                      <td>
                        {" "}
      ...
      {activeTab === "revenue" ? (
        <div className="admin-section admin-financial-panel space-y-4">
          ...
          <div className="admin-financial-period-filters">
            {" "}
            {PERIOD_OPTIONS.filter((item) => item.id !== "today").map(
              (item) => (
                <button
                  key={item.id}
                  type="button"
              ...
              <div className="admin-financial-kpi-grid">
                ...
                <KpiCard
                  ...
                  value={formatCurrencyTotals(
                    revenueReport.recognizedRevenueToday,
                  )}
                />{" "}
                <KpiCard
                  label="الأسبوع"
                  value={formatCurrencyTotals(
                    revenueReport.recognizedRevenueWeek,
                  )}
                />{" "}
                <KpiCard
                  label="الشهر"
                  value={formatCurrencyTotals(
                    revenueReport.recognizedRevenueMonth,
                  )}
                />{" "}
                <KpiCard
                  label="السنة"
                  value={formatCurrencyTotals(
                    revenueReport.recognizedRevenueYear,
                  )}
                />{" "}
                <KpiCard
                  label="الإجمالي"
                  value={formatCurrencyTotals(
                    revenueReport.recognizedRevenueTotal,
                  )}
                />{" "}
              </div>{" "}
              <article className="admin-section admin-financial-panel">
              ...
              <div className="admin-table-wrap overflow-x-auto">
                ...
                <table className="admin-user-table">
                  ...
                  <thead>
                    ...
                    <tr>
                      {" "}
                      <th>التاريخ</th> <th>مفعّلة</th> <th>USD</th>{" "}
                      <th>USDT</th>{" "}
                    </tr>{" "}
                  </thead>{" "}
                  <tbody>
                    ...
                    {revenueReport.daily.map((row) => (
                      <tr key={row.date}>
                        {" "}
                        <td>{row.date}</td> <td>{row.activatedCount}</td>{" "}
      ...
      {activeTab === "referrals" ? (
        <article className="admin-section admin-financial-panel">
          ...
          <p className="mt-3 max-w-2xl text-sm font-bold leading-7 admin-text-subtle">
            {" "}
            عرض read-only — لإدارة الشركاء والسحوبات استخدم صفحة الشركاء.{" "}
          </p>{" "}
          <Link
            href="/admin/partners"
            className="admin-hub-card__cta mt-6 inline-flex"
          >
            {" "}
