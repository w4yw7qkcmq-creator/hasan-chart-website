...
export default function AdminUserQuickPreviewDrawer({ open, userId, onClose }) {
  ...
  const loadPreview = useCallback(async () => {
    ...
    setError("");
    try {
      const overviewResult = await fetchAdminUserSection(
        adminFetch,
        userId,
        "overview",
  ...
  return createPortal(
    <div className="admin-user-preview-overlay" role="presentation">
      ...
      <button
        type="button"
        className="admin-user-preview-overlay__backdrop"
        onClick={onClose}
        aria-label="إغلاق"
      />{" "}
      <aside
      ...
      >
        ...
        <div className="admin-user-drawer__header admin-user-drawer__header--sticky">
          ...
          <button
            type="button"
            className="admin-user-drawer__close admin-user-drawer__close--fixed"
            onClick={onClose}
            aria-label="إغلاق"
          >
            {" "}
