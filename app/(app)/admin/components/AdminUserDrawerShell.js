...
import {
  ...
  isAdminSubscriptionActiveDisplay,
  resolveAdminSubscriptionBadgeClass,
} from "../../../../lib/admin-subscription-request-remove-shared";
...
function UserAvatar({ name, avatarUrl, size = "md" }) {
  ...
  return (
    <span className={`admin-user-avatar__initials ${sizeClass}`}>
      {initials}
    </span>
  );
}
...
function SectionNotEnabledState({ message, detail }) {
  return (
    <div className="admin-user-section-state admin-user-section-state--disabled">
      ...
      <p className="admin-user-section-state__title">
        {message || ADMIN_SECTION_NOT_ENABLED_MESSAGE}
      </p>{" "}
      <p className="admin-user-section-state__detail">
...
function SectionEmptyDataState({
  icon = "📭",
  message = ADMIN_SECTION_EMPTY_MESSAGE,
  detail = "لا توجد بيانات لعرضها في هذا القسم حالياً.",
}) {
  return (
...
function renderSectionFrame(
  section,
  { sectionState, data, onRefreshSection, children, skeletonRows = 4 },
) {
    ...
    <section className="admin-user-hero">
      ...
      <div className="admin-user-hero__main">
        ...
        <div className="admin-user-hero__identity">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {" "}
            <h3 className="admin-heading text-2xl">
              {user.username || "—"}
            </h3>{" "}
            <AccountStatusBadge
              status={user.accountStatus}
              label={user.accountStatusLabel}
              icon={user.accountStatusIcon}
            />{" "}
          </div>{" "}
      ...
      <div className="admin-user-hero__meta">
        ...
        <div>
          ...
          <p className="admin-user-hero__meta-value">
            {user.telegram || "—"}
          </p>{" "}
        </div>{" "}
        <div>
      ...
      <div className="admin-user-stat-grid">
        ...
        <article className="admin-user-stat-card">
          ...
          <p className="admin-user-stat-card__value">
    ...
    actions: [
      {
        ...
        title: "تعليق الحساب",
        description:
          "سيتم تعليق وصول المستخدم إلى الخدمات المحمية وإنهاء جلساته.",
      ...
      },
      {
        ...
        tone: "neutral",
        title: "رفع التعليق",
        description: "استعادة وصول المستخدم.",
        confirmLabel: "تأكيد",
      },
      {
        action: "unban_user",
      ...
      },
      {
        ...
        description: "إعادة تفعيل حساب محذوف/موقوف.",
  ...
  },
  {
    ...
    actions: [
      ...
      },
      {
        ...
        description: "إنشاء رابط است recovery عبر Supabase Admin.",
    ...
    actions: [
      {
        ...
        tone: "danger",
        title: "حظر المستخدم",
        description: "منع تسجيل الدخول عبر Auth ban.",
      ...
      },
      {
        ...
        label: "حذف الحساب",
        tone: "danger",
        title: "حذف الحساب (Soft Delete)",
        description:
          "لن يتم حذف auth.users. سيتم منع الحساب من استخدام المنصة ويمكن استعادته لاحقًا.",
...
export default function AdminUserDrawerShell({
  ...
  const openPaymentProof = async (requestId) => {
    ...
    try {
      ...
      setProofPreview({ ...proof, planName: proof.planName || proof.plan });
    } catch {
      setProofPreview({ planName: "طلب اشتراك", proof: "", isInline: false });
    } finally {
      setProofLoadingId("");
    }
  };
    ...
    () => [
      {
        ...
        label: "+7 أيام",
        payload: { preset: "7d", days: 7 },
      },
      {
        action: "extend_subscription",
        label: "+30 يومًا",
        payload: { preset: "1m", days: 30 },
      },
      {
        action: "extend_subscription",
        label: "+90 يومًا",
        payload: { preset: "3m", days: 90 },
      },
      {
        action: "extend_subscription",
        label: "+365 يومًا",
        payload: { preset: "1y", days: 365 },
      },
    ],
    [],
  );
  const isSelfTarget =
    String(currentAdminUserId || "") === String(user?.id || user?.uid || "");
  const isPageLayout = layoutMode === "page";
  const isFullscreen = !isPageLayout && drawerViewMode === "fullscreen";
  return (
    <aside
      className={`admin-user-drawer admin-user-drawer--wide ${isPageLayout ? "admin-user-center-shell admin-user-center-shell--page" : ""} ${isFullscreen ? "admin-user-drawer--fullscreen" : ""}`}
    ...
    >
      ...
      {!isPageLayout ? (
        <div className="admin-user-drawer__header">
          ...
          <div className="admin-user-drawer__header-actions">
            {" "}
            <div
              className="admin-user-drawer__view-toggle"
              role="group"
            ...
            <button
              type="button"
              className="admin-user-drawer__close"
              onClick={onClose}
            >
              {" "}
              ✕{" "}
      ...
      <div
        className={`admin-user-drawer__tabs admin-user-drawer__tabs--scroll ${isPageLayout ? "admin-user-drawer__tabs--sticky" : ""}`}
        role="tablist"
      >
        {" "}
        {tabs.map((tab) => (
      ...
      <div
        className={`admin-user-drawer__body ${isPageLayout ? "admin-user-drawer__body--page" : ""}`}
      >
        {" "}
        {activeTab === "overview" &&
              ...
              (services?.services || []).length > 0 ? (
                <div className="admin-user-services-grid">
                  ...
                  {(services?.services || []).map((service) => (
                    <article
                      key={service.key}
                      className="admin-user-service-tile"
                    >
                      {" "}
                      <div className="admin-user-service-tile__head">
                        ...
                        <span
                          className={`admin-user-service-tile__badge ${service.isActive ? "is-active" : "is-inactive"}`}
                        >
                          {" "}
                      ...
                      <div className="admin-user-service-tile__dates">
                        ...
                        <p>
                          <span>البداية</span>
                          <strong>{formatDateTime(service.startedAt)}</strong>
                        </p>{" "}
                        <p>
                          <span>الانتهاء</span>
                          <strong>{formatDateTime(service.endsAt)}</strong>
                        </p>{" "}
                        {!service.manageable ? (
                          <p className="text-xs admin-text-warning">
                            {service.unmanageableReason || "غير قابلة للإدارة"}
                          </p>
                        ) : null}{" "}
                      </div>{" "}
                      <div className="admin-user-service-tile__actions">
                        ...
                        <button
                          type="button"
                          disabled={
                            Boolean(actionLoading) || !service.manageable
                          }
                          onClick={() =>
                            onRequestAction({
                              ...
                              description:
                                "تفعيل الخدمة فعلياً في قاعدة البيانات.",
                              confirmLabel: "تفعيل",
                              tone: "success",
                ...
                <>
                  ...
                  <div className="admin-user-services-grid">
                    ...
                    {(subscriptions?.subscriptions || []).map((sub) => {
                      ...
                      const canRemove = canRemoveSubscriptionRequest(
                        sub.rawStatus,
                        sub.adminDisabled,
                      );
                      const badgeClass =
                        resolveAdminSubscriptionBadgeClass(sub);
                      return (
                        <article
                          key={sub.id}
                          className="admin-user-service-tile"
                        >
                          ...
                          <div className="admin-user-service-tile__head">
                            ...
                            <span
                              className={`admin-user-service-tile__badge ${badgeClass}`}
                            >
                              {sub.status}
                            </span>{" "}
                          </div>{" "}
                          <div className="admin-user-service-tile__dates">
                            ...
                            <p>
                              <span>البداية</span>
                              <strong>{formatDateTime(sub.startedAt)}</strong>
                            </p>{" "}
                            <p>
                              <span>الانتهاء</span>
                              <strong>{formatDateTime(sub.endsAt)}</strong>
                            </p>{" "}
                            <p>
                              <span>المصدر</span>
                              <strong>{sub.activationSource}</strong>
                            </p>{" "}
                            <p>
                              <span>تجديد تلقائي</span>
                              <strong>{sub.autoRenew ? "نعم" : "لا"}</strong>
                            </p>{" "}
                            {sub.adminDisabled ? (
                              <p>
                                <span>إدارة</span>
                                <strong>تم إنهاؤه من الإدارة</strong>
                              </p>
                            ) : null}{" "}
                          </div>{" "}
                          {isActive ? (
                            <>
                                ...
                                <button
                                  ...
                                  onClick={() =>
                                    onRequestAction({
                                      ...
                                      description:
                                        "تعيين تاريخ انتهاء يدوي (UTC).",
                                      confirmLabel: "حفظ",
                                      payload: {
                                        subscriptionId: sub.id,
                                        expiresAt: manualExpiry
                                          ? new Date(manualExpiry).toISOString()
                                          : null,
                                      },
                                      refresh: [
                                        "overview",
                              ...
                              {canRemove ? (
                                <button
                                  ...
                                  onClick={() =>
                                    onRequestSubscriptionRemove?.(sub)
                                  }
                                >
                                  {" "}
                                  إزالة الاشتراك{" "}
        ...
        {activeTab === "payments" &&
          renderSectionFrame("payments", {
            ...
            children: (
              <>
                {" "}
                <p className="admin-user-payments-disclaimer">
                  {payments?.disclaimer}
                </p>{" "}
                {(payments?.reviews || []).length > 0 ? (
                  <>
                    ...
                    <div className="admin-table-wrap mt-4 overflow-x-auto">
                      ...
                      <table className="admin-user-table">
                        ...
                        <thead>
                          ...
                          <tr>
                            {" "}
                            <th>الخطة</th> <th>السعر</th> <th>حالة المراجعة</th>{" "}
                            <th>تاريخ الطلب</th> <th>تاريخ التفعيل</th>{" "}
                            <th>إثبات</th>{" "}
                          </tr>{" "}
                        </thead>{" "}
                        <tbody>
                              ...
                              <td>
                                {review.proofAvailable ? (
                                  <button
                                    ...
                                    onClick={() =>
                                      void openPaymentProof(review.requestId)
                                    }
                                  >
                                    {" "}
                                    {proofLoadingId === String(review.requestId)
                                      ? "..."
                                      : "معاينة"}{" "}
                                  </button>
                                ) : (
                    ...
                    <p className="mt-3 text-xs font-bold admin-text-warning/90">
                      {" "}
                  ...
                  <SectionEmptyDataState
                    icon="💰"
                    message="لا توجد طلبات بإثبات دفع لهذا المستخدم."
                  />
                )}{" "}
              </>
            ),
          ...
          renderSectionFrame("notifications", {
            ...
            children: (
              <div className="space-y-6">
                  ...
                  {(notifications?.notifications || []).length > 0 ? (
                    <>
                        ...
                        {(notifications?.notifications || []).map((row) => (
                          ...
                          >
                            ...
                            <div className="flex justify-between gap-3">
                              ...
                              <span
                                className={`admin-user-status ${row.isRead ? "admin-user-status--active" : ""}`}
                              >
                                {" "}
                            ...
                            <p className="mt-2 text-xs admin-text-subtle">
                      ...
                      <PaginationBar
                        ...
                        onPageChange={(page) =>
                          onPageChange("notifications", page)
                        }
                      />{" "}
                    </>
                  ) : (
                ...
                <section>
                  {" "}
                  <h4 className="admin-heading text-lg">
                    البريد الإلكتروني
                  </h4>{" "}
                  {(emails?.emails || []).length > 0 ? (
                    <>
                        ...
                        {(emails?.emails || []).map((row) => (
                          ...
                          >
                            ...
                            <p className="mt-2 text-xs admin-text-subtle">
                              ...
                              {row.sentAt
                                ? ` — أُرسل: ${formatDateTime(row.sentAt)}`
                                : " — لم يُرسل بعد"}{" "}
                            </p>{" "}
                            {row.error ? (
                              <p className="mt-1 text-xs admin-text-warning">
                                فشل إرسال الرسالة
                              </p>
                            ) : null}{" "}
                          </article>
                      ...
                      <PaginationBar
                        ...
                        onPageChange={(page) => onPageChange("emails", page)}
                      />{" "}
                    </>
                  ) : (
                    <div className="mt-3">
                      ...
                      <SectionEmptyDataState
                        icon="✉️"
                        message="لا توجد رسائل بريد مسجلة."
                      />{" "}
                    </div>
                  )}{" "}
                </section>{" "}
                  ...
                  <div className="admin-user-timeline-filters">
                    ...
                    {TIMELINE_FILTER_OPTIONS.map((item) => (
                      <button
                        key={item.id}
                        type="button"
                        className={`admin-user-timeline-filters__btn ${activityFilter === item.id ? "is-active" : ""}`}
                    ...
                    <>
                      ...
                      <ol className="admin-user-timeline">
                        ...
                        {(activity?.events || []).map((event) => (
                          <li
                            key={event.id}
                            className="admin-user-timeline__item"
                          >
                            {" "}
                            <div className="admin-user-timeline__icon">
                              {event.icon}
                            </div>{" "}
                            <div className="admin-user-timeline__content">
                              ...
                              <div className="admin-user-timeline__head">
                                {" "}
                                <p className="admin-user-timeline__title">
                                  {event.title}
                                </p>{" "}
                                <span className="admin-user-timeline__type">
                                  {event.label}
                                </span>{" "}
                              </div>{" "}
                      ...
                      <PaginationBar
                        ...
                        onPageChange={(page) => onPageChange("activity", page)}
                      />{" "}
                    </>
                  ) : (
                    <SectionEmptyDataState
                      icon="🕒"
                      message="لا توجد أحداث ضمن هذا الفلتر."
                    />
                  )}{" "}
                </>
              ) : (
                  ...
                  <>
                    ...
                    <div className="mt-4 space-y-3">
                      ...
                      {(notes?.notes || []).map((note) => (
                        <article
                          key={note.id}
                          className={`admin-user-service-tile ${note.is_pinned ? "is-pinned" : ""}`}
                        >
                          {" "}
                    ...
                    <PaginationBar
                      ...
                      onPageChange={(page) => onPageChange("notes", page)}
                    />{" "}
                  </>
                ) : (
                  <div className="mt-4">
            ...
            children: (
              <>
                ...
                {MANAGEMENT_SECTIONS.map((section) => (
                  <section
                    key={section.title}
                    className="admin-user-management-section admin-user-section-card admin-user-management-section--calm"
                  >
                    {" "}
                    <h4 className="admin-user-drawer__section-title">
                      {section.title}
                    </h4>{" "}
                      ...
                      {section.actions.map((item) => {
                        ...
                        return (
                          <button
                            ...
                            title={
                              selfBlocked
                                ? "لا يمكن تنفيذ هذا الإجراء على حسابك"
                                : undefined
                            }
                      ...
                      (audit?.logs || []).length > 0 ? (
                        ...
                        <div className="mt-4">
                          ...
                          <SectionEmptyDataState
                            icon="🛡️"
                            message={
                              audit?.message || ADMIN_SECTION_EMPTY_MESSAGE
                            }
                          />{" "}
                        </div>
                      ),
