...
export default function AdminHubHero({
  ...
  const displayName =
    ...
    user?.email?.split("@")[0] ||
    "المدير";
  const serverLabel = isRefreshing
    ? "جاري المزامنة"
    : serverOnline
      ? "السيرفر متصل"
      : "تحقق من الاتصال";
  const serverClass = isRefreshing
    ? "is-sync"
    : serverOnline
      ? "is-online"
      : "is-offline";
    ...
    <header className="admin-hub-hero admin-hub-hero--premium admin-section">
      ...
      <div className="admin-hub-hero__main">
        ...
        <div className="admin-hub-hero__intro">
          {" "}
          <p className="admin-hub-hero__eyebrow">
            HasaN CharT World · Admin
          </p>{" "}
          <h1 className="admin-heading text-3xl md:text-4xl">لوحة التحكم</h1>{" "}
          <p className="admin-hub-hero__desc">
            {" "}
            مركز إدارة المنصة — المستخدمون، المالية، الطلبات، والنشاط في مكان
            واحد.{" "}
          </p>{" "}
          <div className="admin-hub-hero__admin-meta">
            ...
            <div className="admin-hub-hero__admin-chip">
              {" "}
              <span className="admin-hub-hero__admin-label">
                المدير الحالي
              </span>{" "}
              <span className="admin-hub-hero__admin-name">{displayName}</span>{" "}
              <span className="admin-hub-hero__admin-role">
                {roleLabel}
              </span>{" "}
            </div>{" "}
          </div>{" "}
      ...
      <div className="admin-hub-hero__status-bar">
        ...
        <div className="admin-hub-hero__status-items">
          {" "}
          <span className={`admin-hub-hero__server ${serverClass}`}>
            {serverLabel}
          </span>{" "}
        ...
        <button
          type="button"
          className="admin-hub-hero__refresh"
          onClick={onRefresh}
