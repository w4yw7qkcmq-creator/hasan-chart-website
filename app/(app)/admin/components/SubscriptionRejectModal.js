...
export default function SubscriptionRejectModal({
  ...
  const selectedReason = useMemo(
    () =>
      SUBSCRIPTION_REJECTION_REASONS.find(
        (item) => item.value === reasonValue,
      ) || null,
    [reasonValue],
  );
  ...
  const handleConfirm = () => {
    ...
    onConfirm?.({
      ...
      notes: trimmedNotes,
      message: buildSubscriptionRejectionMessage(
        ...
        <div className="admin-crm-action-modal__body">
          ...
          <label className="admin-crm-action-modal__field">
            ...
            <span className="admin-crm-action-modal__field-hint">
              {" "}
              {notes.length.toLocaleString("ar")} /{" "}
              {NOTES_MAX_LENGTH.toLocaleString("ar")}{" "}
            </span>{" "}
          </label>{" "}
          {validationError ? (
            <p
              className="admin-crm-action-modal__validation-error"
              role="alert"
            >
              {" "}
              {validationError}{" "}
