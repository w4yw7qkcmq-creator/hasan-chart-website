    ...
    <section className="admin-subscription-timeline" aria-label="سجل الطلب">
      <header className="admin-subscription-timeline__summary">
        <div className="admin-subscription-timeline__summary-item">
          <span className="admin-subscription-timeline__summary-label">إجمالي الأحداث</span>
          <strong className="admin-subscription-timeline__summary-value">
            {summaryData.totalEvents}
          </strong>
        </div>
        <div className="admin-subscription-timeline__summary-item">
          <span className="admin-subscription-timeline__summary-label">آخر تحديث</span>
          <strong className="admin-subscription-timeline__summary-value">
            {summaryData.lastUpdateLabel}
          </strong>
        </div>
        <div className="admin-subscription-timeline__summary-item">
          <span className="admin-subscription-timeline__summary-label">آخر إداري</span>
      ...
      {sparse ? (
        <article className="admin-subscription-timeline__empty">
          <span className="admin-subscription-timeline__empty-icon" aria-hidden="true">
            🕘
          </span>
        ...
        <ol className="admin-subscription-timeline__list">
          {events.map((event, index) => (
            ...
            >
              <div className="admin-subscription-timeline__marker" aria-hidden="true">
              ...
              <div className="admin-subscription-timeline__content">
                <div className="admin-subscription-timeline__head">
                  <span className="admin-subscription-timeline__icon" aria-hidden="true">
                    {event.icon}
                  </span>
                  <h4 className="admin-subscription-timeline__title">{event.title}</h4>
                </div>

                {event.description ? (
                  <p className="admin-subscription-timeline__description">{event.description}</p>
                ) : null}
                ...
                <div className="admin-subscription-timeline__meta">
                  ...
                  {event.adminEmail && event.type !== "created" && event.type !== "payment_proof" ? (
                    <span className="admin-subscription-timeline__admin">{event.adminEmail}</span>
                  ) : null}
                </div>
              </div>
            </li>
