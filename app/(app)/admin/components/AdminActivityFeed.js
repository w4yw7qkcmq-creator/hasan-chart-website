    ...
    <section className="order-4 admin-section admin-activity-feed admin-activity-feed--premium p-5 md:p-6">
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {refreshing ? (
            <span className="admin-activity-feed__badge admin-activity-feed__badge--info">⟳ تحديث</span>
          ) : null}
          {showSoftWarning ? (
          ...
          <button type="button" onClick={onRefresh} className="admin-btn-surface px-4 py-2 text-xs font-black">
            تحديث النشاط
          </button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="admin-premium-empty mt-5">
          ...
          <p className="admin-premium-empty__title">لا توجد نشاطات حديثة</p>
          <p className="admin-premium-empty__desc">ستظهر هنا أحداث المستخدمين والإدارة فور حدوثها.</p>
          {showSoftWarning ? (
        ...
        <div className="admin-activity-feed__list mt-5">
          {events.map((event) => {
            const fullDate = event.occurredAt ? new Date(event.occurredAt).toLocaleString("ar") : "—";
            ...
            return (
              <button
                key={event.id}
                type="button"
                className={`admin-activity-feed__item admin-activity-feed__item--premium ${toneClass} ${
                  clickable ? "admin-activity-feed__item--clickable" : ""
                }`}
                onClick={() => {
