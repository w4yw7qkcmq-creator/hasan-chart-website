...
const STAT_CARDS = [
  {
    ...
    icon: "👥",
    filterHint: "عرض جميع المستخدمين",
  },
  {
    key: "active",
    label: "المستخدمون النشطون",
    icon: "🟢",
    filterHint: "تصفية: نشط",
  },
  {
    key: "suspended",
    label: "المعلقون",
    icon: "🟡",
    filterHint: "تصفية: معلق",
  },
  ...
  {
    key: "priceAlertsActive",
    label: "تنبيهات نشطة",
    icon: "🔔",
    filterHint: "تصفية: تنبيهات نشطة",
  },
  {
    key: "expiredSubscriptions",
    label: "الاشتراكات المنتهية",
    icon: "⌛",
    filterHint: "تصفية: اشتراكات منتهية",
  },
  {
    key: "newToday",
    label: "جدد اليوم",
    icon: "✨",
    filterHint: "تصفية: مسجلون اليوم",
  },
  {
    key: "newThisWeek",
    label: "جدد هذا الأسبوع",
    icon: "📅",
    filterHint: "تصفية: مسجلون هذا الأسبوع",
  },
];
    ...
    <section className="admin-user-dashboard">
      ...
      <div className="admin-user-dashboard__head">
        ...
        <div>
          ...
          <h3 className="admin-heading text-xl">
            نظرة عامة على المستخدمين
          </h3>{" "}
        </div>{" "}
        {stats?.scanComplete === false ? (
          <span className="admin-user-dashboard__hint">
            مؤشرات الخدمات من عينة {stats?.scannedSampleSize || 0} مستخدم
          </span>
        ) : (
          <span className="admin-user-dashboard__hint">
            انقر على بطاقة لتطبيق الفلتر مباشرة
          </span>
        )}{" "}
      </div>{" "}
        ...
        {STAT_CARDS.map((card) => {
          ...
          return (
            ...
            >
              {" "}
              <div
                className="admin-user-dashboard__card-icon"
                aria-hidden="true"
              >
                {" "}
                {card.icon}{" "}
              ...
              <div className="min-w-0 flex-1">
                {" "}
                <p className="admin-user-dashboard__card-label">
                  {card.label}
                </p>{" "}
                {loading ? (
                  <div
                    className="admin-user-dashboard__card-skeleton"
                    aria-hidden="true"
                  />
                ) : (
