...
export async function generateMetadata({ params }) {
  ...
  const post = await fetchPublishedContentPostBySlug("result", resolved.slug).catch(() => null);
  if (!post) {
    return { title: "النتيجة غير موجودة | HasaN CharT World" };
  }

  return buildArticleMetadata({
    path: `/results/${post.slug}`,
    title: post.title,
    description: post.summary || post.body.slice(0, 160),
    image: post.image_url,
    publishedTime: post.published_at,
    modifiedTime: post.updated_at,
    section: "HasaN CharT Result",
    tags: [post.category, post.highlight_value, "HasaN CharT Result"].filter(Boolean),
  });
}
...
export default async function ResultDetailPage({ params }) {
  ...
  const breadcrumbs = [
    ...
    { label: post.title, href: `/results/${post.slug}` },
  ];
  ...
  const articleJsonLd = buildContentPostArticleJsonLd({
    ...
    articleSection: "HasaN CharT Result",
  });
