    ...
    <nav className="analysis-request-hub" aria-label={title}>
      ...
      <div className="analysis-request-hub__grid">
        {ANALYSIS_REQUEST_HUB_LINKS.map((link) => (
          <Link key={link.href} href={link.href} className="analysis-request-hub__link">
            {link.label}
          </Link>
        ))}
      </div>
    ...
    <nav className="analysis-request-assets" aria-label="أصول سريعة">
      ...
      <div className="analysis-request-assets__grid">
        {ANALYSIS_REQUEST_ASSET_LINKS.map((link) => (
          <Link key={link.href} href={link.href} className="analysis-request-assets__link">
            {link.label}
          </Link>
        ))}
      </div>
...
export default function AnalysisRequestAuthenticated() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        const message =
          result.error || "حدث خطأ أثناء إرسال طلب التحليل، يرجى المحاولة مرة أخرى.";
    ...
    <main className="analysis-request-page">
      ...
      <div className="analysis-request-page__inner">
        ...
        <header className="analysis-request-hero">
          <span className="analysis-request-hero__eyebrow">تحليل مخصص من الخبراء</span>
          <h1 className="analysis-request-hero__title">طلب تحليل</h1>
          <p className="analysis-request-hero__text">
            اطلب تحليلاً مفصلاً لأصل أو زوج محدد من خبراء HasaN CharT World. التحليل
            الاحترافي يصدر من فريق بشري، والذكاء الاصطناعي أداة مساعدة منفصلة عند
            طلبها صراحةً.
          </p>
        </header>
        ...
        <section className="analysis-request-note">
          <p>
            املأ الزوج والفريم المطلوبين بدقة. سيصلك التحليل داخل المنصة بعد مراجعته من
            الفريق.
          </p>
        </section>

        <section className="analysis-request-form-card" aria-label="نموذج طلب التحليل">
