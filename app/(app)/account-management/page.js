...
function AccountManagementAuthenticated() {
  ...
  const [successModal, setSuccessModal] = useState({
    ...
    message:
      "تم إرسال طلب إدارة الحساب إلى فريق الإدارة وسيتم التواصل معك قريباً.",
  });
  ...
  const validateImageFile = (file) => {
    ...
    if (!allowedImageTypes.includes(file.type)) {
      return { valid: false, message: "يسمح فقط برفع صور JPG أو PNG أو WEBP." };
    }
    if (file.size > maxImageSize) {
      return { valid: false, message: "الحد الأقصى لحجم الصورة هو 15MB." };
    }
    return { valid: true };
  };
  ...
  return (
    <main className="min-h-screen ui-page-dark admin-text py-12 px-4">
      {" "}
      <SuccessModal
      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        <div className="space-y-3">
          ...
          {remainingTime && (
            <div className="ui-account-info-banner">
              {" "}
        ...
        <form onSubmit={handleSpotSubmit} className="box space-y-6">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div className="flex flex-col md:col-span-2">
              {" "}
              <label className="mb-2 admin-text-muted">
                أرفق صورة لرأس مالك
              </label>{" "}
              <input
                type="file"
                ...
                onChange={(e) =>
                  handleFileChange(spot, setSpot, e.target.files[0])
                }
                className="p-4 rounded-2xl ui-input-dark border admin-panel-border admin-text"
              />{" "}
              {spot.file && (
        ...
        <form onSubmit={handleFuturesSubmit} className="box space-y-6">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <input
              ...
              onChange={(e) =>
                setFutures({ ...futures, telegram: e.target.value })
              }
              className="input"
              required
            />{" "}
            <input
              ...
              onChange={(e) =>
                setFutures({ ...futures, capital: e.target.value })
              }
              className="input"
              required
            />{" "}
            <input
              ...
              onChange={(e) =>
                setFutures({ ...futures, apiKey: e.target.value })
              }
              className="input"
              required
            />{" "}
            <input
              ...
              onChange={(e) =>
                setFutures({ ...futures, secretKey: e.target.value })
              }
              className="input"
              required
            />{" "}
            <div className="flex flex-col md:col-span-2">
              ...
              <input
                ...
                accept="image/jpeg,image/jpg,image/png,image/webp"
                onChange={(e) =>
                  handleFileChange(futures, setFutures, e.target.files[0])
                }
                className="p-4 rounded-2xl ui-input-dark border admin-panel-border admin-text"
              />{" "}
              {futures.file && (
                <p className="mt-2 text-sm font-bold admin-text-success">
                  {" "}
                  تم اختيار: {futures.file.name} (
                  {formatFileSize(futures.file.size)}){" "}
                </p>
              )}{" "}
            </div>{" "}
          </div>{" "}
        ...
        <form onSubmit={handleForexSubmit} className="box space-y-6">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div className="flex flex-col md:col-span-2">
              {" "}
              <label className="mb-2 admin-text-muted">
                أرفق صورة توضح قيمة الحساب
              </label>{" "}
              <input
                type="file"
                accept="image/jpeg,image/jpg,image/png,image/webp"
                onChange={(e) =>
                  handleFileChange(forex, setForex, e.target.files[0])
                }
                className="p-4 rounded-2xl ui-input-dark border admin-panel-border admin-text"
              />{" "}
              {forex.file && (
                <p className="mt-2 text-sm font-bold admin-text-success">
                  {" "}
                  تم اختيار: {forex.file.name} (
                  {formatFileSize(forex.file.size)}){" "}
                </p>
              )}{" "}
            </div>{" "}
          </div>{" "}
