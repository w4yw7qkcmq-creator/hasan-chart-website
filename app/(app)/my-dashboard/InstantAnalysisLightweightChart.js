...
function toSeriesCandles(candles) {
  const sorted = [...(candles || [])]
    .filter(
      (c) => Number.isFinite(c?.open) && Number.isFinite(c?.close) && c?.time,
    )
    ...
    void import("lightweight-charts").then(
      ({ createChart, ColorType, CrosshairMode, LineStyle }) => {
        ...
        chart = createChart(container, {
          ...
          height: Math.max(
            360,
            Math.min(520, Math.round(container.clientWidth * 0.48)),
          ),
          layout: {
        ...
        for (const ann of annotations || []) {
          ...
          const label =
            ann.label ||
            LEGEND_ITEMS.find((item) => item.key === ann.type)?.label ||
            ann.type;
          if (
            ann.type === "BOS" ||
            ann.type === "CHOCH" ||
            ...
            addPriceLine(
              ...
              color,
              label,
              ann.type === "LIQUIDITY" ? LineStyle.Dotted : LineStyle.Solid,
            );
          }
          if (ann.type === "TARGET") {
            addPriceLine(
        ...
        const resize = () => {
          ...
          chart.applyOptions({
            ...
            height: Math.max(
              ...
              Math.min(
                520,
                Math.round(containerRef.current.clientWidth * 0.48),
              ),
            ),
          });
        };
  ...
  return (
    <figure className="ia-v3-chart-wrap">
      ...
      <div className="ia-v3-chart-head">
        {" "}
        <strong>{symbol}</strong> <span>{timeframeLabel}</span>{" "}
