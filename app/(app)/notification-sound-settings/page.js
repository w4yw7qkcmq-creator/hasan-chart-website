...
function ToggleRow({
  ...
  return (
    <label className="flex items-start justify-between gap-4 rounded-2xl border admin-panel-border ui-glass-04 p-4">
      {" "}
      <span>
        {" "}
...
function NotificationKeyRow({
  ...
  return (
    <div className="space-y-3 rounded-2xl border admin-panel-border ui-glass-04 p-4">
      {" "}
      <div className="flex flex-wrap items-start justify-between gap-3">
        {" "}
        <div>
          {" "}
          <p className="text-sm font-black admin-text">{label}</p>{" "}
          <p className="mt-1 text-xs font-bold admin-text-subtle">
            {notificationKey}
          </p>{" "}
        </div>{" "}
        <label className="flex items-center gap-2 text-xs font-black admin-text-muted">
          {" "}
          <span>تشغيل</span>{" "}
          <input
            type="checkbox"
...
export default function NotificationSoundSettingsPage() {
  ...
  const notificationKeyDefinitions = useMemo(
    () => getNotificationSoundKeyDefinitions(),
    [],
  );
  ...
  const persistKeyPatch = useCallback(
    async (notificationKey, keyPatch, { localOnly = false } = {}) => {
      ...
      await persistPatch({
        sound_preferences: { [notificationKey]: keyPatch },
      });
    },
  ...
  if (!authResolved) {
    return (
      <main className="notificationsPage relative min-h-[calc(100vh-120px)] overflow-hidden rounded-[34px] border p-6">
        {" "}
        <div className="relative z-10 mx-auto max-w-3xl text-center text-sm font-bold admin-text-muted">
          {" "}
  ...
  return (
    <main className="notificationsPage relative min-h-[calc(100vh-120px)] overflow-hidden rounded-[34px] border p-6 shadow-[0_25px_90px_rgba(0,102,255,0.16)]">
      ...
      <div className="relative z-10 mx-auto max-w-3xl space-y-6">
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            ...
            <ToggleRow
              ...
              disabled={saving}
              onChange={(checked) =>
                void persistPatch({ sound_enabled: checked })
              }
            />{" "}
            <div className="rounded-2xl border admin-panel-border ui-glass-04 p-4">
              ...
              <input
                ...
                onChange={(event) => {
                  ...
                  setSettings((current) => ({
                    ...current,
                    sound_volume: nextVolume,
                  }));
                }}
                onMouseUp={(event) =>
                  void persistPatch({
                    sound_volume: Number(event.currentTarget.value) / 100,
                  })
                }
                onTouchEnd={(event) =>
                  void persistPatch({
            ...
            <div className="space-y-3">
              {" "}
              <h2 className="ui-public-seo-title text-lg">
                أنواع الإشعارات
              </h2>{" "}
        ...
        {message ? (
          <p className="admin-banner-success px-4 py-3 text-sm font-bold">
            {" "}
            {message}{" "}
          </p>
        ) : null}{" "}
        {error ? (
          <p className="admin-banner-danger px-4 py-3 text-sm font-bold">
            {" "}
            {error}{" "}
          </p>
        ) : null}{" "}
